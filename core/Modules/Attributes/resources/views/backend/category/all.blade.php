...
@section('title') {{__('Product Category')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Categories')}}</h3>
                <p class="text-xs text-muted">{{__('Manage product categories')}}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            @can('product-category-delete')
                <x-bulk-action permissions="product-category-delete"/>
                <a href="{{route('tenant.admin.product.category.trash.all')}}"
        ...
        <table class="w-full text-left" id="categoryTable">
            ...
            @foreach($all_category as $key => $category)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5">
                        <span class="tw-pill {{ $category->status?->name === 'Publish' ? 'tw-pill-success' : 'tw-pill-warning' }}">
                            {{$category->status?->name ?? __('N/A')}}
                        </span>
                    </td>
                    <td class="px-4 py-3.5">
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            @can('product-category-delete')
                                <button type="button" class="tw-btn-icon tw-btn-icon-danger swal_delete_button"
                                        data-route="{{ route('tenant.admin.product.category.delete', $category->id) }}"
                                        title="{{__('Delete')}}">
        ...
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            <div class="flex items-center gap-3 px-5 py-4 border-b border-main flex-shrink-0">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-[11px] text-muted">{{__('Add a new product category')}}</p>
                </div>
            ...
            <form action="{{ route('tenant.admin.product.category.new') }}" method="post" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                @csrf
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" class="lnd-input" rows="2" placeholder="{{__('Description')}}"></textarea>
                    </div>
                    <div>
...
<div id="category_edit_modal" class="fixed inset-0 z-[800] hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            ...
            <form action="{{ route('tenant.admin.product.category.update') }}" method="post" class="flex flex-col flex-1 min-h-0">
                @csrf
                ...
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-1.5">{{__('Description')}}</label>
                        <textarea name="description" id="edit_description" class="lnd-input" rows="2" placeholder="{{__('Description')}}"></textarea>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-1.5">{{__('Image')}}</label>
...
@section('scripts')
    ...
    @can('product-category-delete')
        <x-bulk-action-js :url="route('tenant.admin.product.category.bulk.action')"/>
    @endcan
    <script>
    (function ($) {
        "use strict";
