...
@section('title') {{__('Order Success Page Settings')}} @endsection
        ...
        <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden">
            <div class="px-4 sm:px-6 py-4 border-b border-main flex items-center gap-3">
                <div class="w-9 h-9 rounded-lg bg-success-soft flex items-center justify-center flex-shrink-0">
                    <i class="mdi mdi-check-circle-outline text-success text-base"></i>
                </div>
                <div>
                    <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Order Success Page Settings')}}</h3>
                    <p class="text-xs text-muted">{{__('Configure the success page content')}}</p>
                </div>
            </div>

            <div class="p-4 sm:p-6">
                <form action="{{route(route_prefix().'admin.product.order.success.page')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="space-y-5">
                        <div>
                        ...
                        <div>
                            ...
                            <div class="bg-secondary border border-main rounded-xl px-4 py-2.5 focus-within:border-primary transition">
                                <textarea name="site_order_success_page_description" rows="6"
                                          placeholder="{{__('Enter description')}}"
                                          class="w-full bg-transparent text-sm text-dark placeholder-subtle outline-none border-none focus:ring-0 p-0 resize-none">{{get_static_option('site_order_success_page_description')}}</textarea>
                            </div>
                        </div>
