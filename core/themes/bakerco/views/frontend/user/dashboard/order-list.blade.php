...
<div class="bk-dash-section-title">
    <i class="mdi mdi-package-variant-closed" style="color:var(--bk-rose);"></i> {{ __('My Orders') }}
</div>
        ...
        <table class="bk-dash-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:var(--bk-muted);">{{ $order->created_at->format('d M Y') }}</td>
                    <td>
                        <span class="bk-status-badge" style="
                            background:{{ $os_ok ? 'rgba(76,175,80,.12)' : ($os==='cancel'?'rgba(212,133,106,.12)':'rgba(255,193,7,.12)') }};
                            color:{{ $os_ok ? '#4CAF50' : ($os==='cancel'?'var(--bk-rose)':'#D4A017') }};">
                            {{ __($os) }}
                        </span>
                        <span class="bk-status-badge ms-1" style="
                    ...
                    <td style="font-weight:700;color:var(--bk-rose);">{{ amount_with_currency_symbol($order->total_amount) }}</td>
                    <td>
                        <a href="{{ theme_user_package_orders_url() }}/{{ $order->id }}" class="bk-btn bk-btn-outline bk-btn-sm">
                        ...
                        <form class="order-cancel-form d-inline" action="{{ theme_user_cancel_order_url() }}" method="POST">
                            ...
                            <button type="submit" class="cancel-btn bk-btn bk-btn-sm" style="background:rgba(212,133,106,.1);color:var(--bk-rose);border:1.5px solid var(--bk-rose);">
...
<div class="bk-dash-box" style="padding:56px;text-align:center;">
    <i class="mdi mdi-package-variant" style="font-size:52px;color:var(--bk-border);display:block;margin-bottom:14px;"></i>
    <p style="color:var(--bk-muted);margin-bottom:20px;">{{ __('No orders found.') }}</p>
    <a href="{{ theme_shop_url() }}" class="bk-btn bk-btn-rose">{{ __('Start Shopping') }}</a>
</div>
@endif
