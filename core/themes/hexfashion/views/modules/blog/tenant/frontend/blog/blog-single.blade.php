...
<div class="container" style="padding-top:40px;padding-bottom:72px;">
    <div class="row g-4">
        ...
        {{-- Article --}}
        <div class="col-lg-8">
            <div class="hf-blog-article">

                @if($post->has_image())
                <div class="hf-blog-article-img">
                    <img src="{{ $post->image_url('full') }}" alt="{{ $post->title() }}">
                </div>
                @endif

                <div class="hf-blog-article-body">
                    @if($post->category())
                    <div style="margin-bottom:14px;">
                        <a href="{{ $post->category()->url() }}" class="hf-blog-cat-badge" style="position:static;display:inline-block;">{{ $post->category()->name() }}</a>
                    </div>
                    @endif

                    <h1 class="hf-blog-article-title" style="font-size:26px;font-weight:800;color:#1a1a1a;line-height:1.3;margin:0 0 12px;">{{ $post->title() }}</h1>

                    <div class="hf-blog-article-divider">
                    ...
                    <div class="hf-blog-article-content">
                        {!! $post->content() !!}
                    </div>

                    {{-- Tags & Share --}}
                    <div class="hf-blog-article-footer">
                        <div style="display:flex;flex-wrap:wrap;gap:8px;">
                            @foreach($post->tags() as $tag)
                            <a href="{{ $tag->url() }}" class="hf-blog-article-tag">
            ...
            {{-- Comments --}}
            <div class="hf-blog-comments-card" id="comment-area">
                <h3 class="hf-blog-comments-title">
                    {{ __('Comments') }}
                ...
                <div id="comment_data" data-items="{{ theme_comments()->count() }}">
                    ...
                    <div class="hf-comment-item">
                        <div class="hf-comment-avatar">
                        ...
                        <div style="flex:1;">
                            ...
                            <div class="hf-comment-date">{{ $comment->date() }}</div>
                            <p class="hf-comment-body">{!! $comment->body() !!}</p>
                            @if($comment->can_reply())
                            <button class="hf-comment-reply-btn btn-replay" data-comment_id="{{ $comment->id() }}">
                                <i class="las la-reply"></i> {{ __('Reply') }}
                            </button>
                            @endif
                            @if($comment->replies()->count())
                            <div class="hf-comment-replies">
                                @foreach($comment->replies() as $reply)
                                <div class="hf-comment-item" style="padding:10px 0 0;">
                                    <div class="hf-comment-avatar" style="width:36px;height:36px;font-size:14px;">
                                        {{ strtoupper(substr($reply->author() ?? 'U', 0, 1)) }}
                                    </div>
                                    <div style="flex:1;">
                                        <div class="hf-comment-name">{{ $reply->author() }}</div>
                                        <div class="hf-comment-date">{{ $reply->date() }}</div>
                                        <p class="hf-comment-body" style="margin:0;">{!! $reply->body() !!}</p>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                ...
                <div style="text-align:center;margin-top:20px;">
                    <button class="hf-btn hf-btn-outline" id="load_more_comment_button">{{ __('Load More Comments') }}</button>
                </div>
                @endif
            </div>
            ...
            <div class="hf-comment-form" id="blog-comment-form">
                <h3 class="hf-comment-form-title">{{ __('Leave a Comment') }}</h3>
                <input type="hidden" name="blog_id" value="{{ $post->id() }}">
                <input type="hidden" name="user_id" value="{{ theme_auth()->id() }}">
                <input type="hidden" name="comment_id" value="">
                <div class="error-wrap mb-3"></div>
                <div class="mb-3">
                    <label class="hf-form-label">{{ __('Name') }}</label>
                    <input type="text" id="commented_by" class="hf-form-input" value="{{ theme_current_user()->name }}" readonly style="background:#fafafa;">
                </div>
                <div class="mb-3">
                    <label class="hf-form-label">{{ __('Comment') }} <span class="hf-form-required">*</span></label>
                    <textarea id="comment_content" class="hf-form-input hf-form-textarea" rows="4" placeholder="{{ __('Write your comment…') }}"></textarea>
                </div>
                <button type="button" id="comment_submit_btn" class="hf-btn hf-btn-primary">
                    <i class="las la-paper-plane"></i> {{ __('Post Comment') }}
                </button>
            </div>
            @else
            <div class="hf-comment-form" style="text-align:center;">
                <p style="color:#888;margin-bottom:16px;">{{ __('Sign In To Leave Your Comment') }}</p>
                <a href="{{ theme_login_url() }}" class="hf-btn hf-btn-primary">
        ...
        <div class="col-lg-4">
            ...
            <div class="hf-blog-sidebar-card">
                <div class="hf-blog-sidebar-title">{{ __('Recent Posts') }}</div>
                @foreach(theme_blogs()->take(4) as $recent)
                <div class="hf-blog-recent-item">
                    @if($recent->has_image())
                    <div class="hf-blog-recent-thumb">
                        <a href="{{ $recent->url() }}"><img src="{{ $recent->image_url() }}" alt="{{ $recent->title() }}"></a>
                    </div>
                    @endif
                    <div style="flex:1;min-width:0;">
                        <a href="{{ $recent->url() }}" class="hf-blog-recent-title">{!! $recent->title() !!}</a>
                        <div class="hf-blog-recent-date"><i class="las la-calendar" style="color:#E8603C;"></i> {{ $recent->date() }}</div>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="hf-blog-sidebar-card">
                ...
                <ul class="hf-blog-cat-list">
                    ...
                    <li class="hf-blog-cat-item">
                        <a href="{{ $cat->url() }}" class="hf-blog-cat-link {{ $post->category()?->id() == $cat->id() ? 'active' : '' }}">
            ...
            <div class="hf-blog-sidebar-card">
                <div class="hf-blog-sidebar-title">{{ __('Tags') }}</div>
...
(function ($) {
    $(document).ready(function () {
        ...
        $(document).on('click', '.btn-replay', function () {
            ...
            var parent_name = $(this).closest('.hf-comment-item').find('.title').data('parent_name');
