...
@if(session('msg'))
<div style="padding:12px 16px;margin-bottom:18px;border-radius:var(--ch-radius);background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.3);color:#166534;font-size:13px;">
    {{ session('msg') }}
</div>
@endif

@if($errors->any())
<div style="padding:12px 16px;margin-bottom:18px;border-radius:var(--ch-radius);background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.3);color:#991b1b;font-size:13px;">
...
<div class="ch-dash-card-header" style="background:var(--ch-warm);border:1px solid var(--ch-border);border-radius:var(--ch-radius);margin-bottom:20px;">
    <i class="mdi mdi-headset" style="margin-right:8px;"></i>
...
<div class="ch-dash-card">
    <form action="{{ theme_user_ticket_store_url() }}" method="POST" enctype="multipart/form-data">
        ...
        @php $inp = 'width:100%;padding:10px 14px;border:1.5px solid var(--ch-border);border-radius:var(--ch-radius);font-size:14px;font-family:inherit;outline:none;background:#fff;transition:border-color .2s;'; @endphp
        @php $lbl = 'font-size:12px;font-weight:700;color:var(--ch-text);margin-bottom:5px;display:block;'; @endphp
        <div class="row g-3 mb-3">
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Title') }} <span style="color:var(--ch-red);">*</span></label>
                <input type="text" name="title" value="{{ old('title') }}" style="{{ $inp }}" placeholder="{{ __('Ticket title') }}" required>
            </div>
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Subject') }} <span style="color:var(--ch-red);">*</span></label>
                <input type="text" name="subject" value="{{ old('subject') }}" style="{{ $inp }}" placeholder="{{ __('Brief subject') }}" required>
            </div>
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Priority') }} <span style="color:var(--ch-red);">*</span></label>
            ...
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Department') }} <span style="color:var(--ch-red);">*</span></label>
            ...
            <div class="col-12">
                <label style="{{ $lbl }}">{{ __('Description') }} <span style="color:var(--ch-red);">*</span></label>
