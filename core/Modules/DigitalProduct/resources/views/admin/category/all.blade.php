...
@section('title') {{__('Digital Product Category')}} @endsection
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                <i class="mdi mdi-shape-outline text-primary text-base"></i>
            </div>
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Digital Product Categories')}}</h3>
                <p class="text-xs text-muted">{{__('Manage digital product categories')}}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            ...
            @can('product-category-create')
                <button type="button" onclick="openModal('category_create_modal')"
                    ...
                    <i class="mdi mdi-plus-circle-outline text-base"></i> {{__('New Category')}}
                </button>
            @endcan
        </div>
    </div>
        ...
        <table class="w-full text-left" id="categoryTable">
            <thead>
                <tr class="border-b border-main">
                    ...
                    <th class="px-4 py-3 text-[10px] font-bold text-muted uppercase tracking-widest hidden md:table-cell">{{__('Description')}}</th>
                    <th class="px-4 py-3 text-[10px] font-bold text-muted uppercase tracking-widest">{{__('Image')}}</th>
                    <th class="px-4 py-3 text-[10px] font-bold text-muted uppercase tracking-widest hidden md:table-cell">{{__('Type')}}</th>
            ...
            @foreach($all_category ?? [] as $key => $category)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5 hidden md:table-cell">
                        <span class="text-sm text-muted">{{ Str::limit($category->description, 50) }}</span>
                    </td>
                    <td class="px-4 py-3.5">
                    ...
                    <td class="px-4 py-3.5 hidden md:table-cell">
                        <span class="tw-pill tw-pill-info">{{$category->product_type?->name ?? __('N/A')}}</span>
                    </td>
                    <td class="px-4 py-3.5">
                        <span class="tw-pill {{ $category->status == 1 ? 'tw-pill-success' : 'tw-pill-warning' }}">
                    ...
                    <td class="px-4 py-3.5">
                        <div class="flex items-center justify-end gap-1.5">
                            @can('product-category-edit')
                                ...
                                @endphp
                                <button type="button"
                                        class="tw-btn-icon tw-btn-icon-edit category_edit_btn"
                                        title="{{__('Edit')}}"
                                        data-id="{{$category->id}}"
                            ...
                            @can('product-category-delete')
                                <button type="button" class="tw-btn-icon tw-btn-icon-danger swal_delete_button"
                                        data-route="{{ route('tenant.admin.digital.product.category.delete', $category->id) }}"
                                        title="{{__('Delete')}}">
...
{{-- Create Modal --}}
@can('product-category-create')
<div id="category_create_modal" class="fixed inset-0 z-[800] hidden">
    <div class="fixed inset-0 bg-black/40 backdrop-blur-sm" onclick="closeModal('category_create_modal')"></div>
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            <div class="flex items-center gap-3 px-5 py-4 border-b border-main flex-shrink-0">
                ...
                <div class="flex-1">
                    <h5 class="text-sm font-bold text-dark font-urbanist">{{__('Create Category')}}</h5>
                    <p class="text-[11px] text-muted">{{__('Add a new digital product category')}}</p>
                </div>
                <button type="button" onclick="closeModal('category_create_modal')" class="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-secondary transition">
                    <i class="mdi mdi-close text-muted"></i>
                </button>
            </div>
            <form action="{{ route('tenant.admin.digital.product.category.new') }}" method="post" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                @csrf
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-1.5">{{__('Product Type')}}</label>
                        <select name="type_id" class="lnd-input">
                ...
                <div class="flex items-center justify-end gap-2 px-5 py-4 border-t border-main flex-shrink-0" style="background: var(--color-bg-secondary);">
                    <button type="button" onclick="closeModal('category_create_modal')" class="px-4 py-2 rounded-xl text-sm font-semibold text-dark bg-secondary border border-main hover:border-hover transition">{{__('Cancel')}}</button>
...
{{-- Edit Modal --}}
@can('product-category-edit')
<div id="category_edit_modal" class="fixed inset-0 z-[800] hidden">
    <div class="fixed inset-0 bg-black/40 backdrop-blur-sm" onclick="closeModal('category_edit_modal')"></div>
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            <div class="flex items-center gap-3 px-5 py-4 border-b border-main flex-shrink-0">
                <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1">
                    <h5 class="text-sm font-bold text-dark font-urbanist">{{__('Update Category')}}</h5>
                    <p class="text-[11px] text-muted">{{__('Edit category details')}}</p>
                </div>
                <button type="button" onclick="closeModal('category_edit_modal')" class="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-secondary transition">
                    <i class="mdi mdi-close text-muted"></i>
                </button>
            </div>
            <form action="{{ route('tenant.admin.digital.product.category.update') }}" method="post" class="flex flex-col flex-1 min-h-0">
                @csrf
                ...
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-1.5">{{__('Product Type')}}</label>
                        <select name="type_id" class="lnd-input" id="edit_type">
                            <option value="" selected disabled>{{__('Select a product type')}}</option>
                            @foreach($all_product_type as $type)
                                <option value="{{ $type->id }}">{{ $type->name }}</option>
                            @endforeach
                ...
                <div class="flex items-center justify-end gap-2 px-5 py-4 border-t border-main flex-shrink-0" style="background: var(--color-bg-secondary);">
                    <button type="button" onclick="closeModal('category_edit_modal')" class="px-4 py-2 rounded-xl text-sm font-semibold text-dark bg-secondary border border-main hover:border-hover transition">{{__('Cancel')}}</button>
                    <button type="submit" class="inline-flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold text-white bg-primary hover:opacity-90 transition">
...
@section('scripts')
    ...
    @can('product-category-delete')
        <x-bulk-action-js :url="route('tenant.admin.digital.product.category.bulk.action')"/>
    @endcan
    <script>
    (function ($) {
        "use strict";
        ...
        $(document).ready(function () {
            if ($.fn.DataTable && !$.fn.dataTable.isDataTable('#categoryTable')) {
            ...
            $(document).on('click', '.category_edit_btn', function () {
                ...
                modal.find('#edit_type').val(el.data('type'));
