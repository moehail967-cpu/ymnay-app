<template>
    <div class="dashboard_posSystem__item__thumb">
        <div class="dashboard_posSystem__item__thumb__main">
            <img :src="product.img_url" alt="">
        </div>
    </div>
    <div class="dashboard_posSystem__item__contents">
        <h5 class="dashboard_posSystem__item__title">{{ product.title }}</h5>
        <span class="dashboard_posSystem__item__price">{{ getCurrencySymbolWithAmount(product.discount_price ?? product.price) }}</span>
        <div class="dashboard_posSystem__item__btn btn_flex">
            <button class="posBtn_details posBtn" @click="loadProductDetails($event, product)" v-if="!product.is_cart_able">View Details</button>
            <button class="posBtn_cart posBtn"
                v-bind:data-product-id="product.prd_id"
                @click="addToCartButton($event, product)"
                v-if="product.is_cart_able">Add to Cart</button>
        </div>
    </div>
</template>
