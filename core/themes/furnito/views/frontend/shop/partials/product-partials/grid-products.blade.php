{{-- Furnito: Shop grid products — direct fn-product-grid children --}}
@foreach($products as $product)
    ...
    <div class="fn-card h-100 d-flex flex-column">
        ...
        <div class="fn-card-body d-flex flex-column flex-grow-1">
            @if($product->category?->name)
                <div class="fn-card-cat">{{ $product->category->name }}</div>
            @endif
            <div class="fn-card-name flex-grow-1">
                <a href="{{ $url }}">{{ \Illuminate\Support\Str::words($product->name, 7) }}</a>
            </div>
            <div class="fn-card-stars mt-auto">{!! theme_star_rating($product) !!}</div>
...
@if(count($links) > 1)
<div class="fn-pagination mt-4" style="grid-column:1/-1;">
    @foreach($links as $page => $url)
        <button class="fn-page-btn {{ $page == $current_page ? 'active' : '' }}"
                data-page="{{ $page }}"
                onclick="fnFilterRequest({{ $page }})">
            {{ $page }}
        </button>
    @endforeach
</div>
