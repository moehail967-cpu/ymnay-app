...
<head>
    ...
    <title>
        ...
        @else
            @if(!empty($page_post->title ?? ''))
                {{ $page_post->title }} - {{ get_static_option('site_title') }}
            @else
                {{get_static_option('site_title')}}
...
{{--    {!! /* render themes styles */ !!}--}}
    ...
    @if(in_array('loader', $loadCoreStyle))
        <link rel="stylesheet" href="{{global_asset('assets/common/css/loader.css')}}?v=2">
    @endif
    ...
    {{-- System CSS: always loaded so default pages (shop, cart, checkout, blog) work correctly --}}
    <link rel="stylesheet" href="{{global_asset('assets/tenant/frontend/css/shop-order-custom.css')}}">
    <link rel="stylesheet" href="{{global_asset('assets/tenant/frontend/css/blog-common.css')}}">
    ...
    {{-- Theme CSS: loads after system CSS so theme styles take precedence --}}
