...
<div class="mc-dash-card">
    <div class="mc-dash-card-title"><i class="las la-box"></i> {{ __('Package Orders') }}</div>
    ...
    <div class="table-responsive">
        <table class="mc-dash-table">
            ...
            <tr>
                ...
                <td>
                    <span class="mc-badge mc-badge-{{ $po->status === 'complete' ? 'success' : ($po->status === 'pending' ? 'warning' : 'muted') }}">
                        {{ ucfirst($po->status) }}
                    </span>
                </td>
                ...
                <td class="d-flex gap-2">
                    ...
                    <a href="{{ theme_package_order_confirm_url($po->id) }}" class="mc-btn mc-btn-primary mc-btn-sm"
                       onclick="return confirm('{{ __("Confirm this order?") }}')">
                        <i class="las la-check"></i>
                    </a>
                    @endif
                    <a href="{{ theme_package_invoice_url() }}" class="mc-btn mc-btn-ghost mc-btn-sm" target="_blank">
