...
@section('content')
    ...
    <section class="el-page-section">
        <div class="container">
            ...
            <div class="el-compare-wrap">
                <h2 class="el-compare-heading">{{ __('Product Comparison') }}</h2>
                ...
                @if($compare_items->isEmpty())
                    <div class="el-empty-wrap text-center" style="padding:60px 0;">
                        <i class="las la-retweet" style="font-size:56px;color:#e0d0cc;display:block;margin-bottom:16px;"></i>
                        <h4 class="el-empty-title">{{ __('No Product Available') }}</h4>
                        <a href="{{ theme_shop_url() }}" class="el-btn el-btn-primary mt-4">{{ __('Browse Products') }}</a>
                    </div>
                @else
                            ...
                            <div class="el-compare-card">
                                <div class="el-compare-img-wrap">
                                    <a href="{{ theme_product_url($product_slug) }}">
                                        {!! render_image_markup_by_attachment_id($product->options->image, 'el-compare-img', 'grid') !!}
                                    </a>
                                </div>
                                ...
                                <div class="el-compare-body">
                                    ...
                                    <div class="el-compare-price-row">
                                        <span class="el-compare-price">{{ amount_with_currency_symbol($product->price) }}</span>
                                        @if(!empty($discount))
                                            <span class="el-card-badge" style="position:static;display:inline-block;">{{ $discount }}% {{ __('off') }}</span>
                                        @endif
                                    </div>

                                    @if(!empty($product?->options['description']))
                                        <p class="el-compare-sku">{{ __('SKU:') }} <strong>{{ $product?->options?->sku }}</strong></p>
                                    @endif
                                    ...
                                    <ul class="el-compare-attrs">
                                        @if(!empty($product?->options['description']))
                                            <li>
                                                ...
                                                <span class="el-compare-attr-val">{!! $product?->options['description'] !!}</span>
                                            </li>
                                        @endif
                                        ...
                                        @if(!empty($product->options["color_name"] ?? ''))
                                            <li>
                                                ...
                                                <span class="el-compare-attr-val">{{ $product->options['color_name'] }}</span>
                                            </li>
                                        @endif
                                        ...
                                        @if(!empty($product->options["size_name"]))
                                            <li>
                                                ...
                                                <span class="el-compare-attr-val">{{ $product->options['size_name'] }}</span>
                                            </li>
                                        @endif

                                        @forelse($product->options["attributes"] ?? [] as $key => $value)
                                            <li>
                                                <span class="el-compare-attr-key">{{ $key }}</span>
                                                <span class="el-compare-attr-val">{{ $value }}</span>
                                            </li>
                                        @empty
                                        @endforelse
                                    </ul>
    ...
    <script>
        $(function () {
            $(document).on('click', '.compare-remove-btn', function () {
                ...
                let $card = $(this).closest('.el-compare-card');
