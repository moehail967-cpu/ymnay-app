<div class="single-shop-details-wrapper mc-pd-options">
    ...
    @if(!empty($product->badge))
        <span class="mc-card-badge" style="position:static;display:inline-block;margin-bottom:12px;">{{ $product?->badge?->name }}</span>
    @endif

    {{-- Title + Wishlist + Compare --}}
    <div class="mc-pd-title-row">
        <h1 class="mc-pd-title">{!! $product->name !!}</h1>
        <div class="mc-pd-actions">
            <a href="javascript:void(0)"
               class="{{ $quickView ? 'quick_view_add_to_wishlist' : 'add_to_wishlist_single_page' }} add-to-wishlist-btn mc-pd-action-btn"
               title="{{ __('Add to Wishlist') }}">
                <i class="lar la-heart"></i>
            </a>
            <a href="javascript:void(0)"
               class="mc-pd-action-btn {{ $quickView ? 'quick-view-' : '' }}compare-btn add-to-compare-btn"
    ...
    @endphp
    <div class="mc-pd-price-row">
        <span class="mc-pd-price flash-prices"
        ...
        @if($deleted_price != null && $deleted_price != $final_price)
            <span class="mc-pd-price-old flash-old-prices">{{ amount_with_currency_symbol($deleted_price) }}</span>
        @endif
    ...
    @if(!empty($product->summary))
        <p class="mc-pd-summary">{{ $product->summary }}</p>
    @endif

    <hr class="mc-pd-divider">

    {{-- Attributes (Size, Color, Custom) --}}
    <div class="mc-pd-variants">
        {{-- Size --}}
        @if($productSizes->count() > 0 && !empty(current(current($productSizes))))
            <div class="value-input-area mc-pd-variant-row {{ $quickView ? 'quick-view-value-input-area' : '' }}">
                <span class="mc-pd-variant-label">
    ...
    {{-- Quantity --}}
    <div class="mc-pd-qty-row">
        <span class="mc-pd-qty-label">{{ __('Quantity:') }}</span>
        <div class="mc-pd-qty-ctrl">
            <button class="{{ $quickView ? 'quick-view-' : '' }}substract substract mc-pd-qty-btn" type="button">
            ...
            <input class="{{ $quickView ? 'quick-view-' : '' }}quantity-input quantity-input qty_"
                   ...
                   name="quantity"
                   value="1" min="1">
            <button class="{{ $quickView ? 'quick-view-' : '' }}plus plus mc-pd-qty-btn" type="button">
        ...
        @endphp
        <span class="stock-available color-stock {{ $text_color }} mc-pd-stock-count"
    ...
    {{-- Add to Cart / Buy Now --}}
    @if($product?->inventory?->stock_count > 0)
        <div class="mc-pd-cta-row">
            <a href="javascript:void(0)"
               class="{{ $quickView ? 'quick_view_add_to_cart' : 'add_to_cart_single_page' }} mc-btn mc-btn-primary mc-btn-block cart-loading">
                <i class="las la-shopping-bag"></i> {{ __('Add to Cart') }}
            </a>
            <a href="javascript:void(0)"
               class="{{ $quickView ? 'quick_view_but_now' : 'but_now_single_page' }} mc-btn mc-btn-outline mc-btn-block cart-loading">
                {{ __('Buy Now') }}
            </a>
        </div>
    @endif
    ...
    {{-- Category / SKU meta --}}
    <div class="mc-pd-meta-list shop-details-stock pt-4">
    ...
    {{-- Social Share --}}
    <div class="wishlist-share social_share_parent mc-pd-share">
        @php
    ...
    @if($custom_specifications->isNotEmpty())
        <div class="mc-pd-specs">
            <h6 class="mc-pd-specs-title">{{ __('Product Details:') }}</h6>
            <table class="mc-dash-table specification-table">
                <thead>
                    <tr>
