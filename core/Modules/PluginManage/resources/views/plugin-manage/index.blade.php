...
@php
    $totalPlugins    = count($pluginList);
    $activePlugins   = collect($pluginList)->where('status', true)->count();
    $inactivePlugins = $totalPlugins - $activePlugins;
    $corePlugins     = collect($pluginList)->filter(fn($p) => !$p->external)->count();
    $extPlugins      = $totalPlugins - $corePlugins;

    $countAll      = $totalPlugins;
    $countLandlord = collect($pluginList)->where('scope', 'landlord')->count();
    $countTenant   = collect($pluginList)->where('scope', 'tenant')->count();
    $countBoth     = collect($pluginList)->where('scope', 'both')->count();
@endphp
...
<div class="plugin-filter-tabs mb-5">
    ...
    <button class="pf-tab" data-filter="tenant">
        <i class="mdi mdi-account-group text-sm"></i> {{__('Tenant')}} <span class="pf-count">{{$countTenant}}</span>
    </button>
...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5" id="pluginGrid">
    ...
    <div class="plugin-card plugin-card-item {{ $isTenantOnly ? 'is-tenant-managed' : '' }}"
        ...
        <div class="plugin-footer">
            @if($isTenantOnly)
                {{-- A: Tenant-only — landlord cannot manage, show read-only indicator --}}
                <span class="plugin-tenant-label">
                    <i class="mdi mdi-lock-outline"></i>
                    {{__('Managed by each tenant individually')}}
                </span>
            @elseif($plugin->external)
                <button type="button"
