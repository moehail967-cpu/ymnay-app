...
<div class="bg-surface rounded-xl border border-main p-5">
    ...
    @switch($type)
        ...
        @case('repeater')
            ...
            @endphp
            <div class="repeater-wrapper space-y-2" data-field="{{ $key }}" data-subfields="{{ json_encode($subFields) }}">
                @foreach((array)$rows as $rowIdx => $rowData)
                    <div class="repeater-row flex items-start gap-2 p-3 bg-secondary rounded-lg border border-main">
                        @foreach($subFields as $sf)
                            @php $sfType = $sf['type'] ?? 'text'; $sfVal = $rowData[$sf['key']] ?? ''; @endphp
                            <div class="flex-1 min-w-0">
                                <label class="block text-xs text-muted mb-1">{{ $sf['label'] ?? $sf['key'] }}</label>
                                @if($sfType === 'select')
                                    <select name="{{ $key }}[{{ $rowIdx }}][{{ $sf['key'] }}]"
                                            class="w-full px-2 py-1.5 text-sm border border-main rounded-lg bg-surface focus:outline-none focus:border-primary">
                                        @foreach($sf['options'] ?? [] as $optV => $optL)
                                            <option value="{{ $optV }}" {{ $sfVal == $optV ? 'selected' : '' }}>{{ $optL }}</option>
                                        @endforeach
                                    </select>
                                @elseif($sfType === 'number')
                                    <input type="number" name="{{ $key }}[{{ $rowIdx }}][{{ $sf['key'] }}]" value="{{ $sfVal }}"
                                           class="w-full px-2 py-1.5 text-sm border border-main rounded-lg bg-surface focus:outline-none focus:border-primary">
                                @else
                                    <input type="text" name="{{ $key }}[{{ $rowIdx }}][{{ $sf['key'] }}]" value="{{ $sfVal }}"
                                           class="w-full px-2 py-1.5 text-sm border border-main rounded-lg bg-surface focus:outline-none focus:border-primary">
                                @endif
                            </div>
                        @endforeach
