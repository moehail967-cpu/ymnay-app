<script>
    (function($){
        ...
        $(document).ready(function(){
            ...
            // Change country and get state (edit modal)
                    ...
                    success: function(res) {
                        if (res.status === 'success') {
                            ...
                            $.each(all_state, function(index, value) {
                                ...
                                all_options += "<option "+ isSelected +" value='" + value.id + "'>" + value.name + "</option>";
                            });

                            $(".get_country_state").html(all_options);
                            $(".info_msg").html('');
                            if(all_state.length <= 0){
                                $(".info_msg").html('<small class="text-xs text-danger"> {{ __("No state found for selected country!") }} </small>');
                            }
                        }
                    }
                })
            ...
            // Change country and get state (add modal)
                    ...
                    success:function(res){
                        if(res.status === 'nothing'){
                            $('.search_result').html(
                                ...
                                '<i class="mdi mdi-magnify-close text-3xl text-muted"></i>' +
                                '<p class="text-sm text-muted">{{ __("No Data Found") }}</p></div>'
                            );
                        }else{
                            $('.search_result').html(res);
                        }
