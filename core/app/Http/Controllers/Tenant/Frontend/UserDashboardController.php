...
class UserDashboardController extends Controller
{
    ...
    public function user_index(){
        ...
        $total_order = $package_orders = $order_count = ProductOrder::where('user_id', $user_id)->count();
        $total_amount = $order_purchase = ProductOrder::where('user_id', $user_id)->sum('total_amount');
        $product_refunds = RefundProduct::where(['user_id' => $user_id, 'status' => 1])->count();
        $support_tickets = $support_ticket = $total_ticket = $total_support = $ticket_count = SupportTicket::where('user_id', $user_id)->count();
        $total_wishlist = $wishlist_count = ProductWishlist::where('user_id', $user_id)->count();
        $total_download = $download_count = DigitalProductDownload::where('user_id', $user_id)->count();
        $recent_logs = $recent_orders = ProductOrder::where('user_id', $user_id)->orderBy('id', 'desc')->take(5)->get();

        return themeView('user.dashboard.user-home')->with(compact(
            'package_orders', 'order_purchase', 'product_refunds', 'support_tickets',
            'recent_logs', 'total_order', 'total_amount', 'order_count',
            'total_wishlist', 'wishlist_count', 'total_download', 'download_count',
            'total_ticket', 'total_support', 'support_ticket', 'ticket_count',
            'recent_orders'
        ));
    }
...
//        dd($request->all());
    ...
    public function manage_account()
    {
        ...
        return themeView('user.dashboard.manage-account', compact([
            'user_details',
            'countries',
            'states',
            'cities'
    ...
    public function edit_profile()
    {
        ...
        return themeView('user.dashboard.manage-account', compact('user_details'));
    }
    ...
    {
        return themeView('user.dashboard.change-password');
    }
    ...
    public function support_tickets(){
        ...
        return themeView('user.dashboard.support-tickets')->with([ 'all_tickets' => $all_tickets, 'departments' => $departments]);
    }
    ...
    public function support_ticket_view(Request $request,$id){
        ...
        $q = $request->q ?? '';
        return themeView('user.dashboard.view-ticket')->with(['ticket_details' => $ticket_details,'all_messages' => $all_messages,'q' => $q]);
    }
    ...
    public function package_orders(){
        ...
        return themeView('user.dashboard.package-order', compact("order_list"));
    }
    ...
    public function order_list($id = null){
        if (!empty($id)){
            ...
            return themeView('user.dashboard.order-details', compact("order"));
        }
        ...
        $orders_paginator = $order_list;

        return themeView('user.dashboard.order-list', compact("order_list", "orders", "all_orders", "orders_paginator"));
    }
    ...
    private function undostock($order)
    {
        ...
        foreach ($ordered_products ?? [] as $product) {
            DB::transaction(function () use ($product) {
                ...
                $product_inventory = ProductInventory::where('product_id', $product->product_id ?? 0)->lockForUpdate()->first();
                if ($product_inventory) {
                    $product_inventory->increment('stock_count', $product->quantity ?? 0);
                    $product_inventory->sold_count = max(0, ($product_inventory->sold_count ?? 0) - ($product->quantity ?? 0));
                    $product_inventory->save();
                }

                // Reverse campaign sold_count when user cancels an order
                $cp = \Modules\Campaign\Entities\CampaignProduct::where('product_id', $product->product_id)->lockForUpdate()->first();
                if ($cp && $cp->sold_count >= $product->quantity) {
                    $cp->decrement('sold_count', $product->quantity);
                }
            });
        }
    }
    ...
    public function download_list($id = null){
        ...
        return themeView('user.dashboard.download-list', compact("download_list"));
    }
    ...
    /*
     * new commission based code
     */
    private function calculateAvailableBalance($tenant_id)
    {
        // Total earnings
        $total_earnings = DB::table('commission_histories')
            ->where('user_id', $tenant_id)
            ->sum('order_total');

        // Total commission
        $total_commission = DB::table('commission_histories')
            ->where('user_id', $tenant_id)
            ->sum('commission_amount');

        // Already withdrawn
        $total_withdrawn = TenantWithdrawRequest::where('tenant_id', $tenant_id)
            ->where('status', 'approved')
            ->sum('amount');

        return $total_earnings - $total_commission - $total_withdrawn;
    }
    ...
    public function submitWithdrawRequest(Request $request)
    {
        ...
        // Get selected gateway and its configuration
        $gatewayId = $request->input('selected_payment_gateway');
        $gateway = WithdrawGateway::where('gateway_id', $gatewayId)
            ->where('status', true)
            ->first();

        if (!$gateway) {
            return back()->with(['msg' => __('Invalid payment gateway selected'), 'type' => 'danger'])->withInput();
        }

        // Validate request
        $validated = $request->validate($rules, [
            'selected_payment_gateway.required' => 'Please select a payment gateway',
            'amount.required' => 'Please enter withdraw amount',
        ...
        $available_balance = Wallet::where('user_id', $user_id)
            ->value('balance') ?? 0;

        // Validate sufficient balance
        if ($request->amount > $available_balance) {
            return back()->with([
                'msg' => __('Insufficient balance. Available: ') . float_amount_with_currency_symbol($available_balance),
                'type' => 'danger'
            ])->withInput();
        }

        // Minimum withdraw amount check
        $minimumWithdraw = 10;
        if ($request->amount < $minimumWithdraw) {
            return back()->with([
                'msg' => __('Minimum withdraw amount is ') . float_amount_with_currency_symbol($minimumWithdraw),
                'type' => 'danger'
            ])->withInput();
        }

        try {
            // Collect account details from dynamic fields
            $accountDetails = [
                'gateway_name' => $gateway->name,
                'gateway_id' => $gateway->gateway_id,
            ];

            if ($gateway->fields && is_array($gateway->fields)) {
                foreach ($gateway->fields as $field) {
                    $fieldName = $this->sanitizeFieldName($field['label']);
                    $accountDetails[$field['label']] = $request->input($fieldName);
                }
            }

            // Create withdraw request
            TenantWithdrawRequest::create([
                'tenant_id' => $user_id,
            ...
            return redirect()->back()
                ->with(['msg' => __('Withdraw request submitted successfully. Please wait for admin approval.'), 'type' => 'success']);

        } catch (\Exception $e) {
            return back()->with([
                'msg' => __('Failed to submit withdraw request. Please try again.'),
                'type' => 'danger'
            ])->withInput();
        }
    }
    ...
    /**
     * Sanitize field label to use as input name
     */
    private function sanitizeFieldName($label)
    {
        return 'field_' . preg_replace('/[^a-zA-Z0-9_]/', '_', strtolower($label));
    }
