...
{
    protected function getWidgetType(): string { return 'hexfashion_featured_products'; }
    protected function getWidgetName(): string { return 'HexFashion: Featured Products'; }
    protected function getWidgetIcon(): string|array { return 'las la-star'; }
    protected function getWidgetDescription(): string { return __('4-column product slider with dot pagination (Electro style)'); }
    protected function getCategory(): string { return WidgetCategory::THEME; }
    protected function getWidgetTags(): array { return ['featured', 'products', 'hexfashion']; }
    ...
    public function render(array $settings = []): string
    {
        if (!function_exists('tenant') || !tenant()) {
            return '<div style="padding:40px;text-align:center;background:#f9f9f9;border:1px dashed #ccc;border-radius:8px;color:#888;font-family:sans-serif;"><p style="margin:0;font-size:14px;">HexFashion: Featured Products — preview on the live page</p></div>';
        }

        $content   = $settings['general']['content'] ?? [];
        $spacing   = $settings['style']['spacing']   ?? [];
        $count     = (int) ($content['product_count'] ?? $content['count'] ?? 16);
        ...
        return view('theme-hexfashion::widgets.featured_products', [
