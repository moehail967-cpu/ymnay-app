...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (menuToggle && sidebar && overlay && menuIcon) {
    const isOpen = () => !sidebar.classList.contains('translate-x-full');

    const openSidebar = () => {
      sidebar.classList.remove('translate-x-full');
      overlay.classList.remove('hidden');
      menuIcon.classList.replace('tabler-menu-2', 'tabler-x');
    ...
    const closeSidebar = () => {
      ...
      menuIcon.classList.replace('tabler-x', 'tabler-menu-2');
  ...
  // Mobile dropdown toggles (dynamic - works with any number of menu items)
...
$(document).on('click', '.owl-item > div', function () {
  $owl.trigger('to.owl.carousel', [$(this).data('position'), 400]);
});
...
window.addEventListener('load', () => {
  // Collect .slide-box elements after the page has fully loaded so the DOM is ready
  const boxes = gsap.utils.toArray('.slide-box');
  if (!boxes || boxes.length === 0) {
    console.warn('No .slide-box elements found — skipping marquee initialization.');
...
function horizontalLoop(items, config) {
  ...
  // If no items, bail out early to avoid accessing items[0].offsetLeft (uncaught TypeError)
