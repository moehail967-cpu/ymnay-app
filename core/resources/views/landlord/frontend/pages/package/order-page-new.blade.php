...
@section('style')
    ...
    <style>
        ...
        .op-pstep.pstep-active .op-pstep-ico    { border-color: #0C4D54; background-color: #f0fafa; }
        .op-pstep.pstep-active .op-pstep-pending { display: none !important; }
        .op-pstep.pstep-active .op-pstep-spin   { display: block !important; }
        .op-pstep.pstep-active .op-pstep-label  { color: #0C4D54; font-weight: 600; }

        .op-pstep.pstep-done .op-pstep-ico      { border-color: #0C4D54; background-color: #0C4D54; }
        ...
        .op-pstep.pstep-done .op-pstep-label    { color: #374151; }
        ...
        .mode-toggle-btn.active {
            background: var(--sectionC, #0C4D54);
            color: #fff;
        ...
        <div class="flex flex-col items-center text-center px-6 w-full max-w-sm">
            ...
            <div class="w-full mb-6">
                <div class="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span id="opProgressEta" class="text-gray-400 italic">{{__('Estimated: 10–20 seconds')}}</span>
                </div>
            ...
            <div class="w-full space-y-2.5 mb-6 text-left">
                ...
                <div class="op-pstep flex items-center gap-3" data-pstep="{{ $n }}">
                    <span class="op-pstep-ico w-7 h-7 rounded-full border-2 border-gray-200 flex items-center justify-center shrink-0 transition-all duration-300">
                        <i class="ti tabler-minus text-gray-300 text-[11px] op-pstep-pending"></i>
                    ...
                    <span class="text-sm text-gray-400 op-pstep-label transition-colors duration-300">{{ $label }}</span>
                </div>
                @endforeach
            </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="mb-6">
                        ...
                        @if($order_details->description)
                            <p class="text-quate">{{$order_details->description}}</p>
                        @endif
                    </div>
                    ...
                    @if($show_trial_option)
                        <div class="mb-6 bg-[#F8FAFB] border border-borderCS rounded-2xl p-2 flex gap-1">
                    ...
                    @elseif($order_details->has_trial == 1 && $user_has_trial)
                        ...
                        <div class="mb-4 bg-[#F8FAFB] border border-borderCS rounded-2xl p-2 flex gap-1">
                            <button type="button" class="mode-toggle-btn active" data-mode="purchase">
                                <i class="icon-base ti tabler-shopping-cart mr-1"></i>
                                {{__('Purchase')}}
                            </button>
                    ...
                    <form id="order-form" action="{{ route('landlord.frontend.order.payment.form') }}"
                        ...
                        <div class="bg-[#F8FAFB] rounded-2xl shadow-sm border border-borderCS p-8 md:p-10 mb-8">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" id="field-name" name="name"
                                    ...
                                    class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none {{$is_auth ? 'bg-gray-50' : ''}}"
                                    placeholder="{{__('Name')}}"
                                    {{$is_auth ? 'readonly' : ''}}>
                            </div>
                            ...
                            <div id="subdomain-select-wrap" class="{{$is_auth ? '' : 'hidden'}} mb-4">
                                ...
                                <select class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none bg-white subdomain"
                            ...
                            <div class="custom_subdomain_wrapper mb-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
                                    <div class="col-span-1 md:col-span-5">
                                        <input type="text"
                                               class="w-full py-4 px-4 border border-gray-300 rounded-lg focus:outline-none custom_subdomain"
                                               id="custom-subdomain" name="custom_subdomain"
                                               placeholder="{{__('Subdomain')}}" autocomplete="off" value="">
                                    </div>
                                    <div class="col-span-1 md:col-span-2">
                                        <div class="w-full py-4 px-4 border border-[#4D8700] rounded-lg bg-[#92E721]/10 text-gray-700 flex items-center justify-center text-sm text-center">
                                            .{{$base_url}}
                                        </div>
                                    </div>
                                </div>
                                <div id="subdomain-wrap" class="mt-1"></div>
                                <span class="text-sm text-gray-500 mt-1 block">{{__('Your website will be available at subdomain.')}}{{$base_url}}</span>
                            </div>
                        </div>
                        ...
                        <div class="theme-section mb-8">
                            ...
                            <div class="bg-[#f8fafb] p-8 rounded-xl border border-borderCS">
...
//                                        $theme_list = $order_details?->plan_themes?->pluck('theme_slug')->toArray() ?? [];
                                    ...
                                    @foreach(getPricePlanBasedAllThemeData($theme_list) as $theme)
                                        ...
                                        @endphp
                                        <div class="theme-card bg-white rounded-xl overflow-hidden border-2 border-transparent duration-300 hover:border-sectionC {{$is_selected ? 'selected_theme' : ''}}"
                            ...
                            <div class="mb-8" id="payment-section">
                                ...
                                <div class="mt-4 d-none manual_transaction_id">
                                    ...
                                    <input type="text" name="trasaction_id"
                                        class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none mb-3"
                                        placeholder="{{__('Transaction ID')}}">
                                    <input type="file" name="trasaction_attachment"
                                        class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none"
                        ...
                        {{-- ── COUPON + CTA BANNER ── --}}
                        <div class="bg-[#F8FAFB] rounded-2xl border border-borderCS p-8 mb-8">
...
{{--                                </button>--}}
                    ...
                    <div class="bg-[#F8FAFB] border border-borderCS rounded-2xl p-8">
                        ...
                        <div class="mb-8">
                            <div class="flex items-baseline gap-1">
                                ...
                                <span class="text-quate text-sm">{{$order_details->period}}</span>
                            </div>
                        </div>
                            ...
                            <div class="space-y-3">
                                @if(!empty($order_details->page_permission_feature))
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-gray-700">{{__(sprintf('Page %s', $page_permission_feature))}}</span>
                                    </div>
                                @endif
                                ...
                                @if(!empty($order_details->blog_permission_feature))
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-gray-700">{{__(sprintf('Blog %s', $blog_permission_feature))}}</span>
                                    </div>
                                @endif
                                ...
                                @if(!empty($order_details->product_permission_feature))
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-gray-700">{{ __(sprintf('Product %s', $product_permission_feature))}}</span>
                                    </div>
                                @endif
                                ...
                                @if(!empty($order_details->storage_permission_feature))
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-gray-700">{{__(sprintf('Storage %s %s', current($storage_permission_feature), last($storage_permission_feature)))}}</span>
                                    </div>
                                @endif
                                ...
                                @foreach($order_details->plan_features as $key => $item)
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-gray-700">{{__(str_replace('_', ' ', ucfirst($item->feature_name))) ?? ''}}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="space-y-3 mb-6">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-gray-900 font-medium">{{amount_with_currency_symbol($order_details->price)}}</span>
                            </div>
                            <div class="flex justify-between items-center coupon-discount-row" style="display:none!important;">
                                <span class="text-gray-700">{{__('Discount')}}</span>
                        ...
                        {{-- Guarantee --}}
                        <div class="bg-[#92E721]/10 border-2 border-[#4D8700] rounded-xl p-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <h4 class="text-gray-900 font-medium mb-1">{{__('Secure Checkout')}}</h4>
                                    <p class="text-quate text-sm">{{__('Your payment information is safe and secure.')}}</p>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal-overlay" id="loginModal">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-y-auto" style="max-height:92vh;">
            ...
            <div class="flex items-center justify-between px-8 py-5 border-b border-gray-100">
                ...
                <button type="button" class="modal-close-btn w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors" data-modal="loginModal">
                    <i class="icon-base ti tabler-x text-gray-500"></i>
                </button>
            </div>
            ...
            <div id="modal-login-panel" class="px-8 py-8">
                ...
                <p class="text-quate text-sm mb-6">{{__('Sign in to continue.')}}</p>
            ...
            <div id="modal-register-panel" class="px-8 py-8" style="display:none;">
                ...
                <p class="text-quate text-sm mb-6">{{__('Fill in your details to continue.')}}</p>
                <div id="register-modal-msg"></div>
                <div class="mb-4">
                    <label class="block text-secondary font-medium mb-2">{{__('Full Name')}}</label>
                    <input type="text" id="reg_name" class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none" placeholder="{{__('Your Name')}}">
                </div>
                <div class="mb-4">
                    <label class="block text-secondary font-medium mb-2">{{__('Email')}}</label>
                    <input type="email" id="reg_email" class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none" placeholder="{{__('Your Email')}}">
                </div>
                <div class="mb-4">
                    <label class="block text-secondary font-medium mb-2">{{__('Phone')}}</label>
                    <input type="text" id="reg_phone" class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none" placeholder="{{__('Your Phone Number')}}">
                </div>
                <div class="mb-4">
                    <label class="block text-secondary font-medium mb-2">{{__('Username')}}</label>
                    <input type="text" id="reg_username" class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none" placeholder="{{__('Choose a Username')}}">
                </div>
                <div class="mb-4">
                    <label class="block text-secondary font-medium mb-2">{{__('Password')}}</label>
                    <input type="password" id="reg_password" class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none" placeholder="{{__('Minimum 8 characters')}}">
                </div>
                <div class="mb-6">
                    <label class="block text-secondary font-medium mb-2">{{__('Confirm Password')}}</label>
                    <input type="password" id="reg_password_confirmation" class="w-full px-4 py-4 border border-gray-300 rounded-lg focus:outline-none" placeholder="{{__('Confirm Password')}}">
                </div>
            ...
            <div id="modal-otp-panel" class="px-8 py-8" style="display:none;">
                <div class="text-center mb-6">
                    <div class="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3" style="background:rgba(var(--primary-rgb,12,77,84),0.1)">
                        <i class="ti tabler-shield-check text-3xl" style="color:var(--primary-color,#0C4D54)"></i>
                    </div>
                ...
                <div class="flex gap-2 justify-center mb-5" id="otp-input-group">
                    ...
                    <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]"
                           class="otp-digit w-11 h-13 text-center text-xl font-bold border-2 border-gray-300 rounded-lg focus:outline-none focus:border-primary transition-colors"
                ...
                <button type="button" id="verify_otp_btn"
                        ...
                        style="background:var(--primary-color,#0C4D54)">
                    {{__('Verify OTP')}}
                </button>
                ...
                <div class="flex items-center justify-between mt-4 text-sm">
                    <button type="button" id="resend_otp_btn" disabled
                            ...
                            style="color:var(--primary-color,#0C4D54)">
                        {{__('Resend')}} (<span id="resend-countdown">60</span>s)
                    </button>
                    <button type="button" id="back_to_register_btn" class="text-gray-500 hover:underline">
                        &#8592; {{__('Back')}}
                    </button>
                </div>
            </div>
