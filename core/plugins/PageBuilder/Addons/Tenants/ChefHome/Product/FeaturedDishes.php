...
class FeaturedDishes extends PageBuilderBase
{
    ...
    public function admin_render(): string
    {
        ...
        $output .= Number::get(['name' => 'item_count', 'label' => __('Dishes to Show'), 'value' => $v['item_count'] ?? 8]);
        $output .= Select::get([
            'name'    => 'sort_by',
            'label'   => __('Sort By'),
            'options' => [
        ...
        $output .= Text::get(['name' => 'view_all_url', 'label' => __('View All URL (leave empty to use shop page)'), 'value' => $v['view_all_url'] ?? null]);
    ...
    public function frontend_render(): string
    {
        ...
        $data = [
            'title'          => SanitizeInput::esc_html($this->setting_item('title') ?? __('Featured Dishes')),
            'products'       => $products,
        ...
        return self::renderView('tenant.chefhome.product.featured-dishes', $data);
    }

    public function addon_title(): string
    {
        return __('ChefHome: Featured Dishes');
    }
