...
When a repository-aware task explicitly addresses a registered handle (for example `@Adam`, `@Nour`, `@Omar`, or `@Salem`) or clearly asks to work with that named agent, load that agent's definition and follow its role for the current task. The explicit current user instruction still has highest project-level authority.
