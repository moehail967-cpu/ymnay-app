...
class FrontendFormController extends Controller
{
    public function send_contact_message(Request $request)
    {
        ...
        try{
            Mail::to(get_static_option_central('site_global_email'))->send(new ContactMessage($all_field_serialize_data, $all_attachment,
    ...
    public function custom_form_builder_message(Request $request)
    {
        if (!empty(config('services.recaptcha.secret_key'))) {
            ...
            if (!$recaptcha->verify($request->input('g-recaptcha-response'))) {
                return response()->json(['msg' => __('reCAPTCHA verification failed. Please try again.'), 'type' => 'danger']);
            }
        }
    ...
    public function direct_contact_message(Request $request)
    {
        if (!empty(config('services.recaptcha.secret_key'))) {
            $recaptcha = new RecaptchaService();
            if (!$recaptcha->verify($request->input('g-recaptcha-response'))) {
        ...
        $to = filter_var($request->input('recipient_email'), FILTER_VALIDATE_EMAIL)
        ...
        $subject = __('New Contact Message from') . ' ' . get_static_option('site_' . get_default_language() . '_title');

        try {
            Mail::to($to)->send(new ContactMessage($fields, [], $subject));
        } catch (\Exception $e) {
            Log::error('ContactTwo mail failed: ' . $e->getMessage());
            return response()->json(['msg' => __('Sorry! The message could not be sent. Try again later.'), 'type' => 'danger']);
        }

        return response()->json(['msg' => __('Thanks for your message! We will get back to you soon.'), 'type' => 'success']);
    }
