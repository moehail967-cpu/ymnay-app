...
@section('title') {{__('All Pending Orders')}} @endsection
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-warning-soft flex items-center justify-center flex-shrink-0">
                <i class="mdi mdi-clock-outline text-warning text-base"></i>
            </div>
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Pending Orders')}}</h3>
                <p class="text-xs text-muted">{{__('Orders awaiting processing')}}</p>
            </div>
        </div>
        ...
        <table class="w-full text-left" id="all_user_table">
            ...
            @foreach($all_orders as $data)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5">
                        <span class="tw-pill tw-pill-warning">{{__(ucfirst(str_replace('_', ' ', $data->status)))}}</span>
                    </td>
