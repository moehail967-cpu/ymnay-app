<div class="mc-checkout-summary">
    <h2 class="mc-dash-card-title"> {{__('Order Summary')}} </h2>
    <form action="#" class="mc-coupon-row coupon-contents-form mt-4">
        ...
        <input type="hidden" class="coupon-shipping-method" name="coupon_shipping_method"
               value="">
        <input class="mc-form-input coupon-code" type="text" placeholder="{{__('Coupon Code')}}"
               name="used_coupon">
        <button type="button" class="mc-btn mc-btn-outline coupon-btn"> {{__('Apply')}}</button>
    </form>

    <div class="mc-checkout-items coupon-border mt-4 mb-4">
        @php
            $subtotal = null;
            $itemsTotal = null;
            $v_tax_total = 0;
        ...
        @foreach($carts ?? [] as $data)
            ...
            <div class="mc-checkout-item">
                <div class="mc-checkout-item-img">
                    <a href="javascript:void(0)">
                        {!! render_image_markup_by_attachment_id($data?->options?->image) !!}
                    </a>
                </div>
                <div class="mc-checkout-item-name">
                    <a href="javascript:void(0)"> {{$data->name}} </a>
                    <span class="mc-cart-meta d-block mt-1">
                    ...
                    <span class="mc-cart-meta"> {{__('Quantity:')}} {{$data->qty}} </span>
                </div>
                <span class="mc-checkout-item-price"> {{amount_with_currency_symbol($price * $data->qty)}} </span>
            </div>
    ...
    <div class="coupon-contents-details">
        ...
        <div class="shipping_method_wrapper">
            ...
            @if($user != null)
                <div class="mc-summary-row d-block">
                    ...
                    @foreach($shipping_methods ?? [] as $key => $method)
                        ...
                        <div class="mc-summary-row" data-country="{{isset($country) ? $country : 0}}"
    ...
    @endphp
    @if($COD === 'on')
        <div class="mc-check-row cash-on-delivery mt-2">
    ...
    <div class="mc-payment-section mt-4">
        ...
        <div class="payment-card mt-3">
            ...
            <div class="d-none w-100 mt-3 manual_transaction_id">
                ...
                <input type="text" name="trasaction_id" class="mc-form-input"
    ...
    <div class="mc-check-row mt-3">
        <input class="check-input" type="checkbox" id="agree" checked>
        <label for="agree"> {{__('I have read and agree to the website')}} <a
                class="mc-terms-link terms-condition" href="javascript:void(0)"> {{__('terms and conditions*')}} </a> </label>
    </div>

    <div class="mt-3">
        <a href="javascript:void(0)"
           class="mc-btn mc-btn-primary mc-btn-block checkout_disable proceed_checkout_btn"> {{__('Proceed to Checkout')}} </a>
    </div>

    <div class="mt-3">
        <a href="{{theme_cart_url()}}" class="mc-btn mc-btn-outline mc-btn-block"> {{__('Return to Cart')}} </a>
    </div>
</div>
