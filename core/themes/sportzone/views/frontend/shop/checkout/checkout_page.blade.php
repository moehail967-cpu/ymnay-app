...
<div class="container" style="padding-top:28px;">
    ...
    <div style="display:flex;align-items:center;gap:0;margin-bottom:32px;">
        <div style="display:flex;align-items:center;gap:8px;color:var(--sz-muted);padding:10px 20px;font-family:var(--sz-font-head);font-size:12px;letter-spacing:1.5px;text-transform:uppercase;">
            <span style="width:22px;height:22px;border-radius:50%;background:var(--sz-red);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:12px;"><i class="mdi mdi-check"></i></span>
            {{ __('Cart') }}
        </div>
        <div style="flex:1;height:3px;background:var(--sz-red);"></div>
        <div style="display:flex;align-items:center;gap:8px;background:var(--sz-red);color:#fff;padding:10px 20px;font-family:var(--sz-font-head);font-size:12px;font-weight:400;letter-spacing:1.5px;text-transform:uppercase;">
            <span style="width:22px;height:22px;border-radius:50%;background:#fff;color:var(--sz-red);display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:900;">2</span>
            {{ __('Checkout') }}
        </div>
    ...
    <div class="row g-4" style="padding-bottom:72px;">
        ...
        <div class="col-lg-7">
            ...
            <div style="background:var(--sz-white);border:2px solid var(--sz-border);border-radius:var(--sz-radius-xl);overflow:hidden;margin-bottom:20px;">
                <div style="background:var(--sz-navy);padding:14px 20px;font-family:var(--sz-font-head);font-size:13px;color:#fff;text-transform:uppercase;letter-spacing:2px;">
                ...
                <div style="padding:24px;">
                    ...
                    <form id="login_form_order_page">
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            ...
                            <a href="{{ theme_forget_password_url() }}" style="font-size:13px;color:var(--sz-red);">{{ __('Forgot Password?') }}</a>
                        </div>
                        <button type="submit" id="login_btn" class="sz-btn sz-btn-red">{{ __('Sign In') }}</button>
                        <p class="mt-3" style="font-size:13px;color:var(--sz-muted);">
                            {{ __("Don't have an account?") }}
                            <a href="{{ theme_register_url() }}" style="color:var(--sz-red);font-weight:600;">{{ __('Sign up') }}</a>
                        </p>
                    </form>
                </div>
            </div>
            ...
            <form action="{{ theme_checkout_url() }}" method="POST" class="checkout-form" enctype="multipart/form-data">
                    ...
                    <div style="padding:24px;">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6 position-relative">
                                ...
                                <input type="text" name="state" class="stateField live-state-input"
                                       style="{{ $inputStyle }}"
                            ...
                            <div class="col-md-6 position-relative">
                                <label style="{{ $labelStyle }}">{{ __('City / Town') }}</label>
                                <input type="text" name="city" class="live-city-input"
                                       style="{{ $inputStyle }}"
                            ...
                            <div class="col-12">
                                <label style="{{ $labelStyle }}">{{ __('Order Notes') }} <span style="color:var(--sz-muted);font-weight:300;">({{ __('optional') }})</span></label>
                                <textarea name="message" style="{{ $inputStyle }}height:70px;resize:vertical;" rows="2"
                                          placeholder="{{ __('e.g. Leave at door, call before delivery…') }}"
                                          onfocus="this.style.borderColor='var(--sz-red)'" onblur="this.style.borderColor='var(--sz-border)'">{{ old('message') }}</textarea>
                            </div>
                        </div>

                        @if(!theme_is_logged_in())
                        <div class="mt-4">
                            <a href="javascript:void(0)" class="create-accounts" style="color:var(--sz-red);font-size:13px;font-family:var(--sz-font-head);text-transform:uppercase;letter-spacing:1px;">
                ...
                <div style="background:var(--sz-white);border:2px solid var(--sz-border);border-radius:var(--sz-radius-xl);overflow:hidden;">
                    ...
                    <div style="padding:24px;">
                        <div class="mb-3">
                            <label class="d-flex align-items-center gap-2" style="font-size:14px;cursor:pointer;font-family:var(--sz-font-head);text-transform:uppercase;letter-spacing:1px;">
                                <input type="checkbox" id="cash" style="accent-color:var(--sz-red);width:16px;height:16px;">
                                <i class="mdi mdi-cash" style="font-size:20px;color:var(--sz-red);"></i>
                                {{ __('Cash On Delivery') }}
                            </label>
                        </div>
                        ...
                        <div class="summernot_wrap" style="display:none;">
                            <div class="manual_description mt-2" style="font-size:13px;color:var(--sz-muted);"></div>
                        </div>
        ...
        <div class="col-lg-5">
            <div style="background:var(--sz-white);border:2px solid var(--sz-border);border-radius:var(--sz-radius-xl);overflow:hidden;position:sticky;top:100px;">
                <div style="background:var(--sz-navy);padding:14px 20px;font-family:var(--sz-font-head);font-size:13px;color:#fff;text-transform:uppercase;letter-spacing:2px;">
                    {{ __('Your Order') }}
                </div>
                <div style="padding:24px;">
                    ...
                    <div class="mb-3">
                        <div class="d-flex gap-2">
                            ...
                            <input type="text" class="coupon-code flex-grow-1" style="padding:10px 14px;border:2px solid var(--sz-border);border-radius:var(--sz-radius);font-size:13px;font-family:var(--sz-font-body);outline:none;transition:border-color .2s;" placeholder="{{ __('Coupon Code') }}" name="used_coupon_display"
                                   onfocus="this.style.borderColor='var(--sz-red)'" onblur="this.style.borderColor='var(--sz-border)'">
                            <button type="button" class="sz-btn sz-btn-red sz-btn-sm coupon-btn" style="white-space:nowrap;">{{ __('Apply') }}</button>
                        </div>
                    </div>

                    @foreach(theme_cart_items() as $data)
                    <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--sz-border);">
                        <div style="width:52px;height:52px;border-radius:var(--sz-radius);overflow:hidden;border:2px solid var(--sz-border);flex-shrink:0;">
                            {!! render_image_markup_by_attachment_id($data?->options?->image) !!}
                        </div>
                        <div style="flex:1;min-width:0;">
                            <div style="font-family:var(--sz-font-head);font-size:12px;font-weight:400;color:var(--sz-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-transform:uppercase;letter-spacing:.5px;">{{ $data->name }} ×{{ $data->qty }}</div>
                        ...
                        <span class="sz-price-sale" style="font-size:13px;white-space:nowrap;">{{ amount_with_currency_symbol($data->price * $data->qty) }}</span>
                    </div>
                    @endforeach
                    ...
                    <div class="shipping_method_wrapper">
                        ...
                        @endphp
                        @php $rowStyle = 'display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--sz-border);font-size:13px;'; @endphp
                        <div style="{{ $rowStyle }} margin-top:12px;"><span style="color:var(--sz-muted);font-family:var(--sz-font-head);font-size:11px;text-transform:uppercase;letter-spacing:1px;">{{ __('Subtotal') }}</span><span style="font-weight:600;">{{ site_currency_symbol() }}{{ Cart::subtotal() }}</span></div>
                        <div style="{{ $rowStyle }}" class="coupon-price"><span style="color:var(--sz-muted);font-family:var(--sz-font-head);font-size:11px;text-transform:uppercase;letter-spacing:1px;">{{ __('Coupon Discount (-)') }}</span><span style="font-weight:600;color:var(--sz-red);" class="pf-price">{{ amount_with_currency_symbol(0.00) }}</span></div>
                        <div style="{{ $rowStyle }}"><span style="color:var(--sz-muted);font-family:var(--sz-font-head);font-size:11px;text-transform:uppercase;letter-spacing:1px;">{{ __('Tax (Incl)') }}</span><span style="font-weight:600;">{{ $product_tax }}%</span></div>

                        @if(theme_is_logged_in() && !empty($shipping_methods ?? []))
                        <div style="margin:10px 0 4px;font-family:var(--sz-font-head);font-size:11px;text-transform:uppercase;letter-spacing:1.5px;color:var(--sz-muted);">{{ __('Shipping') }}</div>
                        ...
                        <div style="{{ $rowStyle }}" data-country="{{ $country ?? '' }}" data-state="{{ $state ?? '' }}">
                            <label class="d-flex align-items-center gap-2" style="cursor:pointer;font-size:13px;margin:0;">
                                <input type="radio" class="shipping_methods" id="shipping-option-{{ $method['id'] }}" name="shipping_method_display"
                                       value="{{ $method['id'] }}" style="accent-color:var(--sz-red);" {{ $key === 0 ? 'checked' : '' }}>
                                {{ $method['name'] }}
                            </label>
                        ...
                        <div style="{{ $rowStyle }}" class="price-shipping"><span style="color:var(--sz-muted);font-family:var(--sz-font-head);font-size:11px;text-transform:uppercase;letter-spacing:1px;">{{ __('Shipping Cost (+)') }}</span><span style="font-weight:600;" class="pf-price">{{ isset($default_shipping) ? amount_with_currency_symbol($default_shipping['options']['cost']) : '--' }}</span></div>
                        <div style="display:flex;justify-content:space-between;padding:14px 0;border-top:2px solid var(--sz-border);margin-top:4px;" class="price-total" data-total="{{ $subtotal + $taxed_price }}">
                            <span style="font-family:var(--sz-font-head);font-size:14px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:var(--sz-dark);">{{ __('Total Amount') }}</span>
                            <span class="sz-price-sale pf-price" style="font-size:20px;">{{ site_currency_symbol() }}{{ $total }}</span>
                        </div>
                    </div>

                    {!! apply_filters('nazmart:cart_summary', '') !!}
                    <div class="mt-3 d-flex align-items-center gap-2">
                        <input type="checkbox" id="agree" checked style="accent-color:var(--sz-red);width:15px;height:15px;">
                        <label for="agree" style="font-size:12px;color:var(--sz-muted);cursor:pointer;font-family:var(--sz-font-head);text-transform:uppercase;letter-spacing:.5px;">
                            {{ __('I agree to the') }}
                            <a href="javascript:void(0)" style="color:var(--sz-red);">{{ __('Terms & Conditions') }}</a>
                        </label>
                    </div>

                    <button type="button" class="sz-btn sz-btn-red w-100 mt-3 proceed_checkout_btn checkout_disable" style="justify-content:center;font-size:14px;">
                        <i class="mdi mdi-lock"></i> {{ __('Place Order') }}
                    </button>

                    <a href="{{ theme_cart_url() }}" class="sz-btn sz-btn-outline w-100 mt-2" style="justify-content:center;">
                        {{ __('Return to Cart') }}
                    </a>

                    <div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:14px;font-size:11px;color:var(--sz-muted);font-family:var(--sz-font-head);text-transform:uppercase;letter-spacing:1px;">
