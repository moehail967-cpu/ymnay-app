...
@section('title') {{__('Import States')}} @endsection
            ...
            <div class="px-5 sm:px-6 py-4 border-b border-main flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-info-soft flex items-center justify-center flex-shrink-0">
                        <i class="mdi mdi-database-import text-info text-base"></i>
                    </div>
                    <div>
                        <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Import States')}}</h3>
                        <p class="text-xs text-muted">{{__('Upload a CSV file to bulk-import states')}}</p>
                    </div>
                </div>
                <a href="{{route('tenant.admin.settings.csv.download.sample', 'state')}}"
                   download="sample-state-data.csv"
            ...
            <div class="p-5 sm:p-6">
                ...
                @else
                    @php
                        ...
                        $all_countries = \Modules\CountryManage\Entities\Country::all_countries();
                    @endphp
                    ...
                    <form action="{{route('tenant.admin.state.import.database')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="space-y-4 mb-6">
                            ...
                            <div class="bg-secondary rounded-xl border border-main p-4">
                                ...
                                <p class="text-[11px] text-muted mt-2 ml-10">{{__('Select the country these states belong to.')}}</p>
                            </div>
                            ...
                            <div class="bg-secondary rounded-xl border border-main p-4">
                                <div class="flex items-center justify-between gap-4 flex-wrap">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <p class="text-sm font-semibold text-dark">{{__('State Name')}}</p>
                                    </div>
                                    <div class="w-full sm:w-48">
                                ...
                                <p class="text-[11px] text-muted mt-2 ml-10">{{__('Only unique states will be added for the selected country.')}}</p>
                            </div>
