...
class PaymentLogControllerCopy extends Controller
{
    ...
    public function order_payment_form(Request $request)
    {
        ...
        $effectivePrice = $commissionService->handleSubscription($plan, $commissionSetting);
        ...
        if ($request->custom_subdomain != null) {
            ...
            $restricted_words = [
                'https', 'http', 'http://', 'https://', 'www', 'subdomain', 'domain',
                'primary-domain', 'central-domain', 'landlord', 'landlords', 'tenant', 'tenants',
                'admin', 'user', 'users', $site_domain
            ];
        ...
        try {
            ...
            if (!method_exists((new PaymentGatewayCredential()), $gateway_function))
            {
                ...
                if ($payment_gateway_name === 'razorpay' &&
                    RazorpaySubscriptionService::shouldUseRecurring($this->payment_details['tenant_id'])) {

                    $plan = PricePlan::find($this->payment_details['package_id']);

                    // FIRST: Create the PaymentLogs record
                    $paymentLog = PaymentLogs::create([
                        'email' => $this->payment_details['email'] ?? '',
                        'name' => $this->payment_details['name'] ?? '',
                        'package_name' => $this->payment_details['package_name'] ?? '',
                        'package_price' => $this->total,
                        'package_gateway' => 'razorpay',
                        'package_id' => $this->payment_details['package_id'],
                        'user_id' => $this->payment_details['user_id'] ?? null,
                        'tenant_id' => $this->payment_details['tenant_id'],
                        'theme_slug' => $this->payment_details['theme_slug'] ?? null,
                        'status' => 'pending',
                        'payment_status' => 'pending',
                        'is_recurring' => 1,
                    ...
                    // SECOND: Create subscription with the actual PaymentLogs object
                    $result = RazorpaySubscriptionService::createSubscriptionForTenant(
                        $paymentLog,  // ← Now it's a PaymentLogs object
                        $plan
                    );
                    ...
                    if ($result['status'] === 'success') {
                        ...
                        Log::warning('Razorpay subscription creation failed', [
                            'error' => $result['message']
                        ]);
                        return back()->with('msg', $result['message'])->with('type', 'danger');
                    }
                }

                // Fall back to regular one-time payment
...
//            event(new TenantRegisterEvent($user, $log->tenant_id, $log->theme_slug));
            Tenant::create([
                'id' => $log->tenant_id,
                'theme_slug' => $log->theme_slug ?? 'hexfashion'
            ]);
            try {
