            ...
            <div class="col-lg-4">
                <div class="bp-order-summary">
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <div class="bp-mini-cart-row">
                            ...
                            <div class="bp-checkout-mini-item-meta flex-grow-1">
                                ...
                                @if($data?->options?->color_name || $data?->options?->size_name)
                                <div class="bp-checkout-mini-variant">
                    ...
                    <div class="shipping_method_wrapper" style="border-top:1px solid #f0f0f0;padding-top:16px;">
                        ...
                        @if($user != null && isset($shipping_methods) && $shipping_methods->count())
                        <div class="bp-checkout-ship-heading">{{ __('Shipping') }}</div>
                        @foreach($shipping_methods as $method)
                        @php $is_default = false; if ($method->is_default) { $is_default = true; $default_shipping = $method; } @endphp
                        <div class="bp-summary-row bp-radio-row" data-country="{{ isset($country) ? $country : 0 }}" data-state="{{ isset($state) ? $state : 0 }}">
                            <label class="bp-checkout-ship-label d-flex align-items-center gap-2">
                                <input type="radio" class="shipping_methods bp-checkout-radio" id="shipping-option-{{ $method['id'] }}" name="shipping_method" value="{{ $method['id'] }}">
                                {{ $method['name'] }}
                            </label>
                        ...
                        <div class="shipping_tax_total">
                            ...
                            <div class="bp-summary-row price-shipping">
                                <span>{{ __('Shipping Cost (+)') }}</span>
                                <span>{{ isset($is_default) && $is_default ? amount_with_currency_symbol(0) : '--' }}</span>
                            </div>

                            @if(get_static_option('tax_system') == 'advance_tax_system')
                            <div class="bp-summary-row bp-summary-total price-total" data-total="{{ $subtotal + $v_tax_total }}" {{ !isset($is_default) ? 'data-total='.$itemsTotal.'' : '' }}>
                                <span>{{ __('Total Amount') }}</span>
                                <span class="bp-summary-price">{{ amount_with_currency_symbol($itemsTotal) }}</span>
                            </div>
                            @else
                            <div class="bp-summary-row bp-summary-total price-total" data-total="{{ $subtotal + $tax_ }}" {{ !isset($is_default) ? 'data-total='.($itemsTotal + $tax_).'' : '' }}>
                                <span>{{ __('Total Amount') }}</span>
                                <span class="bp-summary-price">{{ amount_with_currency_symbol($itemsTotal + $tax_) }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <p class="bp-checkout-trust-note mt-3">
                        <i class="las la-shield-alt bp-checkout-trust-icon"></i>
...
@section('scripts')
    ...
    $(function () {
        ...
        $(document).on('click', 'input[name=shipping_method]', function () {
            ...
            if (total !== undefined) { getShippingMethodBasedTotal(shipping_method, $('.coupon-country').val(), total); }
        });
        ...
        function getShippingMethodBasedTotal(shipping_method, country, total) {
            ...
            $.ajax({
                ...
                data: { shipping_method: shipping_method, country: country, total: total },
