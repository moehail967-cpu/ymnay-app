...
//
//        $domain = Domain::where('domain', $custom_domain->custom_domain)->first();
////        dd($domain);
//        $domain->delete();
//
...
//    }
    ...
    public function delete_request($id)
    {
        ...
        try {
            ...
            $customDomain->custom_domain_status = 'removed';
            $customDomain->save();

            if ($customDomain->tenant) {
                $subdomain_path = $customDomain->old_domain . '.' . config('app.central_domain');
                $customDomain->tenant->domains()->update(['domain' => $subdomain_path]);
            }
...
//    }
    public function bulk_action(Request $request)
    {
        ...
        try {
            $items = CustomDomain::whereIn('id', $request->ids)->get();
            ...
            foreach ($items as $item) {
                ...
                if ($item->tenant) {
                    $subdomain_path = $item->old_domain . '.' . config('app.central_domain');
                    $item->tenant->domains()->update(['domain' => $subdomain_path]);
                }
