...
@section('scripts')
    <script>
        (function ($) {
            ...
            $(document).on('click', '.mc-cart-action .close-table-cart', function (e){
                ...
                $.ajax({
                    ...
                    success: function (data){
                        if (data.msg) {
                            ...
                            if (data.empty_cart !== '') {
                                $('.mc-cart-section .container').html(data.empty_cart).hide();
                                $('.mc-cart-section .container').fadeIn();
                            }
                            $('.coupon-contents').parent().load(location.href + ' .coupon-contents');
                            $('.track-icon-list').load(location.href + ' .track-icon-list');
                            $('.mc-cart-table').closest('.mc-cart-table-wrap').load(location.href + ' .mc-cart-table');
                        }
                        $('.loader').hide();
                    },
                    error: function (){}
            ...
            $(document).on('click', '.mc-cart-action .close-table-wishlist', function (e){
                let el = $(this).closest('[data-product_hash_id]');
                let product_hash_id = el.data('product_hash_id');
                $.ajax({
            ...
            $(document).on('click', '.mc-cart-action .move-to-wishlist', function (e){
                let el = $(this).closest('[data-product_hash_id]');
                let product_hash_id = el.data('product_hash_id');
                $.ajax({
