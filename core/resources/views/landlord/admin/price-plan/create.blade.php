...
@section('title') {{__('Create Price Plan')}} @endsection
        ...
        <div class="lg:col-span-8 space-y-5">
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden">
                <div class="px-4 sm:px-6 py-4 border-b border-main flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Create Price Plan')}}</h3>
                        <p class="text-xs text-muted">{{__('Define plan details and pricing')}}</p>
                    </div>
                    <div class="ml-auto">
                ...
                <div class="p-4 sm:p-6 space-y-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="title" value="{{old('title')}}" placeholder="{{__('Enter plan title')}}" class="lnd-input">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Package Badge')}}</label>
                            <input type="text" name="package_badge" value="{{old('package_badge')}}" placeholder="{{__('e.g. Popular, Best Value')}}" class="lnd-input">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Package Description')}}</label>
                        <textarea name="package_description" rows="3" placeholder="{{__('Describe this plan')}}" class="lnd-input">{{old('package_description')}}</textarea>
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Price')}}</label>
                        <input type="number" name="price" value="{{old('price')}}" placeholder="{{__('Enter price')}}" class="lnd-input">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden">
                ...
                <div class="p-4 sm:p-6 space-y-5">
                                ...
                                @foreach($features as $key => $feat)
                                    <label for="{{$key}}" class="inline-flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="features[]"
                                               id="{{$key}}" class="exampleCheck1 w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary" value="{{$key}}"
                                               data-feature="{{$key}}">
                    ...
                    {{-- Dynamic permission boxes --}}
                    <div class="page_permission_box hidden">
                        ...
                        <input type="text" min="1" name="page_permission_feature" value="" placeholder="{{__('Page limit')}}" class="lnd-input">
                    </div>

                    <div class="blog_permission_box hidden">
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">
                            {{__('Blog Create Permission')}}
                            <i class="mdi mdi-information-outline text-primary text-xs ml-1" title="{{__('Keep -1 for Unlimited')}}"></i>
                        </label>
                        <input type="text" min="1" name="blog_permission_feature" value="" placeholder="{{__('Blog limit')}}" class="lnd-input">
                    </div>

                    <div class="product_permission_box hidden">
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">
                            {{__('Product Create Permission')}}
                            <i class="mdi mdi-information-outline text-primary text-xs ml-1" title="{{__('Keep -1 for Unlimited')}}"></i>
                        </label>
                        <input type="text" min="1" name="product_permission_feature" value="" placeholder="{{__('Product limit')}}" class="lnd-input">
                    </div>

                    <div class="storage_permission_box hidden">
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">
                            {{__('Storage Create Permission')}}
                            <i class="mdi mdi-information-outline text-primary text-xs ml-1" title="{{__('Keep -1 for Unlimited')}}"></i>
                        </label>
                        <input type="text" min="1" name="storage_permission_feature" value="" placeholder="{{__('Storage limit (MB)')}}" class="lnd-input">
                    </div>
                </div>
            </div>
                    ...
                    @else
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                            @foreach($tenantPlugins as $plugin)
                                <div class="ppc-card" data-plugin="{{ $plugin->id }}">
                                    <input type="checkbox" name="plan_plugins[]" value="{{ $plugin->id }}"
                                           class="plan-plugin-checkbox sr-only">
                                    <div class="ppc-top">
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden">
                <div class="px-4 sm:px-6 py-4 border-b border-main flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted">{{__('Add frequently asked questions')}}</p>
                    </div>
                </div>
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden sticky top-4">
                ...
                <div class="p-4 space-y-5">
                    ...
                    <div>
                        ...
                        <select name="type" class="package_type lnd-input">
                    ...
                    <div class="flex items-center justify-between px-4 py-3 rounded-xl bg-secondary border border-main">
                        ...
                        <label class="dr-toggle">
                            <input type="checkbox" name="has_trial">
                            <span class="dr-toggle-track"></span>
                        </label>
                    </div>

                    {{-- Trial Days --}}
                    <div class="trial_date_box hidden">
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Trial Days')}}</label>
                        <input type="number" name="trial_days" placeholder="{{__('Days..')}}" class="lnd-input">
                    </div>
                    @endif
