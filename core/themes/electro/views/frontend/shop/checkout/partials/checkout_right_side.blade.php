<div class="el-checkout-summary">
    <h2 class="el-dash-card-title"> {{__('Order Summary')}} </h2>
    <form action="#" class="el-coupon-row coupon-contents-form mt-4">
        ...
        <input type="hidden" class="coupon-shipping-method" name="coupon_shipping_method"
               value="">
        <input class="el-form-input coupon-code" type="text" placeholder="{{__('Coupon Code')}}"
               name="used_coupon">
        <button type="button" class="el-btn el-btn-outline coupon-btn"> {{__('Apply')}}</button>
    </form>

    <div class="el-checkout-items coupon-border mt-4 mb-4">
        @php
            $subtotal = null;
            $itemsTotal = null;
            $v_tax_total = 0;
        ...
        @foreach($carts ?? [] as $data)
            ...
            <div class="el-checkout-item">
                <div class="el-checkout-item-img">
                    <a href="javascript:void(0)">
                        {!! render_image_markup_by_attachment_id($data?->options?->image) !!}
                    </a>
                </div>
                <div class="el-checkout-item-name">
                    <a href="javascript:void(0)"> {{$data->name}} </a>
                    <span class="el-cart-meta d-block mt-1">
                    ...
                    <span class="el-cart-meta"> {{__('Quantity:')}} {{$data->qty}} </span>
                </div>
                <span class="el-checkout-item-price"> {{amount_with_currency_symbol($price * $data->qty)}} </span>
            </div>
    ...
    <div class="coupon-contents-details">
        ...
        <div class="shipping_method_wrapper">
            ...
            @if($user != null)
                <div class="el-summary-row d-block">
                    ...
                    @foreach($shipping_methods ?? [] as $key => $method)
                        ...
                        <div class="el-summary-row" data-country="{{isset($country) ? $country : 0}}"
    ...
    @php
        $COD = get_static_option('cash_on_delivery');
    @endphp
    @if($COD === 'on')
        <div class="el-check-row cash-on-delivery mt-2">
    ...
    <div class="el-payment-section mt-4">
        ...
        <div class="payment-card mt-3">
            ...
            <div class="d-none w-100 mt-3 manual_transaction_id">
                ...
                <input type="text" name="trasaction_id" class="el-form-input"
    ...
    <div class="el-check-row mt-3">
        <input class="check-input" type="checkbox" id="agree" checked>
        <label for="agree"> {{__('I have read and agree to the website')}} <a
                class="el-terms-link terms-condition" href="javascript:void(0)"> {{__('terms and conditions*')}} </a> </label>
    </div>

    <div class="mt-3">
        <a href="javascript:void(0)"
           class="el-btn el-btn-primary el-btn-block checkout_disable proceed_checkout_btn"> {{__('Proceed to Checkout')}} </a>
    </div>

    <div class="mt-3">
        <a href="{{theme_cart_url()}}" class="el-btn el-btn-outline el-btn-block"> {{__('Return to Cart')}} </a>
    </div>
</div>
