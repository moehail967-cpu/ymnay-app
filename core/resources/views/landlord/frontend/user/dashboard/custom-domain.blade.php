        ...
        <div class="col-12">
            <div class="card card-new-styles">
                ...
                <div class="card-body">
                    ...
                    @if($screenshotImg)
                    <div class="mt-3">
                        <img src="{{ $screenshotImg }}" alt="{{ __('DNS Setup Example') }}" class="img-fluid rounded">
                    </div>
                    @endif
                                            ...
                                            @foreach($user_domain_infos->tenant_details ?? [] as $tenant)
                                                ...
                                                <td>
                                                    @if(optional($tenant->custom_domain)->custom_domain_status == 'pending')
                                                        <span class="text-warning text-capitalize">{{__(optional($tenant->custom_domain)->custom_domain_status)}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == 'in_progress')
                                                        <span class="text-info text-capitalize">{{__(str_replace('_',' ',optional($tenant->custom_domain)->custom_domain_status))}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == 'connected')
                                                        <span class="text-success text-capitalize">{{__(optional($tenant->custom_domain)->custom_domain_status)}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == 'rejected')
                                                        <span class="text-info text-capitalize">{{__(str_replace('_', ' ',ucwords(optional($tenant->custom_domain)->custom_domain_status)))}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == null)

                                                    @else
                                                        <span class="text-danger text-capitalize">{{__(optional($tenant->custom_domain)->custom_domain_status) ?? __('Removed')}}</span>
                                                    @endif
                                                </td>
                                                <td>
