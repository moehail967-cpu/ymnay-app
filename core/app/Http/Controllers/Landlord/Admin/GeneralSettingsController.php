...
//    }
    ...
    public function update_basic_settings(Request $request)
    {
        // DARK MODE TOGGLE (tenant + landlord)
        if ($request->method() == 'POST' && !empty($request->dark_mode_enable_check)) {
        ...
        $fields = [
            ...
            'site_tag_line' => 'nullable|string',
            'site_footer_copyright_text' => 'nullable|string',
            'login_page_headline' => 'nullable|string',
            'login_page_subtitle' => 'nullable|string',
            'login_page_feature_1' => 'nullable|string',
            'login_page_feature_2' => 'nullable|string',
            'login_page_feature_3' => 'nullable|string',
            'register_page_headline' => 'nullable|string',
            'register_page_subtitle' => 'nullable|string',
            'register_page_promo_text' => 'nullable|string',
        ];
        ...
        if (!tenant()) {
            if (!empty($request->timezone)) {
                setEnvValue(['APP_TIMEZONE' => $request->timezone]);
            }
            setEnvValue(['APP_DEBUG' => !empty($request->debug_mode) ? 'true' : 'false']);
        }
    ...
    {
        $fields = [
            'site_logo' => 'nullable|integer',
            'site_white_logo' => 'nullable|integer',
            'site_favicon' => 'nullable|integer',
        ];
    ...
    public function update_color_settings(Request $request)
    {
        ...
        if (tenant()) {
            ...
            $fields = [
                ...
                // Admin panel colors
                'admin_bg_base' => 'nullable|string|max:191',
                'admin_surface_bg' => 'nullable|string|max:191',
                'admin_primary_color' => 'nullable|string|max:191',
                'admin_primary_hover_color' => 'nullable|string|max:191',
                'admin_primary_soft_color' => 'nullable|string|max:191',
                'admin_primary_soft_hover_color' => 'nullable|string|max:191',
                'admin_sidebar_bg' => 'nullable|string|max:191',
                'admin_secondary_bg' => 'nullable|string|max:191',
                'admin_muted_bg' => 'nullable|string|max:191',
                'admin_brand_text_color' => 'nullable|string|max:191',
                'admin_border_color' => 'nullable|string|max:191',
                'admin_border_subtle_color' => 'nullable|string|max:191',
                'admin_border_hover_color' => 'nullable|string|max:191',
                'admin_sidebar_hover_color' => 'nullable|string|max:191',
                // Admin text colors
                'admin_text_main_color' => 'nullable|string|max:191',
                'admin_text_dark_color' => 'nullable|string|max:191',
                'admin_text_muted_color' => 'nullable|string|max:191',
                'admin_text_subtle_color' => 'nullable|string|max:191',
            ];
        }
    ...
    public function update_typography_settings(Request $request)
    {
        ...
        if (tenant()) {
            ...
            foreach ($font_variant as $variant) {
                update_static_option($variant, serialize(!empty($request->$variant) ? $request->$variant : ['0,400']));
            }
        } else {
            $fields = [
            ...
            $font_variant = [
                'body_font_variant' => 'nullable',
                'heading_font_variant' => 'nullable'
            ];

            $this->validate($request, $font_variant);
            foreach ($font_variant as $key => $item) {
                update_static_option($key, serialize(!empty($request->$key) ? $request->$key : ['0,400']));
            }
        }
    ...
    {
        // Bug 7 fix: SMTP-specific fields are not required when using the sendmail driver
        $smtpRequired = $request->site_smtp_driver !== 'sendmail';

        $fields = [
            'site_global_email'    => 'required|email',
            'site_smtp_host'       => $smtpRequired ? 'required|string|regex:/^\S*$/u' : 'nullable|string',
            'site_smtp_username'   => $smtpRequired ? 'required|string' : 'nullable|string',
            'site_smtp_password'   => $smtpRequired ? 'required|string' : 'nullable|string',
        ...
        $this->validate($request, $fields);
        foreach ($fields as $field_name => $rules) {
            // Bug 5 fix: only update central (landlord) settings here — do NOT touch tenant DB
    ...
    public function send_test_mail(Request $request)
    {
        ...
        // Bug 3 fix: use $driver as the mailer key so config targets the correct mailer,
        // not always mail.mailers.smtp.* regardless of which driver is selected
        $driver     = get_static_option_central('site_smtp_driver') ?: 'smtp';
        $encryption = get_static_option_central('site_smtp_encryption');
        $encryption = ($encryption === 'null' || $encryption === null) ? null : $encryption;

        config([
            'mail.default'                          => $driver,
            "mail.mailers.{$driver}.transport"      => $driver,
            "mail.mailers.{$driver}.host"           => get_static_option_central('site_smtp_host'),
            "mail.mailers.{$driver}.port"           => (int) get_static_option_central('site_smtp_port'),
            "mail.mailers.{$driver}.encryption"     => $encryption,
            "mail.mailers.{$driver}.username"       => get_static_option_central('site_smtp_username'),
            "mail.mailers.{$driver}.password"       => get_static_option_central('site_smtp_password'),
            'mail.from.address'                     => get_static_option_central('site_global_email'),
            'mail.from.name'                        => get_static_option_central('site_title') ?: config('app.name'),
        ]);

        Mail::forgetMailers();

        try {
            $message = __('Hi Landlord,') . '<br>';
            $message .= __('This is Test Mail');

            Mail::to($request->email)->send(new BasicMail($message, __('SMTP Test email')));
        } catch (\Exception $e) {
            // Bug 9 fix: log SMTP errors to laravel.log for diagnosis, not just the UI toast
            Log::error('SMTP test mail failed', [
                'driver'     => $driver,
                'host'       => get_static_option_central('site_smtp_host'),
                'port'       => get_static_option_central('site_smtp_port'),
                'error'      => $e->getMessage(),
                'error_code' => $e->getCode(),
            ]);

            if ($e->getCode() == 535 || str_contains($e->getMessage(), '535')) {
...
//            case "queue":
    ...
    public function update_database_upgrade(Request $request)
    {
        try {
            ...
            // 1 Run landlord migrations and seeders
            Artisan::call('migrate', ['--force' => true]);
            Artisan::call('db:seed', ['--class' => UpdateInfoSeeder::class, '--force' => true]);
            Artisan::call('db:seed', ['--force' => true]);

            // 2 Run tenant migrations and tenant-specific seeders
            ...
            // 3.Backfill tenant renewal data
            try {
            ...
            // 4 Sync tenant digital product permissions (command-based sync)
            try {
            ...
            // 5 Clear caches
            try {
                // Use Cache facade directly instead of Artisan command to avoid CacheManager dependency issues
                ...
                Log::warning('Cache clear partially failed: ' . $e->getMessage());
                // Continue anyway - cache clear failure shouldn't stop the upgrade
            }
