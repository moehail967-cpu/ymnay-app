...
<div class="container" style="padding-top:28px;">
    <div class="d-flex align-items-center justify-content-center gap-0 mb-4">
        <div class="text-center" style="flex:1;">
            <div style="width:36px;height:36px;border-radius:50%;background:var(--ch-green);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;margin:0 auto 6px;">
                <i class="las la-check" style="font-size:18px;"></i>
            </div>
            <div style="font-size:12px;font-weight:700;color:var(--ch-green);">{{ __('Cart') }}</div>
        </div>
        <div style="flex:2;height:2px;background:var(--ch-green);margin-bottom:20px;"></div>
        <div class="text-center" style="flex:1;">
            <div style="width:36px;height:36px;border-radius:50%;background:var(--ch-red);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;margin:0 auto 6px;">2</div>
            <div style="font-size:12px;font-weight:700;color:var(--ch-red);">{{ __('Checkout') }}</div>
        </div>
    ...
    <div class="row g-4" style="padding-bottom:72px;">
        ...
        <div class="col-lg-7">
            ...
            {{-- Guest Login Prompt --}}
            <div class="ch-section-box">
                <div class="ch-section-box-title"><i class="las la-user"></i> {{ __('Sign In to Continue') }}</div>
                <x-flash-msg/>
                <form id="login_form_order_page">
                    <div class="error-wrap mb-3"></div>
                    <div class="mb-3">
                        <label class="ch-label">{{ __('Username') }}</label>
                        <input type="text" name="username" class="ch-price-input" placeholder="{{ __('Type your username') }}">
                    </div>
                    <div class="mb-3">
                        <label class="ch-label">{{ __('Password') }}</label>
                        <input type="password" name="password" class="ch-price-input" placeholder="{{ __('Password') }}">
                    </div>
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <label class="d-flex align-items-center gap-2" style="font-size:13px;cursor:pointer;">
                            <input type="checkbox" name="remember"> {{ __('Remember me') }}
                        </label>
                        <a href="{{ theme_forget_password_url() }}" style="font-size:13px;color:var(--ch-red);">{{ __('Forgot Password?') }}</a>
                    </div>
                    <button type="submit" id="login_btn" class="ch-btn ch-btn-red">{{ __('Sign In') }}</button>
                    <p class="mt-3" style="font-size:13px;color:var(--ch-muted);">
                        {{ __("Don't have an account?") }}
                        <a href="{{ theme_register_url() }}" style="color:var(--ch-red);">{{ __('Sign up') }}</a>
                    </p>
                </form>
            </div>
            @endif
            ...
            <form action="{{ theme_checkout_url() }}" method="POST" class="checkout-form" enctype="multipart/form-data">
                ...
                {{-- Delivery Address --}}
                <div class="ch-section-box">
                    <div class="ch-section-box-title"><i class="las la-map-marker"></i> {{ __('Delivery Address') }}</div>
                    <div class="row g-3">
                        <div class="col-12">
                            <label class="ch-label">{{ __('Full Name') }}</label>
                            <input type="text" name="name" class="ch-price-input"
                                   placeholder="{{ __('Type Full Name') }}"
                                   value="@auth('web'){{ $billing_info ? $billing_info->full_name : auth('web')->user()?->name }}@else{{ old('name') }}@endauth"
                                   {{ $readonly }}>
                        </div>
                        <div class="col-md-6">
                            <label class="ch-label">{{ __('Mobile Number') }}</label>
                            <input type="tel" name="phone" class="ch-price-input"
                        ...
                        <div class="col-md-6">
                            <label class="ch-label">{{ __('Email Address') }}</label>
                            <input type="email" name="email" class="ch-price-input"
                                   placeholder="{{ __('Type Email') }}"
                                   value="@auth('web'){{ $billing_info ? $billing_info->email : auth('web')->user()?->email }}@else{{ old('email') }}@endauth"
                                   {{ $readonly }}>
                        </div>
                        <div class="col-md-6">
                            <label class="ch-label">{{ __('ZIP / PIN Code') }} <span style="color:var(--ch-red);">*</span></label>
                            <input type="text" name="postal_code" class="ch-price-input" maxlength="6"
                        ...
                        <div class="col-md-6">
                            <label class="ch-label">{{ __('Country') }}</label>
                            <select name="country" class="ch-price-input billing_address_country" id="country">
                        ...
                        <div class="col-12">
                            <label class="ch-label">{{ __('Address') }}</label>
                            <textarea name="address" class="ch-price-input" rows="3"
                        ...
                        <div class="col-12">
                            <label class="ch-label">{{ __('Order Notes') }} <span style="color:var(--ch-muted);font-weight:400;">({{ __('optional') }})</span></label>
                            <textarea name="message" class="ch-price-input" rows="2"
                    ...
                    <div class="mt-4">
                        <a href="javascript:void(0)" class="create-accounts fw-500" style="color:var(--ch-red);font-size:13px;">
                        ...
                        <div class="checkout-form-open mt-3">
                            <div class="row g-3">
                                <div class="col-12">
                                    <label class="ch-label">{{ __('Username') }}</label>
                                    <input type="text" name="create_username" class="ch-price-input" placeholder="{{ __('Type a unique username') }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="ch-label">{{ __('Password') }}</label>
                                    <input type="password" name="create_password" class="ch-price-input" placeholder="{{ __('Type a strong password') }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="ch-label">{{ __('Confirm Password') }}</label>
                                    <input type="password" name="create_password_confirmation" class="ch-price-input" placeholder="{{ __('Confirm password') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {{-- Payment Method --}}
                <div class="ch-section-box">
                    <div class="ch-section-box-title"><i class="las la-credit-card"></i> {{ __('Payment Method') }}</div>

                    <div class="ch-checkout-payment-wrap">
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <label class="d-flex align-items-center gap-2 ch-checkout-cod" style="font-size:14px;cursor:pointer;">
                                <input type="checkbox" id="cash" style="accent-color:var(--ch-red);width:16px;height:16px;">
                                <i class="las la-money-bill" style="font-size:20px;color:var(--ch-green);"></i>
                                {{ __('Cash On Delivery') }}
                            </label>
                        </div>
                        ...
                        <div class="form-group d-none manual_transaction_id mt-3">
                            <label class="ch-label">{{ __('Transaction ID') }}</label>
                            <input type="text" name="trasaction_id" class="ch-price-input" placeholder="{{ __('Transaction ID') }}">
                        </div>
                        <div class="summernot_wrap" style="display:none;">
                            <div class="manual_description mt-2" style="font-size:13px;color:var(--ch-muted);"></div>
                        </div>
        ...
        <div class="col-lg-5">
            <div class="ch-order-summary">
                <div class="ch-order-summary-title">{{ __('Your Order') }}</div>
                ...
                <div class="mb-3">
                    <div class="d-flex gap-2">
                        ...
                        <input type="text" class="ch-price-input flex-grow-1 coupon-code"
                               placeholder="{{ __('Coupon Code') }}" name="used_coupon_display">
                        <button type="button" class="ch-btn ch-btn-red ch-btn-sm coupon-btn" style="white-space:nowrap;">
                            {{ __('Apply') }}
                        </button>
                    </div>
                </div>
                ...
                <div class="ch-mini-cart-row">
                    <div class="ch-mini-cart-img">
                        {!! render_image_markup_by_attachment_id($data?->options?->image) !!}
                    </div>
                    <div style="flex:1;min-width:0;">
                        <div style="font-size:13px;font-weight:700;color:var(--ch-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                            {{ $data->name }} ×{{ $data->qty }}
                        </div>
                        @if($data?->options?->color_name || $data?->options?->size_name || !empty($data?->options?->attributes))
                        <div style="font-size:11px;color:var(--ch-muted);">
                    ...
                    <span class="ch-price" style="font-size:13px;white-space:nowrap;">
                ...
                <div class="shipping_method_wrapper">
                    ...
                    <div class="ch-summary-row mt-2">
                        <span>{{ __('Subtotal') }}</span>
                        <span class="ch-summary-price">{{ site_currency_symbol() }}{{ Cart::subtotal() }}</span>
                    </div>
                    <div class="ch-summary-row coupon-price">
                        <span>{{ __('Coupon Discount (-)') }}</span>
                        <span class="ch-summary-price" style="color:var(--ch-green);">{{ amount_with_currency_symbol(0.00) }}</span>
                    </div>
                    <div class="ch-summary-row">
                        <span>{{ __('Tax (Incl)') }}</span>
                        <span class="ch-summary-price">{{ $product_tax }}%</span>
                    </div>

                    @if(theme_is_logged_in() && !empty($shipping_methods ?? []))
                    <div style="margin:10px 0 4px;font-size:12px;font-weight:700;color:var(--ch-dark);">{{ __('Shipping') }}</div>
                    @foreach($shipping_methods ?? [] as $key => $method)
                    <div class="ch-summary-row" data-country="{{ $country ?? '' }}" data-state="{{ $state ?? '' }}">
                        <label class="d-flex align-items-center gap-2" style="cursor:pointer;font-size:13px;margin:0;">
                            <input type="radio" class="shipping_methods"
                                   ...
                                   value="{{ $method['id'] }}"
                                   style="accent-color:var(--ch-red);"
                                   {{ $key === 0 ? 'checked' : '' }}>
                            {{ $method['name'] }}
                        </label>
                        <span class="ch-summary-price">{{ amount_with_currency_symbol($method['options']['cost']) }}</span>
                    </div>
                    @endforeach
                    @endif

                    <div class="ch-summary-row price-shipping">
                        <span>{{ __('Shipping Cost (+)') }}</span>
                        <span class="ch-summary-price">
                            {{ isset($default_shipping) ? amount_with_currency_symbol($default_shipping['options']['cost']) : '--' }}
                        </span>
                    </div>

                    <div class="ch-summary-row total price-total" data-total="{{ $subtotal + $taxed_price }}">
                ...
                <div class="mt-3 d-flex align-items-center gap-2">
                    <input type="checkbox" id="agree" checked style="accent-color:var(--ch-red);width:15px;height:15px;">
                    <label for="agree" style="font-size:12px;color:var(--ch-muted);cursor:pointer;">
                        {{ __('I agree to the') }}
                        <a href="javascript:void(0)" style="color:var(--ch-red);">{{ __('Terms & Conditions') }}</a>
                    </label>
                </div>

                <button type="button" class="ch-btn ch-btn-red w-100 justify-content-center proceed_checkout_btn checkout_disable mt-3"
                        style="font-size:15px;padding:14px;">
                    <i class="las la-lock"></i> {{ __('Place Order') }}
                </button>

                <a href="{{ theme_cart_url() }}" class="ch-btn ch-btn-outline w-100 justify-content-center mt-2">
                    {{ __('Return to Cart') }}
                </a>
...
(function ($) {
    $(document).ready(function () {
        ...
        // Country → fetch states (shift address)
        ...
        // Country/state change → sync total
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.type === 'success') {
                        ...
                        $('.price-shipping .ch-summary-price').html(c + data.selected_shipping_method.options.cost);
                        $('.price-total .ch-summary-price').html(c + data.total);
                        $('.coupon-shipping-method').val(method);
                    } else {
                        toastr.error(data.msg);
                        btn.css({'opacity': '.5', 'cursor': 'not-allowed'}).removeClass('proceed_checkout_btn');
                    }
                    $('.loader').hide();
                }
            });
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.type === 'error') {
                        ...
                        $('.price-total .ch-summary-price').text(c + data.coupon_amount);
                        $('.coupon-price .ch-summary-price').text(c + data.coupon_price);
