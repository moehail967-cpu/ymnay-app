<div class="single-shop-details-wrapper">
    ...
    <div class="wishlist-compare mt-2 mb-2">
    ...
    {{-- Delivery Options — right after Add to Cart / Buy Now --}}
    ...
    {{-- Safe Checkout / payment logos — right after delivery options --}}
        ...
        <ul class="stock-category">
            <li class="category-list">
                <span class="list-item fw-600">
                    ...
                    <a href="{{ theme_shop_url() }}?category={{ $product->category->slug }}">{{ $product->category->name }}</a>
                    @endif
                    @if($product?->subCategory?->name)
                    |
                    <a href="{{ theme_shop_url() }}?subcategory={{ $product->subCategory->slug }}">{{ $product->subCategory->name }}</a>
                    @endif
                    @foreach($product->childCategory ?? [] as $child_category)
                        |
                        <a href="{{ theme_shop_url() }}?child-category={{ $child_category?->slug }}"> {{ $child_category->name }} </a>
                    @endforeach
                </span>
            </li>
            @if($product->uom != null)
    ...
    {{-- Social Share --}}
    <div class="wishlist-share social_share_parent mt-4">
