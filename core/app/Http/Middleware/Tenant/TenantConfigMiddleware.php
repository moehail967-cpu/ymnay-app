...
class TenantConfigMiddleware
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (tenant()){
            ...
            // Bug 4 fix: normalize the string "null" stored by the form to an actual null
            $encryption = $smtp_settings_values['site_smtp_encryption'] ?? Config::get('mail.mailers.smtp.encryption');
            if ($encryption === 'null') {
                $encryption = null;
            }

            // Bug 2 fix: set the active mailer to match the tenant's chosen driver
            Config::set('mail.default', $mailers);

            // Bug 1 fix: removed the erroneous Config::set('mail.mailers', string) that overwrote the entire mailers array
            // Bug 6 fix: removed verify_peer/verify_peer_name/allow_self_signed — SSL certificates must be validated
            Config::set([
                ...
                "mail.mailers.{$mailers}.encryption" => $encryption,
