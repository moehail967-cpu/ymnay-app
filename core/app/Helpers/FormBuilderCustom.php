...
class FormBuilderCustom
{
    public static function render_form($id,$form_id = null,$action = null,$button_class = null,$form_class = null){
        ...
        $recaptcha = self::render_recaptcha();
...
HTML;
    ...
    private static function render_recaptcha(): string
    {
        $siteKey = config('services.recaptcha.site_key', '');
        if (empty($siteKey)) {
            return '';
        }
        return '<div class="form-group recaptcha-wrap"><div class="g-recaptcha" data-sitekey="' . $siteKey . '"></div></div>';
    }
...
public static function render_tw_drag_drop_form_builder($content = '')
{
    $output = '';
    $form_fields = json_decode($content);
    ...
    if (!empty($form_fields)) {
        ...
        foreach ($form_fields->field_type as $key => $ftype) {
            ...
            $output .= self::tw_form_builder_field_markup($key, $ftype, $form_fields->field_name[$key], $form_fields->field_placeholder[$key], $required_field, $args);
        }
    } else {
        $output .= self::tw_form_builder_field_markup('1', 'text', 'your-name', 'Your Name', '');
    }
    $output .= '</ul>';
    return $output;
}

private static function tw_form_builder_field_markup($key, $type, $name, $placeholder, $required, $args = [])
{
    $name = SanitizeInput::esc_html($name);
    $placeholder = SanitizeInput::esc_html($placeholder);
    ...
    if ($type === 'select') {
        $output .= '<div class="mb-3">
            ...
            <textarea name="select_options[]" class="lnd-input" style="max-height:120px" cols="30" rows="5" required>' . SanitizeInput::esc_html($args['select_option']) . '</textarea>
