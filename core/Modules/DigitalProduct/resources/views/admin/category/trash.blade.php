...
@section('title') {{__('Trashed Categories')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main">
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Trashed Products Categories')}}</h3>
                <p class="text-xs text-muted">{{__('Restore or permanently delete')}}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            ...
            @can('product-category-create')
                <a href="{{route('tenant.admin.product.category.all')}}"
    ...
    <div class="tw-table-wrap">
        <table class="w-full text-left" id="trashTable">
            <thead>
            ...
            @foreach($all_category as $key => $category)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    <td class="px-4 py-3.5"><x-bulk-delete-checkbox :id="$category->id"/></td>
                    <td class="px-4 py-3.5"><span class="text-[11px] font-bold text-primary">#{{$loop->iteration}}</span></td>
                    <td class="px-4 py-3.5"><span class="text-sm font-semibold text-dark">{{$category->name}}</span></td>
                    <td class="px-4 py-3.5">
                        <div class="tw-thumb-wrap">{!! render_image_markup_by_attachment_id($category->image_id) !!}</div>
                    </td>
                    <td class="px-4 py-3.5">
                        <div class="flex items-center justify-end gap-1.5">
                            @can('product-category-edit')
                                <a href="{{route('tenant.admin.product.category.trash.restore', $category->id)}}"
                                   class="tw-btn-icon tw-btn-icon-approve" title="{{__('Restore')}}">
                                    <i class="mdi mdi-backup-restore"></i>
                                </a>
                            @endcan
                            @can('product-category-delete')
                                <a href="{{route('tenant.admin.product.category.trash.delete', $category->id)}}"
                                   class="tw-btn-icon tw-btn-icon-danger swal_confirm_link" title="{{__('Delete Permanently')}}">
...
@section('scripts')
    <x-datatable.tw-js/>
    @can('product-category-delete')
        <x-bulk-action-js :url="route('tenant.admin.product.category.trash.delete.bulk')"/>
    @endcan
    <script>
    (function ($) {
        "use strict";
        $(document).ready(function () {
            if ($.fn.DataTable && !$.fn.dataTable.isDataTable('#trashTable')) {
