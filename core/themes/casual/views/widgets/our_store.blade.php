...
<section class="cs-store-section" style="padding-top:{{ $padding_top }}px;padding-bottom:{{ $padding_bottom }}px;">
    <div class="container">
        <div class="cs-section-head">
            <h2 class="cs-section-title">{{ $title }}</h2>
        </div>

        <div class="cs-store-layout" id="{{ $uid }}-wrap">
            {{-- Sidebar: Categories --}}
            <aside class="cs-store-sidebar">
                <ul class="cs-store-catlist">
                    <li>
                        <button class="cs-store-cat-btn active" data-cat="all" data-target="{{ $uid }}">
                            {{ __('All') }}
                        </button>
                    </li>
                    @foreach($categories as $cat)
                    <li>
                        <button class="cs-store-cat-btn" data-cat="{{ $cat->slug ?? $cat->id }}" data-target="{{ $uid }}">
                            {{ $cat->name }}
                        </button>
                    </li>
                    @endforeach
            ...
            {{-- Products Grid --}}
            <div class="cs-store-grid" id="{{ $uid }}">
                ...
                @endphp
                <div class="cs-store-item h-100" data-cat="{{ $catSlug }}">
...
(function(){
    ...
    btns.forEach(function(btn){
        btn.addEventListener('click', function(){
            ...
            grid.querySelectorAll('.cs-store-item').forEach(function(item){
