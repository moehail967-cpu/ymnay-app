...
@section('title') {{__('Product Unit')}} @endsection
    ...
    <div class="xl:col-span-7">
        <div class="bg-surface rounded-xl shadow-main border border-main">
            ...
            <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                        <i class="mdi mdi-weight text-primary text-base"></i>
                    </div>
                    <div>
                        <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Units')}}</h3>
                        <p class="text-xs text-muted">{{__('Manage product measurement units')}}</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    @can('product-unit-delete')
                        <x-bulk-action permissions="product-unit-delete"/>
                    @endcan
                </div>
            </div>
            ...
            <div class="tw-table-wrap">
                <table class="w-full text-left" id="unitTable">
                    <thead>
                    ...
                    @foreach($product_units as $unit)
                        <tr class="border-b border-main hover:bg-muted transition-colors">
                            <td class="px-4 py-3.5">
                                <x-bulk-delete-checkbox :id="$unit->id"/>
                            </td>
                            <td class="px-4 py-3.5">
                                <span class="text-[11px] font-bold text-primary">#{{$unit->id}}</span>
                            </td>
                            <td class="px-4 py-3.5">
                                <span class="text-sm font-semibold text-dark">{{$unit->name}}</span>
                            </td>
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-end gap-1.5">
                                    @can('product-unit-edit')
                                        <button type="button"
                                                class="tw-btn-icon tw-btn-icon-edit unit_edit_btn"
                                                title="{{__('Edit')}}"
                                                data-id="{{$unit->id}}"
                                                data-name="{{$unit->name}}">
                                    ...
                                    @can('product-unit-delete')
                                        <button type="button" class="tw-btn-icon tw-btn-icon-danger swal_delete_button"
                                                data-route="{{ route('tenant.admin.product.units.delete', $unit->id) }}"
                                                title="{{__('Delete')}}">
        ...
        <div class="bg-surface rounded-xl shadow-main border border-main">
            <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Add New Unit')}}</h3>
                        <p class="text-xs text-muted">{{__('Create a new measurement unit')}}</p>
                    </div>
                </div>
            </div>
...
{{-- Edit Modal --}}
@can('product-unit-edit')
<div id="unit_edit_modal" class="fixed inset-0 z-[999] hidden">
    <div class="fixed inset-0 bg-black/40 backdrop-blur-sm" onclick="closeModal('unit_edit_modal')"></div>
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-xl shadow-xl border border-main w-full max-w-md">
            <div class="flex items-center justify-between px-5 py-3.5 border-b border-main">
                <h5 class="text-sm font-bold text-dark">{{__('Update Unit')}}</h5>
                <button type="button" onclick="closeModal('unit_edit_modal')" class="text-muted hover:text-danger transition">
            ...
            <form action="{{ route('tenant.admin.product.units.update') }}" method="post">
                ...
                <div class="flex items-center justify-end gap-2 px-5 py-3.5 border-t border-main">
                    <button type="button" onclick="closeModal('unit_edit_modal')" class="px-4 py-2 rounded-lg text-sm font-medium text-muted hover:bg-secondary transition">{{__('Close')}}</button>
...
@section('scripts')
    <x-datatable.tw-js/>
    <x-bulk-action-js :url="route('tenant.admin.product.units.bulk.action')"/>
    <script>
    (function ($) {
        "use strict";
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.unit_edit_btn', function () {
                var el = $(this);
                $('#unit_id').val(el.data('id'));
                $('#edit_name').val(el.data('name'));
                openModal('unit_edit_modal');
            });
