<div class="mc-cart-items {{ $wishlist ? 'mc-cart-items--full' : '' }}">
    <div class="mc-cart-table-wrap">
        <table class="mc-cart-table">
            <thead>
                <tr>
                    <th>{{__('Product')}}</th>
                    <th>{{__('Price')}}</th>
            ...
            @foreach($cart_data as $key => $data)
                ...
                @endphp
                <tr class="mc-cart-row" data-product-id="{{$key}}" data-varinat-id="{{$data->variant_id}}">
                    <td data-label="{{__('Product')}}">
                        <div class="mc-cart-product">
                            <div class="mc-cart-thumb"
                            ...
                            <div>
                                ...
                                @endphp
                                <a href="{{$product_details_route}}" class="mc-cart-name">{{$data->name}}</a>
                                <span class="mc-cart-type">{{$product_type}}</span>
                    ...
                    <td data-label="{{__('Action')}}" class="mc-cart-action" data-product_hash_id="{{$data->rowId}}">
                        <div class="mc-cart-action-inner">
                            @if($wishlist)
    ...
    <div class="mc-cart-actions mt-4">
        <a href="{{theme_shop_url()}}" class="mc-btn mc-btn-outline">{{__('Continue Shopping')}}</a>
        @if(!$wishlist)
            <a href="javascript:void(0)" class="mc-btn mc-btn-ghost clear-cart-btn">{{__('Clear Cart')}}</a>
        @endif
    </div>
</div>
