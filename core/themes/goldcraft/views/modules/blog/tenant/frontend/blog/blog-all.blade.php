...
@section('title') {{ __('Craft Stories') }} @endsection

@section('content')
<div style="background:var(--gc-warm);border-bottom:1px solid var(--gc-border);padding:36px 0 28px;">
    <div class="container">
        <span style="display:block;font-size:10px;font-weight:400;letter-spacing:3px;text-transform:uppercase;color:var(--gc-rose);margin-bottom:8px;">{{ __('Stories & Inspiration') }}</span>
        <h2 style="font-size:26px;font-weight:400;color:var(--gc-dark);margin-bottom:8px;font-style:italic;">{{ __('The Craft Journal') }}</h2>
                        ...
                        <div style="background:#fff;border:1px solid var(--gc-border);border-radius:var(--gc-radius);overflow:hidden;box-shadow:var(--gc-shadow);transition:all .25s;"
                            ...
                            <div style="padding:20px;">
                                <div style="display:flex;align-items:center;gap:12px;font-size:11px;color:var(--gc-muted);margin-bottom:10px;font-style:italic;">
                                    <span><i class="mdi mdi-calendar-outline" style="font-style:normal;"></i> {{ $post->date() }}</span>
                                    @if($post->author())
                                        <span><i class="mdi mdi-account-outline" style="font-style:normal;"></i> {{ $post->author() }}</span>
                                    @endif
                                </div>
                                <h3 style="font-size:17px;font-weight:400;color:var(--gc-dark);margin-bottom:8px;line-height:1.4;font-style:italic;">
                ...
                @empty
                    <div class="col-12">
                        <div style="text-align:center;padding:60px 20px;background:var(--gc-warm);border-radius:var(--gc-radius);border:1px dashed var(--gc-border);">
                            <i class="las la-gem" style="font-size:48px;display:block;margin-bottom:16px;"></i>
                            <p style="color:var(--gc-muted);font-style:italic;">{{ __('No Stories Available Yet') }}</p>
                        </div>
                    </div>
                @endforelse
            </div>
        ...
        <div class="col-lg-4">
                ...
                <form action="{{ theme_blog_search_url() }}" method="GET">
                    <div style="display:flex;border:1.5px solid var(--gc-border);border-radius:var(--gc-radius);overflow:hidden;">
                        <input type="text" name="search" placeholder="{{ __('Search stories…') }}"
                               style="flex:1;padding:9px 14px;border:0;font-size:13px;font-family:Georgia,serif;outline:none;font-style:italic;">
            ...
            <div style="background:#fff;border:1px solid var(--gc-border);border-radius:var(--gc-radius);padding:24px;margin-bottom:24px;box-shadow:var(--gc-shadow);">
                ...
                <ul style="list-style:none;margin:0;padding:0;">
                    ...
                    <li style="border-bottom:1px dashed var(--gc-border);padding:8px 0;">
                        <a href="{{ $cat->url() }}" style="display:flex;align-items:center;justify-content:space-between;font-size:13px;color:var(--gc-dark);text-decoration:none;font-style:italic;"
                           onmouseover="this.style.color='var(--gc-rose)'" onmouseout="this.style.color='var(--gc-dark)'">
                            <span>{{ $cat->name() }}</span>
                            <span style="background:var(--gc-warm);color:var(--gc-rose);font-size:10px;font-weight:700;padding:2px 10px;border-radius:3px;font-style:normal;">{{ $cat->count() }}</span>
                        </a>
                    </li>
                    @endforeach
                </ul>
            ...
            <div style="background:#fff;border:1px solid var(--gc-border);border-radius:var(--gc-radius);padding:24px;box-shadow:var(--gc-shadow);">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:8px;">
                    ...
                    <a href="{{ $tag->url() }}"
                       style="display:inline-block;padding:5px 14px;background:var(--gc-warm);border:1.5px solid var(--gc-border);border-radius:3px;font-size:11px;font-weight:700;color:var(--gc-muted);text-decoration:none;font-style:italic;transition:all .2s;"
