...
{
    protected function getWidgetType(): string { return 'electro_brand_logos'; }
    protected function getWidgetName(): string { return 'Electro: Brand Logos'; }
    ...
    protected function getWidgetTags(): array { return ['brand', 'logos', 'electro']; }
    ...
    public function render(array $settings = []): string
    {
        if (!function_exists('tenant') || !tenant()) {
            return '<div style="padding:40px;text-align:center;background:#f9f9f9;border:1px dashed #ccc;border-radius:8px;color:#888;font-family:sans-serif;"><p style="margin:0;font-size:14px;">Electro: Brand Logos — preview on the live page</p></div>';
        }
        ...
        try {
            if (class_exists(\Modules\Attributes\Entities\Brand::class)) {
                $brands = \Modules\Attributes\Entities\Brand::take($limit)->get();
            }
        } catch (\Exception $e) {
            $brands = collect();
        }
        ...
        return view('theme-electro::widgets.brand_logos', [
            'slides'         => $slides,
