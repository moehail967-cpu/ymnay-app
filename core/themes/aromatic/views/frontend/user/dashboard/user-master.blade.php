...
<div class="ar-dash-section">
    <div class="container">

        <button class="ar-dash-mobile-toggle" id="ar-dash-toggle">
            <i class="mdi mdi-menu"></i> {{ __('Account Menu') }}
        </button>

        <div class="ar-dash-wrap">

            {{-- Sidebar --}}
            <div class="ar-dash-sidebar" id="ar-dash-sidebar">
                @php $u = auth('web')->user(); @endphp
                <div class="ar-dash-user">
                    <div class="ar-dash-avatar">{{ strtoupper(substr($u?->name ?? 'U', 0, 1)) }}</div>
                    <div class="ar-dash-user-info">
                        <div class="ar-dash-name">{{ $u?->name }}</div>
                        <div class="ar-dash-email">{{ $u?->email }}</div>
                    </div>
                </div>
                <ul class="ar-dash-nav">
...
<x-media-upload.js/>
<script>
$('#ar-dash-toggle').on('click', function(){
    $('#ar-dash-sidebar').toggleClass('open');
});
</script>
