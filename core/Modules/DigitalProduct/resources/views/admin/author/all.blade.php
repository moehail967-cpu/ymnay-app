...
@section('title') {{__('Digital Product Author')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                <i class="mdi mdi-account-edit-outline text-primary text-base"></i>
            </div>
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Digital Product All Author')}}</h3>
                <p class="text-xs text-muted">{{__('Manage product authors')}}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            ...
            @can('digital-author-create')
                <button type="button" onclick="openModal('author_create_modal')"
                    ...
                    <i class="mdi mdi-plus-circle-outline text-base"></i> {{__('New Author')}}
                </button>
            @endcan
        </div>
    </div>
    ...
    <div class="tw-table-wrap">
        <table class="w-full text-left" id="authorTable">
            <thead>
                    ...
                    <td class="px-4 py-3.5">
                        <div class="flex items-center justify-end gap-1.5">
                            @can('digital-author-edit')
                                ...
                                @endphp
                                <button type="button" class="tw-btn-icon tw-btn-icon-edit author_edit_btn"
                                        title="{{__('Edit')}}"
                                        data-id="{{$author->id}}"
                            ...
                            @can('digital-author-delete')
                                <button type="button" class="tw-btn-icon tw-btn-icon-danger swal_delete_button"
                                        data-route="{{ route('tenant.admin.digital.product.author.delete', $author->id) }}"
                                        title="{{__('Delete')}}">
...
{{-- Create Modal --}}
@can('digital-author-create')
<div id="author_create_modal" class="fixed inset-0 z-[800] hidden">
    <div class="fixed inset-0 bg-black/40 backdrop-blur-sm" onclick="closeModal('author_create_modal')"></div>
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            <div class="flex items-center gap-3 px-5 py-4 border-b border-main flex-shrink-0">
                ...
                <div class="flex-1">
                    <h5 class="text-sm font-bold text-dark font-urbanist">{{__('Create Author')}}</h5>
                    <p class="text-[11px] text-muted">{{__('Add a new author')}}</p>
                </div>
                <button type="button" onclick="closeModal('author_create_modal')" class="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-secondary transition">
                    <i class="mdi mdi-close text-muted"></i>
                </button>
            </div>
            <form action="{{ route('tenant.admin.digital.product.author.new') }}" method="post" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                @csrf
                ...
                <div class="flex items-center justify-end gap-2 px-5 py-4 border-t border-main flex-shrink-0" style="background: var(--color-bg-secondary);">
                    <button type="button" onclick="closeModal('author_create_modal')" class="px-4 py-2 rounded-xl text-sm font-semibold text-dark bg-secondary border border-main hover:border-hover transition">{{__('Cancel')}}</button>
...
{{-- Edit Modal --}}
@can('digital-author-edit')
<div id="author_edit_modal" class="fixed inset-0 z-[800] hidden">
    <div class="fixed inset-0 bg-black/40 backdrop-blur-sm" onclick="closeModal('author_edit_modal')"></div>
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            <div class="flex items-center gap-3 px-5 py-4 border-b border-main flex-shrink-0">
                <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1">
                    <h5 class="text-sm font-bold text-dark font-urbanist">{{__('Update Author')}}</h5>
                    <p class="text-[11px] text-muted">{{__('Edit author details')}}</p>
                </div>
                <button type="button" onclick="closeModal('author_edit_modal')" class="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-secondary transition">
                    <i class="mdi mdi-close text-muted"></i>
                </button>
            </div>
            <form action="{{ route('tenant.admin.digital.product.author.update') }}" method="post" class="flex flex-col flex-1 min-h-0">
                @csrf
                ...
                <div class="flex items-center justify-end gap-2 px-5 py-4 border-t border-main flex-shrink-0" style="background: var(--color-bg-secondary);">
                    <button type="button" onclick="closeModal('author_edit_modal')" class="px-4 py-2 rounded-xl text-sm font-semibold text-dark bg-secondary border border-main hover:border-hover transition">{{__('Cancel')}}</button>
                    <button type="submit" class="inline-flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold text-white bg-primary hover:opacity-90 transition">
...
@section('scripts')
    ...
    @can('digital-author-delete')
        <x-bulk-action-js :url="route('tenant.admin.digital.product.author.bulk.action')"/>
    @endcan
    ...
    (function ($) {
        ...
        $(document).ready(function () {
            if ($.fn.DataTable && !$.fn.dataTable.isDataTable('#authorTable')) {
            ...
            // Edit button
            $(document).on('click', '.author_edit_btn', function () {
                var el = $(this);
                var modal = $('#author_edit_modal');

                modal.find('#edit_author_id').val(el.data('id'));
