...
<div style="background:var(--tr-bark);padding:24px 0;border-bottom:2px solid var(--tr-olive);">
    <div class="container">
        <h1 style="font-size:26px;font-weight:900;color:#fff;margin-bottom:6px;">{{ $category_name ?? __('Blog Category') }}</h1>
        ...
        <div class="col-lg-8">
            ...
            @if($posts && $posts->count())
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <article style="background:#fff;border:1px solid var(--tr-border);border-radius:var(--tr-radius);overflow:hidden;box-shadow:var(--tr-shadow);height:100%;display:flex;flex-direction:column;">
                            @if($post->has_image())
                            <a href="{{ $post->url() }}" style="display:block;overflow:hidden;aspect-ratio:16/9;">
                                <img src="{{ $post->image_url() }}" alt="{{ $post->title() }}" style="width:100%;height:100%;object-fit:cover;" loading="lazy">
                            </a>
                            @endif
                ...
                <div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:48px;flex-wrap:wrap;">
                    @if($paginator->onFirstPage())
                        <span style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:var(--tr-radius);border:1.5px solid var(--tr-border);color:var(--tr-stone);opacity:.4;"><i class="mdi mdi-chevron-left"></i></span>
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:var(--tr-radius);border:1.5px solid var(--tr-border);color:var(--tr-bark);text-decoration:none;"><i class="mdi mdi-chevron-left"></i></a>
                    @endif
                    @foreach($paginator->getUrlRange(1,$paginator->lastPage()) as $page => $url)
                        ...
                        @else
                            <a href="{{ $url }}" style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:var(--tr-radius);border:1.5px solid var(--tr-border);color:var(--tr-bark);font-size:13px;font-weight:600;text-decoration:none;">{{ $page }}</a>
                        @endif
                    @endforeach
                    @if($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:var(--tr-radius);border:1.5px solid var(--tr-border);color:var(--tr-bark);text-decoration:none;"><i class="mdi mdi-chevron-right"></i></a>
                    @else
                        <span style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:var(--tr-radius);border:1.5px solid var(--tr-border);color:var(--tr-stone);opacity:.4;"><i class="mdi mdi-chevron-right"></i></span>
                    @endif
                </div>
                @endif
            @else
                <div style="text-align:center;padding:80px 0;color:var(--tr-stone);">
                    ...
                    <p style="font-size:16px;">{{ __('No posts found in this category.') }}</p>
                </div>
            @endif
        </div>
        ...
        <div class="col-lg-4">
            ...
            <div style="background:#fff;border:1px solid var(--tr-border);border-radius:var(--tr-radius);padding:24px;margin-bottom:28px;">
                ...
                <ul style="list-style:none;padding:0;margin:0;">
                    ...
                    <li style="border-bottom:1px solid var(--tr-border);">
                        <a href="{{ $cat->url() }}" style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;font-size:13px;color:{{ ($category_name ?? '') === $cat->name() ? 'var(--tr-olive)' : 'var(--tr-bark)' }};font-weight:{{ ($category_name ?? '') === $cat->name() ? '700' : '400' }};text-decoration:none;">
            ...
            <div style="background:#fff;border:1px solid var(--tr-border);border-radius:var(--tr-radius);padding:24px;">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:8px;">
                    @foreach($tags as $tag)
                    <a href="{{ $tag->url() }}" style="background:var(--tr-cream);color:var(--tr-stone);font-size:12px;font-weight:600;padding:5px 12px;border-radius:4px;text-decoration:none;border:1px solid var(--tr-border);">
                        {{ $tag->name() }}
                    </a>
                    @endforeach
                </div>
