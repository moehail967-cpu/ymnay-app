...
<section style="padding:48px 0;background:var(--ch-cream);">
    <div class="container">
        ...
        <button class="ch-dash-mobile-toggle" id="ch-dash-toggle">
            <i class="las la-bars"></i> {{ __('Dashboard Menu') }}
        </button>

        <div class="ch-dash-wrap gap-4">

            {{-- Sidebar --}}
            <div class="ch-dash-sidebar" id="ch-dash-sidebar">
                @php $u = auth('web')->user(); @endphp
                <div class="ch-dash-user-info">
                    <div class="ch-dash-avatar">{{ strtoupper(substr($u?->name ?? 'U', 0, 1)) }}</div>
                    <div style="min-width:0;">
                        <div class="ch-dash-username">{{ $u?->name }}</div>
                        <div class="ch-dash-email">{{ $u?->email }}</div>
                    </div>
                </div>
                <ul class="ch-dash-nav">
...
<x-media-upload.js/>
<script>
$('#ch-dash-toggle').on('click', function(){
    $('#ch-dash-sidebar').toggleClass('open');
});
</script>
