...
@section('title') {{__('Mobile Sliders (Type 3)')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main mb-6">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Mobile Sliders (Type 3)')}}</h3>
                <p class="text-xs text-muted">{{__('Manage type 3 mobile app sliders')}}</p>
            </div>
        </div>
    ...
    <div class="tw-table-wrap">
        <table class="w-full text-left" id="sliderThreeTable">
            <thead>
                ...
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5">
                        <div class="flex items-center justify-end gap-1.5">
                            @can('state-edit')
                            <a href="{{ route('admin.mobile.slider.three.edit', $slider->id) }}"
                            ...
                            <button type="button"
                                    ...
                                    data-url="{{ route('admin.mobile.slider.three.delete', $slider->id) }}"
                                    title="{{__('Delete')}}">
...
@section('scripts')
    ...
    (function ($) {
        ...
        $(document).ready(function () {
            if ($.fn.DataTable && !$.fn.dataTable.isDataTable('#sliderThreeTable')) {
                $('#sliderThreeTable').DataTable({
                    "order": [[0, "asc"]],
                    "pageLength": 10,
