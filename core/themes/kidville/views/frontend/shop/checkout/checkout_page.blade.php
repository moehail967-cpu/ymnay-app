...
<div class="container" style="padding-top:28px;">
    <div class="d-flex align-items-center justify-content-center gap-0 mb-4">
        <div class="text-center" style="flex:1;">
            <div style="width:36px;height:36px;border-radius:50%;background:var(--kv-green);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:14px;margin:0 auto 6px;">
                <i class="las la-check" style="font-size:18px;"></i>
            </div>
            <div style="font-size:12px;font-weight:900;color:var(--kv-green);">{{ __('Cart') }}</div>
        </div>
        <div style="flex:2;height:3px;background:var(--kv-green);margin-bottom:20px;"></div>
        <div class="text-center" style="flex:1;">
            <div style="width:36px;height:36px;border-radius:50%;background:var(--kv-red);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:14px;margin:0 auto 6px;">2</div>
            <div style="font-size:12px;font-weight:900;color:var(--kv-red);">{{ __('Checkout') }}</div>
        </div>
    ...
    <div class="row g-4" style="padding-bottom:72px;">
        ...
        <div class="col-lg-7">
            ...
            <div class="kv-section-box">
                <div class="kv-section-box-title"><i class="las la-user"></i> {{ __('Sign In to Continue') }}</div>
                <x-flash-msg/>
                <form id="login_form_order_page">
                    <div class="error-wrap mb-3"></div>
                    <div class="mb-3">
                        <label class="kv-label">{{ __('Username') }}</label>
                        <input type="text" name="username" class="kv-price-input" placeholder="{{ __('Type your username') }}">
                    </div>
                    <div class="mb-3">
                        <label class="kv-label">{{ __('Password') }}</label>
                        <input type="password" name="password" class="kv-price-input" placeholder="{{ __('Password') }}">
                    </div>
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <label class="d-flex align-items-center gap-2" style="font-size:13px;cursor:pointer;">
                            <input type="checkbox" name="remember"> {{ __('Remember me') }}
                        </label>
                        <a href="{{ theme_forget_password_url() }}" style="font-size:13px;color:var(--kv-red);">{{ __('Forgot Password?') }}</a>
                    </div>
                    <button type="submit" id="login_btn" class="kv-btn kv-btn-red">{{ __('Sign In') }}</button>
                </form>
            </div>
            @endif
            ...
            <form action="{{ theme_checkout_url() }}" method="POST" class="checkout-form" enctype="multipart/form-data">
                ...
                <div class="kv-section-box">
                    <div class="kv-section-box-title"><i class="las la-map-marker"></i> {{ __('Delivery Address') }}</div>
                    <div class="row g-3">
                        <div class="col-12">
                            <label class="kv-label">{{ __('Full Name') }}</label>
                            <input type="text" name="name" class="kv-price-input"
                                   placeholder="{{ __('Type Full Name') }}"
                                   value="@auth('web'){{ $billing_info ? $billing_info->full_name : auth('web')->user()?->name }}@else{{ old('name') }}@endauth"
                                   {{ $readonly }}>
                        </div>
                        <div class="col-md-6">
                            <label class="kv-label">{{ __('Mobile Number') }}</label>
                            <input type="tel" name="phone" class="kv-price-input"
                        ...
                        <div class="col-md-6">
                            <label class="kv-label">{{ __('Email Address') }}</label>
                            <input type="email" name="email" class="kv-price-input"
                                   placeholder="{{ __('Type Email') }}"
                                   value="@auth('web'){{ $billing_info ? $billing_info->email : auth('web')->user()?->email }}@else{{ old('email') }}@endauth"
                                   {{ $readonly }}>
                        </div>
                        <div class="col-md-6">
                            <label class="kv-label">{{ __('ZIP / PIN Code') }} <span style="color:var(--kv-red);">*</span></label>
                            <input type="text" name="postal_code" class="kv-price-input" maxlength="6"
                        ...
                        <div class="col-md-6">
                            <label class="kv-label">{{ __('Country') }}</label>
                            <select name="country" class="kv-price-input billing_address_country" id="country">
                        ...
                        <div class="col-md-6">
                            <label class="kv-label">{{ __('State') }}</label>
                            <div class="position-relative">
                                <input type="text" class="kv-price-input stateField live-state-input" name="state" id="state"
                                       value="{{ old('state', $billing_info->state->name ?? '') }}"
                                       placeholder="{{ __('Type state…') }}" {{ $readonly }}>
                                <div class="live-dropdown state-dropdown"></div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="kv-label">{{ __('City / Town') }}</label>
                            <div class="position-relative">
                                <input type="text" class="kv-price-input live-city-input" name="city"
                                       value="{{ old('city', $billing_info->city ?? '') }}"
                                       placeholder="{{ __('Type city…') }}" {{ $readonly }}>
                                <div class="live-dropdown city-dropdown"></div>
                            </div>
                        </div>
                        <div class="col-12">
                            <label class="kv-label">{{ __('Address') }}</label>
                            <textarea name="address" class="kv-price-input" rows="3"
                        ...
                        <div class="col-12">
                            <label class="kv-label">{{ __('Order Notes') }} <span style="color:var(--kv-muted);font-weight:600;">({{ __('optional') }})</span></label>
                            <textarea name="message" class="kv-price-input" rows="2"
                                      placeholder="{{ __('e.g. Leave at door…') }}"
                                      style="height:auto;resize:vertical;">{{ old('message') }}</textarea>
                        </div>
                    </div>

                    @if(!theme_is_logged_in())
                    <div class="mt-4" style="border-top:2px dashed var(--kv-border);padding-top:16px;">
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;font-size:14px;font-weight:800;color:var(--kv-dark);">
                            <input type="checkbox" id="kv_create_account_check" style="accent-color:var(--kv-red);width:16px;height:16px;cursor:pointer;">
                            <i class="las la-user-plus" style="font-size:18px;color:var(--kv-red);"></i>
                            {{ __('Create an Account') }}
                        </label>
                        <input type="hidden" name="create_accounts_input" class="create_accounts_input">
                        <div class="checkout-form-open mt-3">
                            <div style="background:var(--kv-light);border:2px solid var(--kv-border);border-radius:var(--kv-radius);padding:18px;">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <label class="kv-label">{{ __('Username') }}</label>
                                        <input type="text" name="create_username" class="kv-price-input" placeholder="{{ __('Type a unique username') }}">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="kv-label">{{ __('Password') }}</label>
                                        <input type="password" name="create_password" class="kv-price-input" placeholder="{{ __('Type a strong password') }}">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="kv-label">{{ __('Confirm Password') }}</label>
                                        <input type="password" name="create_password_confirmation" class="kv-price-input" placeholder="{{ __('Confirm password') }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="kv-section-box">
                    <div class="kv-section-box-title"><i class="las la-credit-card"></i> {{ __('Payment Method') }}</div>
                    <div class="kv-checkout-payment-wrap">
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <label class="d-flex align-items-center gap-2 kv-checkout-cod" style="font-size:14px;cursor:pointer;font-weight:700;">
                                <input type="checkbox" id="cash" style="accent-color:var(--kv-red);width:16px;height:16px;">
                                <i class="las la-money-bill" style="font-size:20px;color:var(--kv-green);"></i>
                                {{ __('Cash On Delivery') }}
                            </label>
                        </div>
                        ...
                        <div class="form-group d-none manual_transaction_id mt-3">
                            <label class="kv-label">{{ __('Transaction ID') }}</label>
                            <input type="text" name="trasaction_id" class="kv-price-input" placeholder="{{ __('Transaction ID') }}">
                        </div>
                        <div class="summernot_wrap" style="display:none;">
                            <div class="manual_description mt-2" style="font-size:13px;color:var(--kv-muted);"></div>
                        </div>
        ...
        <div class="col-lg-5">
            <div class="kv-order-summary">
                <div class="kv-order-summary-title">{{ __('Your Order') }}</div>
                ...
                <div class="mb-3">
                    <div class="d-flex gap-2">
                        ...
                        <input type="text" class="kv-price-input flex-grow-1 coupon-code"
                               placeholder="{{ __('Coupon Code') }}" name="used_coupon_display">
                        <button type="button" class="kv-btn kv-btn-red kv-btn-sm coupon-btn" style="white-space:nowrap;">
                            {{ __('Apply') }}
                        </button>
                    </div>
                </div>
                ...
                <div class="kv-mini-cart-row">
                    <div class="kv-mini-cart-img">
                        {!! render_image_markup_by_attachment_id($data?->options?->image) !!}
                    </div>
                    <div style="flex:1;min-width:0;">
                        <div style="font-size:13px;font-weight:800;color:var(--kv-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                            {{ $data->name }} ×{{ $data->qty }}
                        </div>
                        @if($data?->options?->color_name || $data?->options?->size_name || !empty($data?->options?->attributes))
                        <div style="font-size:11px;color:var(--kv-muted);">
                    ...
                    <span class="kv-price" style="font-size:13px;white-space:nowrap;">
                ...
                <div class="shipping_method_wrapper">
                    ...
                    <div class="kv-summary-row mt-2">
                        <span>{{ __('Subtotal') }}</span>
                        <span class="kv-summary-price">{{ site_currency_symbol() }}{{ Cart::subtotal() }}</span>
                    </div>
                    <div class="kv-summary-row coupon-price">
                        <span>{{ __('Coupon Discount (-)') }}</span>
                        <span class="kv-summary-price" style="color:var(--kv-green);">{{ amount_with_currency_symbol(0.00) }}</span>
                    </div>
                    <div class="kv-summary-row">
                        <span>{{ __('Tax (Incl)') }}</span>
                        <span class="kv-summary-price">{{ $product_tax }}%</span>
                    </div>

                    @if(theme_is_logged_in() && !empty($shipping_methods ?? []))
                    <div style="margin:10px 0 4px;font-size:12px;font-weight:900;color:var(--kv-dark);">{{ __('Shipping') }}</div>
                    @foreach($shipping_methods ?? [] as $key => $method)
                    <div class="kv-summary-row" data-country="{{ $country ?? '' }}" data-state="{{ $state ?? '' }}">
                        <label class="d-flex align-items-center gap-2" style="cursor:pointer;font-size:13px;margin:0;font-weight:700;">
                        ...
                        <span class="kv-summary-price">{{ amount_with_currency_symbol($method['options']['cost']) }}</span>
                    </div>
                    @endforeach
                    @endif

                    <div class="kv-summary-row price-shipping">
                        <span>{{ __('Shipping Cost (+)') }}</span>
                        <span class="kv-summary-price">
                            {{ isset($default_shipping) ? amount_with_currency_symbol($default_shipping['options']['cost']) : '--' }}
                        </span>
                    </div>

                    <div class="kv-summary-row total price-total" data-total="{{ $subtotal + $taxed_price }}">
                ...
                <div class="mt-3 d-flex align-items-center gap-2">
                    <input type="checkbox" id="agree" checked style="accent-color:var(--kv-red);width:15px;height:15px;">
                    <label for="agree" style="font-size:12px;color:var(--kv-muted);cursor:pointer;font-weight:700;">
                        {{ __('I agree to the') }}
                        <a href="javascript:void(0)" style="color:var(--kv-red);">{{ __('Terms & Conditions') }}</a>
                    </label>
                </div>

                <button type="button" class="kv-btn kv-btn-red w-100 justify-content-center proceed_checkout_btn checkout_disable mt-3"
                        style="font-size:15px;padding:14px;">
                    <i class="las la-lock"></i> {{ __('Place Order') }}
                </button>

                <a href="{{ theme_cart_url() }}" class="kv-btn kv-btn-outline w-100 justify-content-center mt-2">
                    {{ __('Return to Cart') }}
                </a>
...
(function ($) {
    $(document).ready(function () {
        ...
        $('#kv_create_account_check').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.type === 'success') {
                        ...
                        $('.price-shipping .kv-summary-price').html(c + data.selected_shipping_method.options.cost);
                        $('.price-total .kv-summary-price').html(c + data.total);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.type === 'error') { toastr.error(data.msg); }
                        ...
                        $('.price-total .kv-summary-price').text(c + data.coupon_amount);
                        $('.coupon-price .kv-summary-price').text(c + data.coupon_price);
