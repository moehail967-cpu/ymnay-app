...
@section('style')
<style>
/* Override shop-common.css form--control inside checkout */
.checkout-form .single-input { position:relative; margin-top:0 !important; margin-bottom:0 !important; }
.checkout-form .single-input .label-title { font-size:13px; font-weight:700; color:var(--ph-dark); display:block; margin-bottom:6px; }
.checkout-form .single-input .form--control { width:100%; padding:11px 14px !important; border:2px solid var(--ph-border) !important; border-radius:var(--ph-radius) !important; font-size:14px; font-family:var(--ph-font); color:var(--ph-dark) !important; background:var(--ph-cream) !important; outline:none; height:auto !important; box-shadow:none !important; transition:border-color .2s; box-sizing:border-box; }
.checkout-form .single-input .form--control:focus { border-color:var(--ph-terra) !important; box-shadow:none !important; }
.live-dropdown { position:absolute; left:0; right:0; background:#fff; border:2px solid var(--ph-border); border-top:0; border-radius:0 0 var(--ph-radius) var(--ph-radius); max-height:220px; overflow-y:auto; z-index:999; display:none; box-shadow:var(--ph-shadow-sm); }
.live-dropdown:empty { display:none !important; }
</style>
@endsection
...
<div class="container" style="padding-top:28px;">
    <div class="d-flex align-items-center justify-content-center gap-0 mb-4">
        <div class="text-center" style="flex:1;">
            <div style="width:36px;height:36px;border-radius:50%;background:var(--ph-sage);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;margin:0 auto 6px;">
                <i class="las la-check" style="font-size:18px;"></i>
            </div>
            <div style="font-size:12px;font-weight:700;color:var(--ph-sage);">{{ __('Cart') }}</div>
        </div>
        <div style="flex:2;height:2px;background:var(--ph-sage);margin-bottom:20px;"></div>
        <div class="text-center" style="flex:1;">
            <div style="width:36px;height:36px;border-radius:50%;background:var(--ph-terra);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;margin:0 auto 6px;">2</div>
            <div style="font-size:12px;font-weight:700;color:var(--ph-terra);">{{ __('Checkout') }}</div>
        </div>
    ...
    <div class="row g-4" style="padding-bottom:72px;">
        ...
        <div class="col-lg-7">
            ...
            <div class="ph-section-box">
                <div class="ph-section-box-title"><i class="las la-user"></i> {{ __('Sign In to Continue') }}</div>
                <x-flash-msg/>
                <form id="login_form_order_page">
                    <div class="error-wrap mb-3"></div>
                    <div class="mb-3">
                        <label class="ph-label">{{ __('Username') }}</label>
                        <input type="text" name="username" class="ph-input" placeholder="{{ __('Type your username') }}">
                    </div>
                    <div class="mb-3">
                        <label class="ph-label">{{ __('Password') }}</label>
                        <input type="password" name="password" class="ph-input" placeholder="{{ __('Password') }}">
                    </div>
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <label class="d-flex align-items-center gap-2" style="font-size:13px;cursor:pointer;">
                            <input type="checkbox" name="remember"> {{ __('Remember me') }}
                        </label>
                        <a href="{{ theme_forget_password_url() }}" style="font-size:13px;color:var(--ph-terra);">{{ __('Forgot Password?') }}</a>
                    </div>
                    <button type="submit" id="login_btn" class="ph-btn ph-btn-terra">{{ __('Sign In') }}</button>
                    <p class="mt-3" style="font-size:13px;color:var(--ph-muted);">
                        {{ __("Don't have an account?") }}
                        <a href="{{ theme_register_url() }}" style="color:var(--ph-terra);">{{ __('Sign up') }}</a>
                    </p>
                </form>
            </div>
            @endif
            ...
            <form action="{{ theme_checkout_url() }}" method="POST" class="checkout-form" enctype="multipart/form-data">
                ...
                <div class="ph-section-box">
                    <div class="ph-section-box-title"><i class="las la-map-marker"></i> {{ __('Delivery Address') }}</div>
                    <div class="row g-3">
                        <div class="col-12">
                            <label class="ph-label">{{ __('Full Name') }}</label>
                            <input type="text" name="name" class="ph-input"
                                   placeholder="{{ __('Type Full Name') }}"
                                   value="@auth('web'){{ $billing_info ? $billing_info->full_name : auth('web')->user()?->name }}@else{{ old('name') }}@endauth"
                                   {{ $readonly }}>
                        </div>
                        <div class="col-md-6">
                            <label class="ph-label">{{ __('Mobile Number') }}</label>
                            <input type="tel" name="phone" class="ph-input"
                        ...
                        <div class="col-md-6">
                            <label class="ph-label">{{ __('Email Address') }}</label>
                            <input type="email" name="email" class="ph-input"
                                   placeholder="{{ __('Type Email') }}"
                                   value="@auth('web'){{ $billing_info ? $billing_info->email : auth('web')->user()?->email }}@else{{ old('email') }}@endauth"
                                   {{ $readonly }}>
                        </div>
                        <div class="col-md-6">
                            <label class="ph-label">{{ __('ZIP / PIN Code') }} <span style="color:var(--ph-terra);">*</span></label>
                            <input type="text" name="postal_code" class="ph-input" maxlength="6"
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="country" class="ph-input billing_address_country" id="country">
                        ...
                        <div class="col-md-6" style="position:relative;">
                            ...
                            <input type="text" name="state" id="state"
                                   class="ph-input live-state-input stateField"
                        ...
                        <div class="col-md-6" style="position:relative;">
                            ...
                            <input type="text" name="city" id="city"
                                   class="ph-input live-city-input cityField"
                                   placeholder="{{ __('City') }}"
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="address" class="ph-input" rows="3"
                        ...
                        <div class="col-12">
                            <label class="ph-label">{{ __('Order Notes') }} <span style="color:var(--ph-muted);font-weight:400;">({{ __('optional') }})</span></label>
                            <textarea name="message" class="ph-input" rows="2"
                    ...
                    <div class="mt-4">
                        <a href="javascript:void(0)" class="create-accounts" style="color:var(--ph-terra);font-size:13px;font-weight:700;">
                        ...
                        <div class="checkout-form-open mt-3">
                            <div class="row g-3">
                                <div class="col-12">
                                    <label class="ph-label">{{ __('Username') }}</label>
                                    <input type="text" name="create_username" class="ph-input" placeholder="{{ __('Type a unique username') }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="ph-label">{{ __('Password') }}</label>
                                    <input type="password" name="create_password" class="ph-input" placeholder="{{ __('Type a strong password') }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="ph-label">{{ __('Confirm Password') }}</label>
                                    <input type="password" name="create_password_confirmation" class="ph-input" placeholder="{{ __('Confirm password') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="ph-section-box">
                    <div class="ph-section-box-title"><i class="las la-credit-card"></i> {{ __('Payment Method') }}</div>

                    <div>
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <label class="d-flex align-items-center gap-2" style="font-size:14px;cursor:pointer;">
                                <input type="checkbox" id="cash" style="accent-color:var(--ph-terra);width:16px;height:16px;">
                                <i class="las la-money-bill" style="font-size:20px;color:var(--ph-sage);"></i>
                                {{ __('Cash On Delivery') }}
                            </label>
                        </div>
                        ...
                        <div class="form-group d-none manual_transaction_id mt-3">
                            <label class="ph-label">{{ __('Transaction ID') }}</label>
                            <input type="text" name="trasaction_id" class="ph-input" placeholder="{{ __('Transaction ID') }}">
                        </div>
                        <div class="summernot_wrap" style="display:none;">
                            <div class="manual_description mt-2" style="font-size:13px;color:var(--ph-muted);"></div>
                        </div>
        ...
        <div class="col-lg-5">
            <div class="ph-order-summary">
                <div class="ph-order-summary-title">{{ __('Your Order') }}</div>
                ...
                <div class="mb-3">
                    <div class="d-flex gap-2">
                        ...
                        <input type="text" class="ph-input flex-grow-1 coupon-code"
                               placeholder="{{ __('Coupon Code') }}" name="used_coupon_display">
                        <button type="button" class="ph-btn ph-btn-terra ph-btn-sm coupon-btn" style="white-space:nowrap;">
                            {{ __('Apply') }}
                        </button>
                    </div>
                </div>
                ...
                <div class="ph-mini-cart-row">
                    <div class="ph-mini-cart-img">
                        {!! render_image_markup_by_attachment_id($data?->options?->image) !!}
                    </div>
                    <div style="flex:1;min-width:0;">
                        <div style="font-size:13px;font-weight:700;color:var(--ph-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                            {{ $data->name }} ×{{ $data->qty }}
                        </div>
                        @if($data?->options?->color_name || $data?->options?->size_name || !empty($data?->options?->attributes))
                        <div style="font-size:11px;color:var(--ph-muted);">
                    ...
                    <span class="ph-price" style="font-size:13px;white-space:nowrap;">
                ...
                <div class="shipping_method_wrapper">
                    ...
                    <div class="ph-summary-row mt-2">
                        <span>{{ __('Subtotal') }}</span>
                        <span class="ph-summary-price">{{ site_currency_symbol() }}{{ Cart::subtotal() }}</span>
                    </div>
                    <div class="ph-summary-row coupon-price">
                        <span>{{ __('Coupon Discount (-)') }}</span>
                        <span class="ph-summary-price" style="color:var(--ph-sage);">{{ amount_with_currency_symbol(0.00) }}</span>
                    </div>
                    <div class="ph-summary-row">
                        <span>{{ __('Tax (Incl)') }}</span>
                        <span class="ph-summary-price">{{ $product_tax }}%</span>
                    </div>

                    @if(theme_is_logged_in() && !empty($shipping_methods ?? []))
                    <div style="margin:10px 0 4px;font-size:12px;font-weight:700;color:var(--ph-dark);">{{ __('Shipping') }}</div>
                    @foreach($shipping_methods ?? [] as $key => $method)
                    <div class="ph-summary-row" data-country="{{ $country ?? '' }}" data-state="{{ $state ?? '' }}">
                        <label class="d-flex align-items-center gap-2" style="cursor:pointer;font-size:13px;margin:0;">
                            <input type="radio" class="shipping_methods"
                                   ...
                                   value="{{ $method['id'] }}"
                                   style="accent-color:var(--ph-terra);"
                                   {{ $key === 0 ? 'checked' : '' }}>
                            {{ $method['name'] }}
                        </label>
                        <span class="ph-summary-price">{{ amount_with_currency_symbol($method['options']['cost']) }}</span>
                    </div>
                    @endforeach
                    @endif

                    <div class="ph-summary-row price-shipping">
                        <span>{{ __('Shipping Cost (+)') }}</span>
                        <span class="ph-summary-price">
                            {{ isset($default_shipping) ? amount_with_currency_symbol($default_shipping['options']['cost']) : '--' }}
                        </span>
                    </div>

                    <div class="ph-summary-row total price-total" data-total="{{ $subtotal + $taxed_price }}">
                ...
                <div class="mt-3 d-flex align-items-center gap-2">
                    <input type="checkbox" id="agree" checked style="accent-color:var(--ph-terra);width:15px;height:15px;">
                    <label for="agree" style="font-size:12px;color:var(--ph-muted);cursor:pointer;">
                        {{ __('I agree to the') }}
                        <a href="javascript:void(0)" style="color:var(--ph-terra);">{{ __('Terms & Conditions') }}</a>
                    </label>
                </div>

                <button type="button" class="ph-btn ph-btn-terra w-100 justify-content-center proceed_checkout_btn checkout_disable mt-3"
                        style="font-size:15px;padding:14px;">
                    <i class="las la-lock"></i> {{ __('Place Order') }}
                </button>

                <a href="{{ theme_cart_url() }}" class="ph-btn ph-btn-outline w-100 justify-content-center mt-2">
                    {{ __('Return to Cart') }}
                </a>
            </div>
        </div>
...
(function ($) {
    $(document).ready(function () {
        ...
        $(document).on('click', '#login_btn', function (e) {
            ...
            $.ajax({
                ...
                error: function (data) {
                    ...
                    html += '</ul></div>'; form.find('.error-wrap').html(html); el.text('{{ __("Sign In") }}');
                }
            });
        });
            ...
            $.ajax({ url: '{{ theme_checkout_shipping_ajax_url() }}', type: 'GET',
                ...
                success: function (data) {
                    if (data.type === 'success') {
                        ...
                        $('.price-shipping .ph-summary-price').html(c + data.selected_shipping_method.options.cost);
                        $('.price-total .ph-summary-price').html(c + data.total);
        ...
        function getCountryStateBasedTotal(country, state) {
            $.ajax({ url: '{{ theme_checkout_total_ajax_url() }}', type: 'GET',
            ...
            $.ajax({ url: '{{ theme_checkout_coupon_ajax_url() }}', type: 'GET',
                ...
                success: function (data) {
                    ...
                    if (data.type === 'error') { toastr.error(data.msg); }
                        ...
                        $('.price-total .ph-summary-price').text(c + data.coupon_amount);
                        $('.coupon-price .ph-summary-price').text(c + data.coupon_price);
