...
@php
    ...
    $countTenant   = count(array_filter($plugins, fn($p) => ($p['manifest']->type ?? '') === 'tenant'));
...
@else
    ...
    <div class="flex flex-wrap items-center gap-2 mb-5" id="plugin-filter-tabs">
        ...
        <button class="pf-tab-btn" data-filter="tenant">
            <i class="mdi mdi-account-group text-xs"></i> {{__('Tenant')}} <span class="pf-tab-count">{{$countTenant}}</span>
        </button>
    </div>
        ...
        @foreach($plugins as $item)
            ...
            <div class="bg-surface rounded-xl border border-main shadow-main flex flex-col overflow-hidden plugin-card {{ ($manifest->type ?? '') === 'tenant' ? 'border-sky-200/60' : '' }}"
                ...
                <div class="px-5 pt-5 pb-3 flex items-start justify-between gap-3">
                    ...
                    @elseif($isCorePlugin)
                        {{-- Core system plugins are always active and cannot be toggled --}}
                ...
                <div class="mt-auto px-5 py-3 bg-secondary border-t border-main flex items-center justify-between gap-2">
                    ...
                    <div class="flex items-center gap-1.5">
                        {{-- Settings link (landlord/both plugins only; tenant-only and core plugins configure elsewhere) --}}
                        @if($manifest->type !== 'tenant' && !$isCorePlugin && app(\App\PluginSystem\SettingsManager::class)->hasDefinitions($manifest->id))
