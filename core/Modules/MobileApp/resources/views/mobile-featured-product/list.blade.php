...
@section('title') {{__('Featured Products')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main mb-6">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Featured Products')}}</h3>
                <p class="text-xs text-muted">{{__('Mobile app featured product listings')}}</p>
            </div>
        </div>
        <a href="{{ route('tenant.admin.featured.product.create') }}"
    ...
    <div class="tw-table-wrap">
        <table class="w-full text-left" id="featuredTable">
            <thead>
...
@section('scripts')
    ...
    (function ($) {
        ...
        $(document).ready(function () {
            if ($.fn.DataTable && !$.fn.dataTable.isDataTable('#featuredTable')) {
                $('#featuredTable').DataTable({
                    "order": [[0, "asc"]],
                    "pageLength": 10,
