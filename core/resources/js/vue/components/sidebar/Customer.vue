<template>
    <div class="dashboard_posSystem__sidebar__item">
        <div class="dashboard_posSystem__tabs">
            ...
            <div class="tab_content_item active mt-3" id="existing_customer">
                <div class="searchParent">
                    <div class="single-input">
                        <label for="findEmail" class="label_title">Find by Phone / Email</label>
                        <input type="text" v-model="search_text"
                               @keyup="handleCustomerSearch($event)"
                               @change="handleCustomerSearch($event)"
                               class="form--control keyupInput"
                               placeholder="Enter Phone or Email"
                               id="findEmail">
                    </div>
                    <div class="searchWrap scrollWrap d-none mt-2">
                            ...
                            <div v-for="customer in customers" :key="customer.id" class="searchWrap__item">
                                <div class="searchWrap__item__flex">
                                    ...
                                    <div class="searchWrap__right">
                                        <button class="searchWrap__right__loyal" @click="handleSelectCustomer(customer.id)">
                                            Select
                                        </button>
                                    </div>
                                </div>
            ...
            <div class="tab_content_item mt-3" id="walk_customer">
                <div class="searchParent">
                    <div class="walk-in-notice">
                        <i class="las la-walking"></i>
                        <p>Ordering for walk-in customer</p>
                    </div>
                </div>
            </div>
        </div>

        <div class="selected-customer-card mt-3" v-if="selectedCustomer">
            <div class="selected-customer-card__header">
                <span class="selected-customer-card__label">Selected Customer</span>
                <button class="selected-customer-card__change" @click="changeSelectedCustomer">
                    <i class="las la-exchange-alt"></i> Change
                </button>
            </div>
            <div class="searchWrap__item__flex mt-2">
                <div class="searchWrap__left">
                    <div class="searchWrap__left__flex">
                        <div class="searchWrap__left__thumb">
                ...
                <div class="searchWrap__right">
                    <p class="text-muted-sm">Member since</p>
                    <p class="text-xs-bold">{{ selectedCustomer.date }}</p>
                </div>
            </div>
        </div>
    </div>

    <div v-show="showHideAddNewCustomer()" id="addCustomerButton" class="dashboard_posSystem__sidebar__item">
        <a href="#1" class="addCustomer" @click.prevent="openAddCustomerPopup">
            <i class="las la-user-plus"></i> Add New Customer
        </a>
    </div>
</template>
...
export default {
  ...
  setup(props,{emit}){
     ...
     const openAddCustomerPopup = () => {
         // Clear the customer search overlay before opening the popup
         document.querySelector('.body-overlay')?.classList.remove('show');
         document.querySelector('#existing_customer .searchParent .searchWrap')?.classList.remove('d-block');
         document.querySelector('#existing_customer .searchParent .searchWrap')?.classList.add('d-none');

         document.querySelector('.add-customer-popup')?.classList.add('popup-active');
         document.querySelector('.add-customer-popup-overlay')?.classList.add('popup-active');
     }
