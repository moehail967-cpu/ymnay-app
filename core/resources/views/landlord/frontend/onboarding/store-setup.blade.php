...
<style>
    ...
    @media(max-width:767px){.ym-onboarding{padding:28px 16px}.ym-progress{grid-template-columns:repeat(5,1fr)}.ym-progress .ym-label{display:none}.ym-progress a,.ym-progress span{justify-content:center}.ym-grid,.ym-theme-grid,.ym-layout{grid-template-columns:1fr}.ym-card{padding:20px}.ym-title{font-size:26px}.ym-actions{flex-direction:column}.ym-btn{width:100%}.ym-summary{order:-1}}
</style>
@endsection
                    ...
                    <div class="ym-grid">
                        @foreach($plans as $item)
                            <label class="ym-option">
                                ...
                                <h2>{{$item->title}}</h2>
                                <p><strong>{{number_format((float) $item->price, 2)}} ريال سعودي</strong> / {{[0=>'شهريًا',1=>'سنويًا',2=>'مدى الحياة'][$item->type] ?? ''}}</p>
                                @if($item->has_trial)<p class="ym-success">{{$item->trial_days}} يوم تجربة مجانية</p>@endif
                            </label>
                        @endforeach
                    </div>
                    ...
                    <div class="ym-grid ym-theme-grid">
                        @foreach($themes as $theme)
                            ...
                            <label class="ym-option">
                                ...
                                <strong>{{$name}}</strong>
                                @if($preview)<a href="{{$preview}}" target="_blank" rel="noopener" onclick="event.stopPropagation()">معاينة القالب</a>@endif
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="ym-layout">
                <section class="ym-card">
                    ...
                    <form method="post" action="{{route('landlord.store.onboarding.details')}}">@csrf
                        <div class="ym-field"><label for="store_name">اسم المتجر</label><input id="store_name" name="store_name" value="{{old('store_name',$onboarding?->store_name)}}" maxlength="191" autocomplete="organization" required></div>
                        <div class="ym-field"><label for="subdomain">عنوان المتجر</label><input class="ym-ltr" id="subdomain" name="subdomain" value="{{old('subdomain',$onboarding?->subdomain)}}" minlength="3" maxlength="63" pattern="[a-z0-9](?:[a-z0-9-]*[a-z0-9])?" autocomplete="off" required><p class="ym-help ym-ltr"><span id="domain-preview">{{$onboarding?->subdomain ?: 'your-store'}}</span>.{{current(config('tenancy.central_domains'))}}</p></div>
                ...
                <section class="ym-card">
                    ...
                    @else
                        ...
                        <form id="register-panel" class="ym-panel">
                            ...
                            <div class="ym-field"><label for="reg_password">كلمة المرور</label><input class="ym-ltr" id="reg_password" type="password" autocomplete="new-password" minlength="8" required></div>
                            <div class="ym-field"><label for="reg_password_confirmation">تأكيد كلمة المرور</label><input class="ym-ltr" id="reg_password_confirmation" type="password" autocomplete="new-password" minlength="8" required></div>
                        ...
                        <form id="otp-panel" class="ym-panel" hidden>
                            <div class="ym-field"><label for="otp">رمز التحقق</label><input class="ym-ltr" id="otp" inputmode="numeric" autocomplete="one-time-code" maxlength="6" required><p class="ym-help">أرسلنا الرمز إلى <span id="otp-email" class="ym-ltr"></span></p></div>
                        ...
                        <form id="login-panel" class="ym-panel" hidden>
                            <div class="ym-field"><label for="login_email">البريد الإلكتروني</label><input class="ym-ltr" id="login_email" autocomplete="email" required></div>
                            <div class="ym-field"><label for="login_password">كلمة المرور</label><input class="ym-ltr" id="login_password" type="password" autocomplete="current-password" required></div>
                ...
                <section class="ym-card">
                    ...
                    @elseif($planChanged)
                        <div class="ym-alert ym-warning">
                            ...
                            @if(isset($onboarding->plan_snapshot['price'])) السعر السابق: {{$onboarding->plan_snapshot['price']}} — @endif السعر الحالي: {{$plan->price}} ريال سعودي.<br>
                    ...
                    @else
                        <div class="ym-alert ym-success"><strong>المطلوب الآن: ٠ ريال سعودي</strong><br>لا يلزم دفع رسوم الاشتراك لبدء التجربة المؤهلة.</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showMessage = (text, type = 'error', target = message) => { if(target) target.innerHTML = `<div class="ym-alert ym-${type}">${text}</div>`; };
    ...
    if(subdomain) subdomain.addEventListener('input', () => document.getElementById('domain-preview').textContent = subdomain.value || 'your-store');
    ...
    if(register) register.addEventListener('submit', async event => {
        ...
        } catch(error) { showMessage(Object.values(error.json?.errors || {}).flat().join('<br>') || error.json?.msg || 'تعذر إرسال رمز التحقق.'); }
    });
