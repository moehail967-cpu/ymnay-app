...
@section('content')
    <section class="el-cart-section">
        <div class="container">
            @if($cart_data->count())
                <div class="el-cart-layout">
...
@section('scripts')
    <script>
        (function ($) {
            ...
            $(document).on('click', '.el-cart-action .close-table-cart', function (e){
                let el = $(this).closest('[data-product_hash_id]');
                ...
                $.ajax({
                    ...
                    success: function (data){
                        if (data.msg) {
                            ...
                            if (data.empty_cart !== '') {
                                $('.el-cart-section .container').html(data.empty_cart).hide();
                                $('.el-cart-section .container').fadeIn();
                            }
                            $('.coupon-contents').parent().load(location.href + ' .coupon-contents');
                            $('.track-icon-list').load(location.href + ' .track-icon-list');
                            $('.el-cart-table').closest('.el-cart-table-wrap').load(location.href + ' .el-cart-table');
                        }
                        $('.loader').hide();
                    },
                    error: function (){}
            ...
            $(document).on('click', '.el-cart-action .close-table-wishlist', function (e){
                let el = $(this).closest('[data-product_hash_id]');
                let product_hash_id = el.data('product_hash_id');
                $.ajax({
            ...
            $(document).on('click', '.el-cart-action .move-to-wishlist', function (e){
                let el = $(this).closest('[data-product_hash_id]');
                let product_hash_id = el.data('product_hash_id');
                $.ajax({
