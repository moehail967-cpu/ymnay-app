<template>
    <div class="dashboard_posSystem__sidebar__item" v-if="data.sub_total > 0">
        <div class="dashboard_posSystem__sidebar__estimate">
            ...
            <div class="dashboard_posSystem__sidebar__estimate__discount">
                <input type="text" v-model="couponCode"
                       class="discountCoupon discountCouponInput"
            ...
            <div class="dashboard_posSystem__sidebar__estimate__count mt-4">
                <div class="dashboard_posSystem__sidebar__estimate__list">
                    ...
                    <div class="dashboard_posSystem__sidebar__estimate__list__item">
                        ...
                        <p class="price discount">— {{ getCurrencySymbolWithAmount(couponAmount) }}</p>
                    </div>
                    <div class="dashboard_posSystem__sidebar__estimate__list__item">
                        <p class="title">VAT <span class="tax-badge">{{ data.tax }}%</span></p>
                        <p class="price">{{ getCurrencySymbolWithAmount(taxAmount.toFixed(2)) }}</p>
                    </div>
                </div>
                <div class="dashboard_posSystem__sidebar__estimate__list total-row">
                    <div class="dashboard_posSystem__sidebar__estimate__list__item">
                        <p class="title"><strong>Total</strong></p>
                        <p class="price total"><strong>{{ getCurrencySymbolWithAmount(customSubTotal) }}</strong></p>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard_posSystem__sidebar__estimate__footer mt-4">
                <div class="dashboard_posSystem__sidebar__estimate__footer__btn btn_flex">
                    <a href="#1" class="orderBtn orderBtn_hold_order" @click.prevent="openHoldPopup" v-if="customSubTotal > 0">
                        <i class="las la-pause-circle"></i> Hold
                    </a>
                    <a href="#1" class="orderBtn orderBtn_cancel" @click.prevent="handleCancelOrder">
                        <i class="las la-times-circle"></i> Cancel
                    </a>
                    <a href="#1" class="orderBtn orderBtn_proceed" @click.prevent="openPaymentPopup">
                        <i class="las la-check-circle"></i> Proceed
                    </a>
                </div>
            </div>
        </div>
...
export default {
  ...
  setup(props,{emit}){
      ...
      function openPaymentPopup() {
          document.querySelector('.payment-popup')?.classList.add('popup-active');
          document.querySelector('.payment-popup-overlay')?.classList.add('popup-active');
      }

      function openHoldPopup() {
          document.querySelector('.hold-popup')?.classList.add('popup-active');
          document.querySelector('.hold-popup-overlay')?.classList.add('popup-active');
      }
