...
// Plugin asset route — serves Modules/{plugin}/resources/{path} files publicly.
// Must be outside the auth/tenant_glvar middleware so CSS/JS load on all pages.
Route::middleware(['web', InitializeTenancyByDomainCustomisedMiddleware::class, PreventAccessFromCentralDomains::class])
    ->get('/plugins/{plugin_id}/assets/{path}', [\App\Http\Controllers\PluginAssetController::class, 'serve'])
    ->name('tenant.plugin.asset')
    ->where('path', '.+');
...
//        Route::get('/product/{slug}', [TenantFrontendController::class, 'product_details'])->name('shop.product.details'); // Product Details
        ...
        Route::get('/wishlist/ids', [TenantFrontendController::class, 'wishlist_ids'])->name('shop.wishlist.ids');
        Route::post('/wishlist/remove-by-product', [TenantFrontendController::class, 'wishlist_remove_by_product_id'])->name('shop.wishlist.remove.by.product');
        Route::get('/products/stock-status', [TenantFrontendController::class, 'product_stock_status'])->name('shop.product.stock.status');
        Route::post('/wishlist/merge', [TenantFrontendController::class, 'wishlist_merge'])->name('shop.wishlist.merge');
        Route::post('/cart/save-for-later', [TenantFrontendController::class, 'wishlist_save_for_later'])->name('shop.cart.save.for.later');
        ...
        Route::post('theme/activate', [TenantDashboardThemeManageController::class,'activateFrontendTheme'])->name('admin.theme.activate');
        ...
        // state and city search for dropdown :
        Route::get('/checkout/search-state', [CheckoutPaymentController::class, 'searchState'])
            ->name('shop.checkout.state.search');

        Route::get('/checkout/search-city', [CheckoutPaymentController::class, 'searchCity'])
            ->name('shop.checkout.city.search');
...
Route::middleware([
    ...
    Route::middleware(['maintenance_mode','package_expire'])->controller(TenantFrontendController::class)->name('tenant.')->group(function () {
        ...
        Route::get('/state-search',  'liveState')->name('state.search');
        Route::get('/city-search', 'liveCity')->name('city.search');
    ...
    Route::middleware(['maintenance_mode','package_expire'])->controller(\App\Http\Controllers\Tenant\Frontend\TenantFrontendController::class)->group(function () {
        ...
        Route::post('/stock-notify','stock_notify_subscribe')->name('tenant.frontend.stock.notify');
