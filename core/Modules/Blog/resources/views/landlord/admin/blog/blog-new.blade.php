...
<form method="post" action="{{route(route_prefix().'admin.blog.new')}}">
    @csrf
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden">
                <div class="px-4 sm:px-6 py-4 border-b border-main flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                        <i class="mdi mdi-pencil-plus-outline text-primary text-base"></i>
                    </div>
                    <div>
                        <h3 class="text-sm font-bold text-dark font-urbanist">{{__('New Blog Post')}}</h3>
                        <p class="text-xs text-muted">{{__('Create a new article with content and SEO settings')}}</p>
                    </div>
                    <div class="ml-auto">
                ...
                <div class="p-4 sm:p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="blog_content" class="summernote" rows="4" placeholder="{{__('Blog Content')}}">{!! old('blog_content') !!}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-secondary border border-main rounded-xl px-4 py-2.5 focus-within:border-primary transition">
                            <textarea name="excerpt" rows="3" class="w-full bg-transparent text-sm text-dark placeholder-subtle outline-none border-none focus:ring-0 p-0 resize-y" placeholder="{{__('Short description...')}}">{{old('excerpt')}}</textarea>
                        </div>
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden">
                ...
                <div class="p-4 sm:p-6">
                    ...
                    <div class="meta-tab-content hidden" id="meta-facebook">
                        ...
                        <div class="mt-4">
                            <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Meta Image')}}</label>
                            <x-fields.tw-media-upload name="meta_fb_image" dimentions="1200x1200" :id="old('meta_fb_image')"/>
                        </div>
                    </div>
                    ...
                    <div class="meta-tab-content hidden" id="meta-twitter">
                        ...
                        <div class="mt-4">
                            <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Meta Image')}}</label>
                            <x-fields.tw-media-upload name="meta_tw_image" dimentions="1200x1200" :id="old('meta_tw_image')"/>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-4 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <label class="flex items-center gap-2 bg-secondary border border-main rounded-xl px-4 py-2.5 cursor-pointer flex-1 has-[:checked]:border-primary has-[:checked]:bg-primary-soft transition">
                                <input class="form-check-input post_type sr-only" type="radio" checked name="inlineRadioOptions" id="radio_general" value="option1">
                                <i class="mdi mdi-cog-outline text-lg text-primary"></i>
                                <span class="text-xs font-semibold text-dark">{{__('General')}}</span>
                            </label>
                            <label class="flex items-center gap-2 bg-secondary border border-main rounded-xl px-4 py-2.5 cursor-pointer flex-1 has-[:checked]:border-primary has-[:checked]:bg-primary-soft transition">
                                <input class="form-check-input post_type sr-only" type="radio" name="inlineRadioOptions" id="radio_video" value="option2">
                    ...
                    <div class="flex items-center justify-between bg-secondary border border-main rounded-xl px-4 py-3">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="featured" value="on" class="sr-only peer">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2.5 bg-secondary border border-main rounded-xl px-4 py-1 focus-within:border-primary transition">
                            ...
                            <select name="visibility" class="flex-1 bg-transparent text-sm text-dark outline-none border-none focus:ring-0 p-0 appearance-none cursor-pointer">
                                ...
                                <option value="logged_user" {{old('visibility') == 'logged_user' ? 'selected' : ''}}>{{__('Logged User')}}</option>
                            </select>
                            <i class="mdi mdi-chevron-down text-base text-primary pointer-events-none"></i>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2.5 bg-secondary border border-main rounded-xl px-4 py-1 focus-within:border-primary transition">
                            ...
                            <select name="status" class="flex-1 bg-transparent text-sm text-dark outline-none border-none focus:ring-0 p-0 appearance-none cursor-pointer">
                                <option value="{{\App\Enums\StatusEnums::DRAFT}}" {{old('status') == \App\Enums\StatusEnums::DRAFT ? 'selected' : ''}}>{{__("Draft")}}</option>
                                <option value="{{\App\Enums\StatusEnums::PUBLISH}}" {{old('status') == \App\Enums\StatusEnums::PUBLISH ? 'selected' : ''}}>{{__("Publish")}}</option>
                            </select>
                            <i class="mdi mdi-chevron-down text-base text-primary pointer-events-none"></i>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <x-fields.tw-media-upload name="image" dimentions="{{__('1920 X 1280 px image recommended')}}" :id="old('image')"/>
                    </div>
                    ...
                    <div>
                        ...
                        @if(old('image_gallery'))
                            <x-landlord-others.tw-edit-media-upload-gallery :name="'image_gallery'" :value="old('image_gallery')"/>
                        @else
                            <x-landlord-others.tw-media-upload-gallery-insert :name="'image_gallery'"/>
                        @endif
                    </div>
                    ...
                    <button type="submit"
                        ...
                        <i class="mdi mdi-content-save-outline text-base"></i> {{__('Submit New Post')}}
                    </button>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    $(document).ready(function () {
        ...
        $(document).on('keyup change', 'input[name="title"]', function () {
            ...
            var url = `{{url('/blog/')}}/` + removeTags(slug);
