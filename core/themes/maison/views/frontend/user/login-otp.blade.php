...
<div style="background:var(--ms-warm);border-bottom:1px solid var(--ms-border);padding:44px 0 32px;text-align:center;">
    <div class="container">
        ...
        <h1 style="font-size:clamp(22px,4vw,36px);font-weight:300;color:var(--ms-dark);margin:0 auto 12px;line-height:1.2;">{{ __('Login with OTP') }}</h1>
            ...
            <div class="ms-auth-card">
                ...
                <div style="text-align:center;margin-bottom:24px;">
                    <div style="width:64px;height:64px;border-radius:50%;background:var(--ms-warm);border:1px solid var(--ms-border);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
                        <i class="mdi mdi-cellphone-message" style="font-size:28px;color:var(--ms-olive);"></i>
                    </div>
                    <h2 style="font-size:20px;font-weight:400;color:var(--ms-dark);margin-bottom:8px;">{{ __('Login with OTP') }}</h2>
                    <p style="font-size:13px;color:var(--ms-muted);line-height:1.6;">
                        {{ __('Enter your phone number and we\'ll send you a one-time password.') }}
                    </p>
                </div>

                <div class="ms-auth-divider"></div>

                <form action="{{ route('tenant.user.login.otp') }}" method="POST" style="margin-top:24px;">
                    @csrf
                    <div class="ms-form-group">
                        <label class="ms-form-label">{{ __('Phone Number') }}</label>
                        <input type="text" name="phone" class="ms-form-input"
                               placeholder="{{ __('e.g. +1 555 000 0000') }}"
                               value="{{ old('phone') }}" required>
                    </div>

                    <button type="submit" class="ms-btn-full ms-btn-dark" style="margin-top:20px;">
                        <i class="mdi mdi-message-text-outline" style="margin-right:8px;font-size:16px;"></i>
                        {{ __('Send OTP') }}
                    </button>
                </form>
