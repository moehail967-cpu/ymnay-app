...
class BlogController extends Controller
{
    ...
    public function blog_comment_store(Request $request)
    {
        ...
        $user_name = $user ? $user->name : $request->commented_by;
        $user_email = $user ? $user->email : '';

        // Resolve the commenter's image URL for instant display in JS
        $user_image_data = $user && $user->image
            ? get_attachment_image_by_id($user->image, 'thumb', false)
            : [];
        $user_image_url = $user_image_data['img_url'] ?? '';

        $sub = __('You have a comment from') . ' ' . get_static_option('site_title');
        $message = __('you have a new comment submitted by') . ' ' . $user_name . ' ' . __('Email') . ' ' . $user_email . ' .' . __('check admin panel for more info');
        ...
        return response()->json([
            ...
            'commented_by' => $request->commented_by,
            'comment_content' => SanitizeInput::esc_html($request->comment_content),
            'is_reply' => !empty($request->comment_id),
        ]);
    }
...
HTML;
    ...
    public function load_more_comments(Request $request)
    {
        ...
        $all_comment = BlogComment::with(['blog', 'user', 'admin', 'reply.user', 'reply.admin'])
        ...
        foreach ($all_comment as $item) {
            $commented_user_image = render_image_markup_by_attachment_id(
                    optional($item->user)->image ?? optional($item->admin)->image ?? get_static_option('blog_avatar_image'),
                    '',
                    'thumb'
                );
...
REPLA;
            ...
            foreach ($item->reply as $repData) {
                $child_image = render_image_markup_by_attachment_id(optional($repData->user)->image ?? optional($repData->admin)->image ?? get_static_option('blog_avatar_image'), '', '', false);
