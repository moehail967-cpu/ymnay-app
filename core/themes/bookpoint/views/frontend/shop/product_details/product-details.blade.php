...
{{-- Breadcrumb --}}
<div class="container bp-pd-breadcrumb">
...
{{-- Product Section --}}
<div class="container bp-pd-section">
    <div class="row g-5">

        {{-- ===== Images ===== --}}
        <div class="col-lg-5">
            <div class="bp-thumb-row" id="shop_details_gallery_slider">
                {{-- Thumbnails --}}
                <div class="bp-thumb-list">
                    @foreach($image_array as $imgId)
                        @php
                            $tdata = get_attachment_image_by_id($imgId, 'grid');
                            $turl  = $tdata['img_url'] ?? null;
                        @endphp
                        <div class="bp-thumb {{ $loop->first ? 'active' : '' }}"
                             onclick="bpSwitchImg(this, '{{ $turl }}')">
                            @if($turl)
                                <img src="{{ $turl }}" alt="">
                            @else
                                <span class="bp-pd-thumb-placeholder"><i class="las la-book"></i></span>
                            @endif
                        </div>
                    @endforeach
                </div>
                {{-- Main image — .long-img required by theme_product_js() syncImage() --}}
                <div class="bp-pd-main-img">
                    <div class="shop-details-thums long-img" data-src="{{ $main_img_url }}">
                        @if($main_img_url)
                            <img id="bp-main-product-img"
                                 src="{{ $main_img_url }}"
                                 alt="{{ $product->name }}"
                                 class="bp-pd-main-img-fill">
                        @else
                            <div class="bp-pd-img-placeholder"><i class="las la-book la-3x"></i></div>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-7">
            @if($product->badge)
                <span class="bp-card-badge bp-pd-badge-inline">{{ $product->badge->name }}</span>
            @endif

            <h1 class="bp-pd-title">{{ $product->name }}</h1>
            ...
            {{-- Product options (sizes, colors, qty, cart buttons) --}}
            <div class="bp-product-form">
                @include(include_theme_path('shop.product_details.partials.product-options'))
            </div>

            {{-- Trust pills --}}
            <div class="d-flex gap-3 flex-wrap mt-4 bp-pd-trust">
                <span><i class="las la-shield-alt bp-pd-trust-secure"></i> {{ __('Secure Checkout') }}</span>
                <span><i class="las la-redo-alt bp-pd-trust-returns"></i> {{ __('Easy Returns') }}</span>
                <span><i class="las la-truck bp-pd-trust-delivery"></i> {{ __('Fast Delivery') }}</span>
            </div>

            {{-- Product meta --}}
            <div class="bp-product-meta mt-4">
                @if($product->sku ?? null)
                    <div class="bp-meta-row">
                        <span class="bp-meta-label">{{ __('SKU') }}:</span>
                        <span>{{ $product->sku }}</span>
                    </div>
                @endif
                @if($product->category?->name)
                    <div class="bp-meta-row">
                        <span class="bp-meta-label">{{ __('Category') }}:</span>
                        <span>{{ $product->category->name }}</span>
                    </div>
                @endif
                @if($product->tags?->isNotEmpty())
                    <div class="bp-meta-row">
                        <span class="bp-meta-label">{{ __('Tags') }}:</span>
                        <span>
                ...
                @if($product->inventory?->stock_count !== null)
                    <div class="bp-meta-row">
                        <span class="bp-meta-label">{{ __('Items Left') }}:</span>
                        <span id="item_left"
    ...
    <div class="bp-pd-tabs-wrap">
        <div class="bp-tab-nav">
            <button class="bp-tab-btn active" data-target="bp-tab-desc">{{ __('Description') }}</button>
            <button class="bp-tab-btn" data-target="bp-tab-reviews">{{ __('Reviews') }}</button>
            <button class="bp-tab-btn" data-target="bp-tab-ship">{{ __('Ship & Return') }}</button>
        </div>

        <div id="bp-tab-desc" class="bp-tab-panel active">
            @include(include_theme_path('shop.product_details.partials.product-description'))
        </div>
        <div id="bp-tab-reviews" class="bp-tab-panel">
            @include(include_theme_path('shop.product_details.partials.product-reviews'))
        </div>
        <div id="bp-tab-ship" class="bp-tab-panel">
    ...
    <div class="bp-pd-related">
        <h3 class="bp-pd-related-title">{{ __('You Might Also Like') }}</h3>
        <div class="row g-3">
                ...
                <div class="col-6 col-md-3">
                    <div class="bp-card">
                        <div class="bp-card-img">
                            ...
                            @else
                                <a href="{{ theme_product_url($rp->slug) }}" class="bp-card-img-placeholder">
                        ...
                        <div class="bp-card-body">
                            ...
                            <div class="bp-card-price">
                                <span class="bp-price-sale">{{ amount_with_currency_symbol($rpdata['sale_price']) }}</span>
                            </div>
                            <a href="{{ theme_product_url($rp->slug) }}" class="bp-card-atc">
...
@section('scripts')
<script>
function bpSwitchImg(el, url) {
    document.querySelectorAll('.bp-thumb').forEach(function (t) { t.classList.remove('active'); });
    el.classList.add('active');
    var mainImg = document.getElementById('bp-main-product-img');
...
$(function () {
    ...
    // Tab switching
    $(document).on('click', '.bp-tab-btn', function () {
        var target = $(this).data('target');
        $('.bp-tab-btn').removeClass('active');
        $(this).addClass('active');
        $('.bp-tab-panel').removeClass('active');
