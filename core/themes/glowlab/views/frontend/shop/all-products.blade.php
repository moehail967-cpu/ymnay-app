...
<div style="background:var(--gl-gold-pale);border-bottom:1px solid var(--gl-border);padding:36px 0 28px;">
    <div class="container">
        <h1 style="font-size:26px;font-weight:300;color:var(--gl-dark);margin-bottom:10px;letter-spacing:-.3px;">{!! $page_post->title !!}</h1>
        <div style="display:flex;align-items:center;gap:8px;font-size:12px;color:var(--gl-muted);">
            ...
            <span style="color:var(--gl-dark);">{!! $page_post->title !!}</span>
        </div>
    </div>
</div>
...
<div style="background:#fff;border-bottom:1px solid var(--gl-border);">
    <div class="container">
        <div style="display:flex;align-items:center;gap:8px;padding:12px 0;overflow-x:auto;flex-wrap:nowrap;">
            <button class="gl-cat-nav-btn active" data-slug=""
                    style="flex-shrink:0;padding:7px 18px;border-radius:50px;border:1.5px solid var(--gl-border);background:var(--gl-dark);color:#fff;font-size:12px;font-weight:600;letter-spacing:.5px;cursor:pointer;transition:all .2s;">
                {{ __('All') }}
            </button>
            @foreach($categories as $cat)
            <button class="gl-cat-nav-btn" data-slug="{{ $cat->slug }}"
                    style="flex-shrink:0;padding:7px 18px;border-radius:50px;border:1.5px solid var(--gl-border);background:transparent;color:var(--gl-muted);font-size:12px;font-weight:600;letter-spacing:.5px;cursor:pointer;transition:all .2s;">
                {{ $cat->name }}
            </button>
            @endforeach
        </div>
                ...
                <div style="padding:8px 0;border-bottom:1px dashed var(--gl-border);display:flex;justify-content:space-between;align-items:center;">
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;color:var(--gl-dark);">
                        <input type="checkbox" class="gl-cat-checkbox-input" data-slug="{{ $cat->slug }}" data-value="{{ $cat->name }}"
                               style="accent-color:var(--gl-gold);width:14px;height:14px;">
                        {{ $cat->name }}
                    </label>
