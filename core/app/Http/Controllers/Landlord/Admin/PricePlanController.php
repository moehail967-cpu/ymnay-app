...
class PricePlanController extends Controller
{
    ...
    public function create_price_plan(){
        $tenantPlugins = $this->getTenantPlugins();
        return view('landlord.admin.price-plan.create', compact('tenantPlugins'));
    }
...
//            $plan->plan_themes()->delete();
    ...
    public function edit_price_plan($id){
        ...
        $tenantPlugins  = $this->getTenantPlugins();
        $planPluginIds  = $plan->plan_plugins->pluck('plugin_id')->toArray();

        return view('landlord.admin.price-plan.edit', compact('plan', 'plan_payment_gateways', 'tenantPlugins', 'planPluginIds'));
    }
    public function store_new_price_plan(Request $request){
        ...
        try {
            ...
            // Sync to Razorpay if recurring is enabled (after save so we have the ID)
            if (!tenant() && RazorpaySubscriptionService::isGloballyEnabled()) {
                RazorpaySubscriptionService::syncPlanToRazorpay($price_plan);
            }
...
//            }
            ...
            if (!tenant() && $request->filled('plan_plugins')) {
                foreach ($request->plan_plugins as $plugin_id) {
                    PlanPlugin::firstOrCreate([
                        'plan_id'   => $price_plan->id,
                        'plugin_id' => $plugin_id,
                    ]);
                }
            ...
            if (!tenant()) {
                app(PluginManager::class)->clearPlanPluginCache($price_plan->id);
            }
        } catch (\Exception $e)
        {
...
//            'themes' => 'required',
        ...
        try {
            ...
            // Re-sync to Razorpay if recurring is enabled (creates new plan since Razorpay doesn't allow updates)
            if (!tenant() && RazorpaySubscriptionService::isGloballyEnabled()) {
                RazorpaySubscriptionService::syncPlanToRazorpay($price_plan);
            }
...
//            }
            ...
            if (!tenant()) {
                $price_plan->plan_plugins()->delete();
                foreach ($request->plan_plugins ?? [] as $plugin_id) {
                    PlanPlugin::create([
                        'plan_id'   => $price_plan->id,
                        'plugin_id' => $plugin_id,
...
//        dd($request->all());
    ...
    private function getTenantPlugins(): \Illuminate\Support\Collection
    {
        try {
            return collect(app(PluginManager::class)->allDiscovered())
                ->filter(fn($m) => in_array($m->type, ['tenant', 'both']))
                ->sortBy('name')
                ->values();
        } catch (\Throwable) {
            return collect();
