<template>
    <div class="popup-fixed interview-popup">
        <div class="popup-contents" v-if="product">
            <span class="popup-contents-close popup-close" @click="closeModal">
            ...
            <div class="modal-body p-0">
                <div class="editProduct">
                    ...
                    <div class="editProduct__breadcrumb">
                        <span class="editProduct__breadcrumb__item">{{ product.product.category.name }}</span>
                        <i class="las la-angle-right"></i>
                        <span class="editProduct__breadcrumb__item">{{ product.product.sub_category.name }}</span>
                        <template v-if="childCategoryString(product.product.child_category)">
                            <i class="las la-angle-right"></i>
                            <span class="editProduct__breadcrumb__item">{{ childCategoryString(product.product.child_category) }}</span>
                        </template>
                    </div>
                    ...
                    <div class="row g-4 mt-0">
                        <div class="col-lg-4">
                            ...
                            <div class="shop-details-thumb-wrapper d-flex gap-2 justify-content-center mt-3">
                                <div class="shop-details-thums" v-for="productGallery in product.product.gallery_images">
                        ...
                        <div class="col-lg-8">
                            <div class="editProduct__contents">
                                <h5 class="editProduct__contents__title">
                                    <a href="{{ product.product_url }}">{{ product.product.name }}</a>
                                </h5>
                                <p class="editProduct__brand" v-if="product.product?.brand?.name">
                                    <i class="las la-tag"></i> {{ product.product.brand.name }}
                                </p>

                                <p class="editProduct__contents__price mt-2">
                                    <b id="price" v-bind:data-main-price="product.product.sale_price">
                                        {{ getCurrencySymbolWithAmount(productPrice) }}
                                    </b>
                                    <s class="original-price">{{ getCurrencySymbolWithAmount(product.product.price) }}</s>
                                </p>

                                <div class="stock-badge stock-badge--in mt-2" v-if="product.product?.inventory?.stock_count > 0">
                                    <i class="las la-check-circle"></i> In Stock ({{ product.product.inventory.stock_count }})
                                </div>
                                <div class="stock-badge stock-badge--out mt-2" v-if="product.product?.inventory?.stock_count < 1">
                                    <i class="las la-times-circle"></i> Out of Stock
                                </div>

                                <div class="valueInput value-input-area mt-3" v-if="product.productSizes">
                                ...
                                <div class="valueInput value-input-area mt-3" v-if="product.productColors">
                                    <span class="valueInput__head">
                                        <strong>Color:</strong>
                                        <input class="form--input value-color" name="color" type="text" value="" readonly>
                                ...
                                <div class="value-input-area mt-3 attribute_options_list"
                                    ...
                                    <ul class="size-lists" v-bind:data-type="attr_name">
                                        <li @click="handleSizeListClick" v-for="option in attribute"
                                ...
                                <div class="btn-wrapper btn_flex justify-content-start mt-4 align-items-center">
                                    <div class="product__quantity m-0">
                                        <span @click="productAmount--" class="substract">
                                            <i class="las la-minus"></i>
                                        </span>
                                        <input class="product__quantity__input" type="number" :value="productAmount">
                                        <span @click="productAmount++" class="plus">
                                            <i class="las la-plus"></i>
                                        </span>
                                    </div>
                                    <a href="#" class="posBtn posBtn_cart"
                                       @click.prevent="variantProductAddToCart(product.product)">
                                        <i class="las la-cart-plus"></i> Add to Cart
                                    </a>
                                    <div class="stock-badge stock-badge--in" v-if="variantStock > 0">
                                        In Stock ({{ variantStock }})
                                    </div>
                                    <div class="stock-badge stock-badge--out" v-if="outOfStock == 1">
                                        Out of stock
                                    </div>
                                </div>
                            </div>
