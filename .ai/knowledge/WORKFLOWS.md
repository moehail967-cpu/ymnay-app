...
- Chain: StoreOnboardingController records plan/theme/store data in StoreOnboardingRequest → LandlordFrontendController OTP registration or login claims the request → the onboarding-owned verification route requires OTP for an existing unverified owner independently of the optional legacy global verification flag; its strict sender reports a safe explicit outcome and rolls back token rotation when delivery fails → review compares saved/current plan values independently of JSON key order → completion locks the central user/request and revalidates all saved store data, current reserved/claimed address rules, verification and trial eligibility → marked TenantRegisterEvent selects StoreOnboardingProvisioner → checkpointed readiness → central trial transaction → signed tenant dashboard URL.
...
- Verification boundary: isolated tests use real framework components and MySQL with explicit synthetic tenant migration/seed/auth/mail adapters. They cover rejected verification delivery (including 550/553), preserved tokens/requests and successful retry without provisioning. Full application boot, actual SMTP, themes/modules, OTP/cookie flows, queue completion and desktop/mobile/RTL browser acceptance remain separate QA gates.
