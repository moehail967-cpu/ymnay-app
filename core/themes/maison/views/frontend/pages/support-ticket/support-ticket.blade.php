...
<div class="ms-dash-card">
    <div class="ms-dash-section-title" style="margin-bottom:20px;">
        <i class="mdi mdi-headset" style="margin-right:6px;color:var(--ms-olive);"></i>
    ...
    <form action="{{ theme_user_ticket_store_url() }}" method="POST" enctype="multipart/form-data">
        ...
        @php $inp = 'width:100%;padding:10px 14px;border:1.5px solid var(--ms-border);border-radius:6px;font-size:14px;font-family:inherit;outline:none;background:#fff;transition:border-color .2s;'; @endphp
        @php $lbl = 'font-size:12px;font-weight:600;color:var(--ms-muted,#9ca3af);margin-bottom:5px;display:block;text-transform:uppercase;letter-spacing:.5px;'; @endphp
        <div class="row g-3 mb-3">
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Title') }} <span style="color:var(--ms-linen);">*</span></label>
                <input type="text" name="title" value="{{ old('title') }}" style="{{ $inp }}" placeholder="{{ __('Ticket title') }}" required>
            </div>
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Subject') }} <span style="color:var(--ms-linen);">*</span></label>
                <input type="text" name="subject" value="{{ old('subject') }}" style="{{ $inp }}" placeholder="{{ __('Brief subject') }}" required>
            </div>
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Priority') }} <span style="color:var(--ms-linen);">*</span></label>
            ...
            <div class="col-md-6">
                <label style="{{ $lbl }}">{{ __('Department') }} <span style="color:var(--ms-linen);">*</span></label>
            ...
            <div class="col-12">
                <label style="{{ $lbl }}">{{ __('Description') }} <span style="color:var(--ms-linen);">*</span></label>
        ...
        <button type="submit" class="ms-btn-dark" style="display:inline-flex;align-items:center;gap:8px;">
