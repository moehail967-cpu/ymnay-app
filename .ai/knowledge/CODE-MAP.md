...
| Plans / subscription checkout | web.php: `/plan-order/{id}`, `/order-confirm`, `/create-store`, C/U | Paid purchase: LandlordFrontendController → PaymentLogController → PaymentGateways. Trial onboarding: StoreOnboardingController → StoreOnboardingRequest → TenantRegisterEvent/TenantTrialPaymentLog | Paid package views remain native; `/create-store` is the resumable five-step trial wizard; WORKFLOWS W01/W10 |
