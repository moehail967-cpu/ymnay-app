                        ...
                        <div class="global-flex-card-contents-bottom border-flex-card pt-4 mt-4 pt-lg-5 mt-lg-5">
                            <div class="global-flex-card-contents-icon">
                                ...
                                @else
                                    <div class="btn-wrapper" data-bs-toggle="tooltip" data-bs-placement="top"
                                         title="{{__('Add to Cart')}}">
                                        <a href="javascript:void(0)" class="product-quick-view-ajax cmn-btn cmn-btn-bg-2 radius-0" data-action-route="{{ theme_quick_view_url($product->slug) }}"> {{__('Add to Cart')}} </a>
                                    </div>
                                    <div class="icon-list wishlist-btn" data-bs-toggle="tooltip" data-bs-placement="top"
                                         title="{{__('Add to Wishlist')}}">
                                        <a class="icon cart-loading product-quick-view-ajax" href="javascript:void(0)" data-action-route="{{ theme_quick_view_url($product->slug) }}"> <i class="lar la-heart"></i>
                                        </a>
                                    </div>
                                    <div class="icon-list compare-btn" data-bs-toggle="tooltip" data-bs-placement="top"
                                         title="{{__('Add to Compare')}}">
                                        <a class="icon cart-loading product-quick-view-ajax" href="javascript:void(0)" data-action-route="{{ theme_quick_view_url($product->slug) }}"> <i
                                                    class="las la-retweet"></i> </a>
                                    </div>
                                @endif
                ...
                <ul class="pagination-list">
                    @if(count($links) > 1)
                        @foreach(generateDynamicPagination($current_page, count($links)) as $index => $link)
                            @if($link === '...')
                                <li>
                                    <a data-page="{{ $current_page }}" href="{{ $current_page }}" class="page-number">{{ $link }}</a>
                                </li>
                            @else
                                <li>
