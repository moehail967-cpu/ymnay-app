...
class PaymentGatewayIpn
{
    ...
    {
        $payment_data = apply_filters('nazmart:before_payment', $payment_data);
        ...
        if (isset($payment_data['status']) && $payment_data['status'] === 'complete') {
            ...
            do_action('nazmart:payment_success', $payment_data, $order);

            $payment_data_order_id = $payment_data['order_id'];
            if($order->payment_status === 'success'){
                Log::info('event is triggered.');
                event(new OrderCompleted($payment_data_order_id, tenant()));
                Log::info('Pass data:', [
                    'order_id' => $payment_data_order_id ?? null,
                    'tenant_id' => tenant()->user_id ?? null,
                ]);
            }
        ...
        $order = isset($payment_data['order_id']) ? ProductOrder::find($payment_data['order_id']) : null;
        do_action('nazmart:payment_failed', $payment_data, $order, 'Payment was not completed');
