...
<div style="background:var(--vl-surface);border-bottom:1px solid var(--vl-border);padding:40px 0 28px;">
    <div class="container">
        ...
        <h2 style="font-size:36px;font-weight:400;color:var(--vl-ivory);margin-bottom:12px;font-family:'Cormorant Garamond',serif;letter-spacing:2px;">{{ $category_name ?? __('Category') }}</h2>
                ...
                @empty
                    <div class="col-12">
                        <div style="text-align:center;padding:64px;border:1px dashed var(--vl-border);">
                            <div style="font-size:10px;letter-spacing:4px;text-transform:uppercase;color:var(--vl-muted);">{{ __('No Articles in This Category') }}</div>
                        </div>
                    </div>
                @endforelse
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div style="background:var(--vl-card);border:1px solid var(--vl-border);padding:28px;margin-bottom:28px;">
                ...
                <ul style="list-style:none;margin:0;padding:0;">
                    ...
                    @php $isActive = $cat->name() === ($category_name ?? ''); @endphp
                    <li style="border-bottom:1px solid rgba(58,36,68,.5);padding:10px 0;">
                        <a href="{{ $cat->url() }}"
                           style="display:flex;align-items:center;justify-content:space-between;font-size:13px;color:{{ $isActive ? 'var(--vl-champagne)' : 'var(--vl-muted)' }};text-decoration:none;letter-spacing:.5px;transition:color .2s;font-weight:{{ $isActive ? '600' : '400' }};"
                           @if(!$isActive)onmouseover="this.style.color='var(--vl-champagne)'" onmouseout="this.style.color='var(--vl-muted)'"@endif>
                            <span>{{ $cat->name() }}</span>
