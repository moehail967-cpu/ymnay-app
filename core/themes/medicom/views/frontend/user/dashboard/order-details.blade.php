...
<div class="mc-dash-card">
    <div class="mc-dash-card-title"><i class="las la-receipt"></i> {{ __('Order') }} #{{ $order->id }}</div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-sm-3">
            ...
            <div style="font-size:16px;font-weight:800;color:#1A85ED;">{{ amount_with_currency_symbol($order->total ?? 0) }}</div>
        </div>
    </div>
    ...
    <div style="text-align:right;margin-top:16px;padding-top:16px;border-top:1.5px solid #EAF2F8;">
        <div style="display:inline-flex;flex-direction:column;gap:8px;min-width:200px;">
            ...
            @endif
            <div class="d-flex justify-content-between gap-4" style="font-size:16px;font-weight:800;color:#1a1a1a;padding-top:8px;border-top:1px solid #EAF2F8;">
                <span>{{ __('Total') }}</span>
                <span style="color:#1A85ED;">{{ amount_with_currency_symbol($order->total ?? 0) }}</span>
            </div>
        </div>
    </div>
</div>

<a href="{{ route('tenant.user.dashboard.package.order') }}" class="mc-btn mc-btn-ghost">
