    ...
    <div class="row g-4">
        ...
        <div class="col-lg-3">
            ...
            <div class="lg-sidebar" style="margin-bottom:20px;">
                ...
                <div style="display:flex;border:1.5px solid var(--lx-border,#e0e0e0);border-radius:var(--lx-radius,4px);overflow:hidden;">
                    <input type="text" id="lg-search-input" placeholder="{{ __('Search products…') }}"
                        style="flex:1;border:none;outline:none;padding:9px 12px;font-size:13px;background:transparent;">
                    <button type="button" onclick="luxegemsFilterRequest()"
                        style="background:var(--lx-gold,#C9A84C);color:#fff;border:none;padding:0 14px;cursor:pointer;">
            ...
            <div class="lg-sidebar">
                ...
                <div style="margin-top:4px;">
                    <a href="javascript:void(0)" onclick="$('.lg-rating-filter,.lg-cat-checkbox').prop('checked',false);luxegemsFilterRequest();"
                        style="font-size:12px;color:var(--lx-muted,#888);text-decoration:none;">
...
$(function () {
    function luxegemsFilterRequest(page) {
        ...
        var cats = [];
        $('.lg-cat-checkbox:checked').each(function () { cats.push($(this).data('slug')); });
        if (cats.length) params.category = cats.join(',');

        var rating = $('input[name=lg_rating]:checked').val();
        if (rating) params.rating = rating;

        var search = $('#lg-search-input').val();
    ...
    $(document).on('keydown', '#lg-search-input', function (e) {
        if (e.key === 'Enter') { luxegemsFilterRequest(); }
    });
});
</script>
@endsection
