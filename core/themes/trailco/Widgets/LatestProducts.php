...
{
    protected function getWidgetType(): string { return 'trailco_latest_products'; }
    protected function getWidgetName(): string { return 'TrailCo: Latest Products List'; }
    protected function getWidgetIcon(): string|array { return 'mdi mdi-format-list-bulleted'; }
    protected function getWidgetDescription(): string { return __('Products in a compact list layout — image left, details right'); }
    protected function getCategory(): string { return WidgetCategory::ECOMMERCE; }
    protected function getWidgetTags(): array { return ['products', 'list', 'latest', 'trailco']; }
    ...
    public function getGeneralFields(): array
    {
        ...
        $control->addGroup('content', 'Content')
            ...
            ->registerField('subtitle', FieldManager::TEXT()->setLabel('Subtitle')->setDefault('Latest gear just dropped'))
            ->registerField('view_all_url', FieldManager::URL()->setLabel('View All URL')->setDefault('#'))
            ->endGroup();

        $control->addGroup('query', 'Product Query')
            ->registerField('item_count', FieldManager::NUMBER()->setLabel('Products to Show')->setDefault(6)->setMin(2)->setMax(12))
            ->registerField('sort_by', FieldManager::SELECT()
                ->setLabel('Sort By')
                ->setOptions(['latest' => 'Latest', 'price_asc' => 'Price: Low to High', 'price_desc' => 'Price: High to Low', 'popular' => 'Most Popular'])
                ->setDefault('latest'))
            ->endGroup();
    ...
    public function render(array $settings = []): string
    {
        ...
        if (!function_exists('tenant') || !tenant()) {
            return '<div style="padding:40px;text-align:center;background:#f9f9f9;border:1px dashed #ccc;border-radius:8px;color:#888;font-family:sans-serif;">
                        <p style="margin:0;font-size:14px;">TrailCo: Latest Products List — preview on the live page</p>
                    </div>';
        }
        ...
        return view('theme-trailco::widgets.latest_products', [
