...
<div class="el-dash-card">
    <div class="el-dash-card-title"><i class="las la-headset"></i> {{ __('Ticket') }} #{{ $ticket?->id }} — {{ $ticket?->subject }}</div>
    ...
    <div style="display:flex;flex-direction:column;gap:14px;margin-bottom:28px;">
        ...
        <div style="display:flex;gap:12px;{{ $msg->user_type === 'admin' ? 'flex-direction:row-reverse;' : '' }}">
            <div style="width:38px;height:38px;border-radius:50%;background:{{ $msg->user_type === 'admin' ? '#E8603C' : '#6B7280' }};color:#fff;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:800;flex-shrink:0;">
                {{ $msg->user_type === 'admin' ? 'A' : strtoupper(substr(auth('web')->user()?->name ?? 'U', 0, 1)) }}
            </div>
            <div style="max-width:75%;background:{{ $msg->user_type === 'admin' ? '#fff5f0' : '#f9f9f9' }};border-radius:8px;padding:12px 16px;border:1.5px solid #f0e0d6;">
    ...
    <form action="{{ theme_user_ticket_reply_url() }}" method="post">
        @csrf
        <label class="el-form-label" style="margin-bottom:8px;display:block;">{{ __('Your Reply') }}</label>
        <textarea name="message" class="el-form-input el-form-textarea" rows="4" style="margin-bottom:12px;" placeholder="{{ __('Type your message…') }}"></textarea>
...
<a href="{{ theme_user_tickets_url() }}" class="el-btn el-btn-ghost">
