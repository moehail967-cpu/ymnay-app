...
<section class="el-pay-section">
    <div class="container">
        <div class="el-order-confirm">
            <div class="el-dash-card-title"><i class="las la-receipt"></i> {{ __('Order Details') }}</div>
            {!! theme_flash_msg() !!}
            {!! theme_error_msg() !!}
            <form action="{{ theme_payment_form_url() }}" method="post" enctype="multipart/form-data" class="mt-4">
                ...
                <div class="el-confirm-table-wrap">
                    <table class="el-dash-table">
                        <tr>
                            <td style="width:160px;font-weight:600;">{{ __('Your Name') }}</td>
                            <td>
                                @if(auth()->check())
                                    <input type="text" name="name" value="{{ $name }}" class="el-form-input" placeholder="{{ __('Enter Your Name') }}" readonly>
                                @else
                                    <input type="text" name="name" id="pkg_user_name" value="{{ $name }}" class="el-form-input" placeholder="{{ __('Enter Your Name') }}">
                                @endif
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                @if(auth()->check())
                                    <input type="email" name="email" value="{{ $email }}" class="el-form-input" placeholder="{{ __('Enter Your Email') }}" readonly>
                                @else
                                    <input type="email" name="email" id="pkg_user_email" value="{{ $email }}" class="el-form-input" placeholder="{{ __('Enter Your Email') }}">
                                @endif
                            </td>
                        </tr>
                        <tr>
                ...
                <div class="mt-4">
                    <button class="el-btn el-btn-primary el-btn-block" id="pay_now" type="submit">{{ __('Pay Now') }}</button>
                </div>
            </form>
        </div>
    </div>
