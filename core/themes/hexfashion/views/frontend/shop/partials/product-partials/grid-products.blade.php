<div class="hf-shop-grid mt-4">
    @foreach($products as $product)
    @php
        $pd       = get_product_dynamic_price($product);
        $price    = $pd['sale_price'];
        $oldPrice = $pd['regular_price'];
        $discount = $pd['discount'];
        $pImg     = theme_product_image($product->image_id ?? null, 'grid');
        $pUrl     = theme_product_url($product->slug ?? $product->id);
    @endphp
    <div class="hf-card h-100 d-flex flex-column">
        <div class="hf-card-img-wrap">
            <a href="{{ $pUrl }}">
                @if($pImg)
                <img src="{{ $pImg }}" alt="{{ $product->name }}" class="hf-card-img" loading="lazy">
                @else
                <div class="hf-card-img-ph"><i class="las la-laptop"></i></div>
                @endif
            </a>
            <button class="add-to-wishlist-btn hf-card-wish" data-product_id="{{ $product->id }}" title="{{ __('Wishlist') }}">
                <i class="las la-heart"></i>
            </button>
            @if($discount)
            <span class="hf-card-badge">{{ $discount }}% {{ __('off') }}</span>
            @endif
            @if(!empty($product->badge))
            <span class="hf-card-badge hf-card-badge-new" style="top:36px">{{ $product->badge->name }}</span>
            @endif
        </div>
        <div class="hf-card-body d-flex flex-column flex-grow-1">
            <a href="{{ $pUrl }}" class="hf-card-name flex-grow-1">{{ Str::limit($product->name, 45) }}</a>
            <div class="hf-card-footer">
                <div class="hf-card-prices">
                    <span class="hf-card-price mt-auto">{{ amount_with_currency_symbol($price) }}</span>
                    @if($oldPrice)
                    <span class="hf-card-old">{{ amount_with_currency_symbol($oldPrice) }}</span>
                    @endif
                </div>
                <button class="add-to-cart-btn hf-card-atc" data-product_id="{{ $product->id }}" title="{{ __('Add to Cart') }}">
                    <i class="las la-plus"></i>
                </button>
            </div>
        </div>
    </div>
    @endforeach
</div>

{{-- Pagination --}}
@if(count($links) > 1)
<div class="hf-pagination mt-5">
    @foreach(generateDynamicPagination($current_page, count($links)) as $index => $link)
        @if($link === '...')
            <span class="hf-pg-btn hf-pg-dots">…</span>
        @else
            <button class="hf-pg-btn {{ $link === $current_page ? 'hf-pg-active' : '' }}"
                    onclick="elFilterRequest({{ $link }})">{{ $link }}</button>
        @endif
    @endforeach
</div>
@endif
