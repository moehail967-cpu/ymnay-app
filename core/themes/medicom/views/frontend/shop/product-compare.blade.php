...
<div class="mc-page-banner">
    <div class="container">
        <h1 class="mc-page-banner-title">{{ __('Compare Products') }}</h1>
...
<div class="mc-compare-section">
    <div class="container">

        @if(count($compare_products) > 0)

        <div class="table-responsive mc-compare-table-wrap">
            <table class="mc-compare-table">
                <thead>
                    <tr>
                        <th class="mc-compare-head-label"></th>
                        @foreach($compare_products as $product)
                        <th class="mc-compare-head-cell">
                            @php $img = theme_product_image($product->image_id ?? null, 'grid'); @endphp
                            <div class="mc-compare-product-card">
                                <a href="{{ theme_product_url($product->slug) }}" class="mc-compare-img-link">
                                    @if($img)
                                        <img src="{{ $img }}" alt="{{ $product->name }}" class="mc-compare-product-img">
                                    @else
                                        <div class="mc-compare-img-placeholder"><i class="las la-medkit"></i></div>
                                    @endif
                                </a>
                                <div class="mc-compare-product-name">
                                    <a href="{{ theme_product_url($product->slug) }}">{{ \Illuminate\Support\Str::words($product->name, 7) }}</a>
                                </div>
                                <button class="mc-compare-remove compare-remove-btn"
                ...
                <tbody>
                    {{-- Price --}}
                    <tr class="mc-compare-row">
                        <td class="mc-compare-label"><i class="las la-tag"></i> {{ __('Price') }}</td>
                        @foreach($compare_products as $product)
                        @php $pdata = theme_product_price($product); @endphp
                        <td class="mc-compare-cell">
                            <div class="mc-compare-price-wrap">
                                <span class="mc-compare-price">{{ amount_with_currency_symbol($pdata['sale_price']) }}</span>
                                @if($pdata['regular_price'] && $pdata['regular_price'] != $pdata['sale_price'])
                                    <span class="mc-compare-old-price">{{ amount_with_currency_symbol($pdata['regular_price']) }}</span>
                                @endif
                                @if($pdata['discount'])
                                    <span class="mc-compare-discount-badge">{{ $pdata['discount'] }}% {{ __('off') }}</span>
                                @endif
                            </div>
                        </td>
                        @endforeach
                    ...
                    {{-- Rating --}}
                    <tr class="mc-compare-row mc-compare-row-alt">
                        <td class="mc-compare-label"><i class="las la-star"></i> {{ __('Rating') }}</td>
                        @foreach($compare_products as $product)
                        <td class="mc-compare-cell">
                    ...
                    {{-- SKU --}}
                    <tr class="mc-compare-row">
                        <td class="mc-compare-label"><i class="las la-barcode"></i> {{ __('SKU') }}</td>
                        @foreach($compare_products as $product)
                        <td class="mc-compare-cell mc-compare-muted">
                    ...
                    {{-- Category --}}
                    <tr class="mc-compare-row mc-compare-row-alt">
                        <td class="mc-compare-label"><i class="las la-th-large"></i> {{ __('Category') }}</td>
                        @foreach($compare_products as $product)
                        <td class="mc-compare-cell mc-compare-muted">
                            @if($product->category)
                                <a href="{{ theme_category_url($product->category->slug) }}" class="mc-compare-cat-link">
                    ...
                    {{-- Availability --}}
                    <tr class="mc-compare-row">
                        <td class="mc-compare-label"><i class="las la-boxes"></i> {{ __('Availability') }}</td>
                        @foreach($compare_products as $product)
                        @php $in_stock = theme_product_in_stock($product); @endphp
                        <td class="mc-compare-cell">
                            @if($in_stock)
                                <span class="mc-compare-in-stock"><i class="las la-check-circle"></i> {{ __('In Stock') }}</span>
                            @else
                                <span class="mc-compare-out-stock"><i class="las la-times-circle"></i> {{ __('Out of Stock') }}</span>
                            @endif
                        </td>
                        @endforeach
                    </tr>
                    ...
                    {{-- Add to Cart --}}
                    <tr class="mc-compare-row mc-compare-row-alt mc-compare-row-atc">
                        <td class="mc-compare-label"></td>
                        @foreach($compare_products as $product)
                        <td class="mc-compare-cell">
                            <a href="{{ theme_product_url($product->slug) }}"
                               class="mc-btn mc-btn-primary mc-btn-sm mc-compare-atc-btn">
        ...
        <div class="mc-compare-footer">
            <a href="{{ theme_shop_url() }}" class="mc-btn mc-btn-ghost">
        ...
        <div class="mc-compare-empty">
            <div class="mc-compare-empty-icon">
            ...
            <a href="{{ theme_shop_url() }}" class="mc-btn mc-btn-primary">
