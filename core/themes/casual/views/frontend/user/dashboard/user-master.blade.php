...
<div class="cs-dash-section">
    <div class="container">
        ...
        <button class="cs-dash-mobile-toggle" id="cs-dash-toggle">
            <i class="las la-bars"></i> {{ __('Account Menu') }}
        </button>

        <div class="cs-dash-wrap">

            {{-- Sidebar --}}
            <div class="cs-dash-sidebar" id="cs-dash-sidebar">
                @php $u = auth('web')->user(); @endphp
                <div class="cs-dash-user">
                    <div class="cs-dash-avatar">{{ strtoupper(substr($u?->name ?? 'U', 0, 1)) }}</div>
                    <div class="cs-dash-user-info">
                        <div class="cs-dash-name">{{ $u?->name }}</div>
                        <div class="cs-dash-email">{{ $u?->email }}</div>
                    </div>
                </div>
                <ul class="cs-dash-nav">
...
<x-media-upload.js/>
<script>
$('#cs-dash-toggle').on('click', function () {
    $('#cs-dash-sidebar').toggleClass('open');
});
</script>
