...
namespace Plugins\PageBuilder\Addons\Tenants\ChefHome\Header;
...
class HeroSection extends PageBuilderBase
{
    ...
    public function admin_render(): string
    {
        ...
        $output .= Text::get(['name' => 'title_accent',      'label' => __('Headline Accent Word (highlighted)'), 'value' => $v['title_accent'] ?? null]);
        ...
        $output .= Image::get(['name' => 'hero_image',       'label' => __('Hero Dish Image (right side)'),     'value' => $v['hero_image'] ?? null, 'dimensions' => '500x500 px (PNG transparent preferred)']);
    ...
    public function frontend_render(): string
    {
        $data = [
            ...
            'title_accent'   => SanitizeInput::esc_html($this->setting_item('title_accent') ?? ''),
        ...
        return self::renderView('tenant.chefhome.header.hero-section', $data);
    }

    public function addon_title(): string
    {
        return __('ChefHome: Hero Section');
    }
