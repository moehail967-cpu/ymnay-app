            ...
            <div class="mc-auth-card">
                <div style="text-align:center;margin-bottom:28px;">
                    <div class="mc-auth-icon">
                        <i class="las la-shield-alt" style="font-size:32px;color:#1A85ED;"></i>
                    </div>
                    ...
                    <p style="font-size:13px;color:#888;">
                        ...
                        <strong id="mc-otp-timer" style="color:#1A85ED;">05:00</strong>.
                    </p>
                </div>
                ...
                <form action="{{ theme_otp_verify_url() }}" method="post">
                    ...
                    <div style="margin-bottom:20px;">
                        <label class="mc-form-label">{{ __('OTP Code') }} <span class="mc-form-required">*</span></label>
                        <input type="text" name="otp" class="mc-form-input" placeholder="{{ __('Enter 6-digit code') }}"
                               maxlength="6" inputmode="numeric" autocomplete="one-time-code"
                               style="font-size:22px;font-weight:800;text-align:center;letter-spacing:.3em;">
                    </div>
                    <button type="submit" id="otp_verify_btn" class="mc-btn mc-btn-primary mc-btn-block" style="padding:13px;font-size:15px;">
                        <i class="las la-check-circle"></i> {{ __('Verify & Sign In') }}
                    </button>
                </form>

                <div style="text-align:center;margin-top:16px;">
                    <button id="mc-otp-resend" class="mc-btn mc-btn-ghost mc-btn-sm" disabled>
...
(function($){
    ...
    var timer = setInterval(function(){
        ...
        $('#mc-otp-timer').text((m < 10 ? '0' : '') + m + ':' + (s < 10 ? '0' : '') + s);
        if(seconds <= 0){ clearInterval(timer); $('#mc-otp-resend').prop('disabled', false); }
    }, 1000);

    $('#mc-otp-resend').on('click', function(){
        var btn = $(this);
