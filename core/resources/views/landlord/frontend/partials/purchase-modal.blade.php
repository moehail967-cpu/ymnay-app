...
<div id="purchaseFlowModal"
    ...
    {{-- Modal Card --}}
    <div class="relative z-10 w-full max-w-3xl max-h-[92vh] flex flex-col bg-white rounded-2xl shadow-2xl overflow-hidden
                opacity-0 translate-y-4 transition-all duration-300 ease-out" id="pmCard">

        {{-- ── HEADER ── --}}
        <div class="relative flex items-center justify-center px-6 py-4 border-b border-gray-100 shrink-0">


            <div class="text-center">
                <h2 class="text-base font-semibold text-secondary font-urbanist" id="pmTitle">{{__('Create Your Shop')}}</h2>
                <p class="text-xs text-gray-400" id="pmSubTitle">{{__('Step 1 of 4 — Choose a plan')}}</p>
            </div>
            <button id="pmClose" class="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition text-gray-400 hover:text-gray-600">
                <i class="ti tabler-x text-lg"></i>
            </button>
        </div>

        {{-- ── STEPPER ── --}}
        <div class="px-6 py-3 border-b border-gray-100 shrink-0 bg-[#F8FAFB]" id="pmStepper">
                    ...
                    <div class="flex items-center pm-step-item" data-step="{{ $si + 1 }}">
                        <div class="flex flex-col items-center">
                            <div class="pm-step-bubble w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold border-2 transition-all duration-300
                                        {{ $si === 0 ? 'bg-primary border-primary text-white' : 'bg-white border-gray-200 text-gray-400' }}">
                            ...
                            <span class="mt-1 text-[10px] font-medium hidden sm:block
                                         {{ $si === 0 ? 'text-primary' : 'text-gray-400' }} pm-step-label">
                                {{ $step['label'] }}
                            </span>
                        </div>
                        @if(!$loop->last)
            ...
            <div class="p-6">
                ...
                <div class="pm-step-content" data-step-content="1">
                    <div class="text-center mb-6">
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{__('Pick the plan that fits your business. You can upgrade later.')}}</p>
                    </div>
                                ...
                                @if(isset($_modal_plans_by_type[$typeVal]) && $_modal_plans_by_type[$typeVal]->count())
                                    <button type="button"
                                            class="pm-plan-tab px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200
                                                   {{ $typeVal == PricePlanTypEnums::MONTHLY ? 'bg-white text-secondary shadow-sm' : 'text-gray-500 hover:text-secondary' }}"
                                            data-type="{{ $typeVal }}">
                                        {{ $typeLabel }}
                                    </button>
                                @endif
                        ...
                        <div class="pm-plan-group px-5 {{ $typeVal != PricePlanTypEnums::MONTHLY ? 'hidden' : '' }}"
                            ...
                            <div class="grid grid-cols-2 gap-3" id="pmPlanGrid-{{ $typeVal }}">
                                @foreach($plans as $plan)
                                    <div class="pm-plan-card relative flex flex-col rounded-xl cursor-pointer bg-white group overflow-hidden"
                                        ...
                                        <div class="p-4 flex flex-col flex-1">
                                            ...
                                            <div class="flex items-baseline gap-1 mb-3">
                                                ...
                                                @if($periodLabel)
                                                    <span class="text-gray-400 text-[11px]">{{ $periodLabel }}</span>
                                                @endif
                                            </div>

                                            {{-- Divider --}}
                                            <div class="border-t border-dashed border-gray-100 mb-3"></div>
                ...
                <div class="pm-step-content hidden" data-step-content="2">
                    <div class="text-center mb-6">
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{__('Pick the look for your store. You can change it later from your dashboard.')}}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4" id="pmThemeGrid">
                        @foreach($_modal_all_themes as $_th_key => $theme)
                            ...
                            <div class="pm-theme-card relative flex flex-col border-2 rounded-xl overflow-hidden cursor-pointer
                                        hover:border-primary transition-all duration-200 bg-white group
                                        {{ $is_default ? 'border-primary' : 'border-gray-200' }}"
...
{{--                                    @endif--}}
                                ...
                                <div class="px-3 py-2 bg-white border-t border-gray-100 text-center">
                    ...
                    {{-- No-theme fallback --}}
                    <div id="pmNoTheme" class="hidden text-center py-10 text-gray-400">
                ...
                <div class="pm-step-content hidden" data-step-content="3">
                    <div class="text-center mb-6">
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{__('Choose a unique name for your store. This will be your store\'s web address.')}}</p>
                    </div>
                    ...
                    <div class="max-w-md mx-auto">
                        ...
                        @else
                                ...
                                <div class="mb-5">
                                    ...
                                    <div class="relative">
                                        <select id="pmShopSelect"
                                                class="w-full appearance-none border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700
                                                       outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary bg-white pr-10 transition">
                            ...
                            <div id="pmNewShopWrap">
                                ...
                                <div class="flex rounded-xl overflow-hidden border border-gray-200 focus-within:ring-2 focus-within:ring-primary/30 focus-within:border-primary transition">
                                    <input type="text" id="pmSubdomainInput"
                                           class="flex-1 px-4 py-3 text-sm text-gray-700 outline-none bg-white placeholder-gray-400"
                                           placeholder="{{ __('e.g. myshop') }}" autocomplete="off">
                                    <div class="flex items-center px-4 bg-[#92E721]/10 border-l border-gray-200 text-sm text-gray-500 whitespace-nowrap shrink-0">
                                        .{{ $_modal_base_url }}
                                    </div>
                                </div>
                ...
                <div class="pm-step-content hidden" data-step-content="4">
                    <div class="text-center mb-6">
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{__('Choose your payment method and confirm your order.')}}</p>
                    </div>
                    ...
                    <div class="max-w-lg mx-auto">
                        ...
                        <div id="pmModeToggle" class="hidden mb-5 bg-[#F8FAFB] border border-gray-200 rounded-2xl p-1.5 flex gap-1">
                        ...
                        {{-- Summary strip --}}
                        <div class="flex flex-wrap justify-center gap-1 mb-2 p-2 bg-[#F8FAFB] rounded-xl border border-gray-200">
                            ...
                            <div class="flex items-center gap-2 text-sm">
                                ...
                                <span class="text-gray-500">{{__('Theme:')}}</span>
                            ...
                            <div class="flex items-center gap-2 text-sm">
                                ...
                                <span class="text-gray-500">{{__('Store:')}}</span>
                        ...
                        <div class="mb-5" id="pmPaymentSection">
                            ...
                            <div class="pm-manual-fields hidden mt-4 space-y-3">
                                @if($_modal_manual_gw)
                                    <p class="text-xs text-blue-700 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
                                        {{ json_decode($_modal_manual_gw->credentials)->description ?? '' }}
                                    </p>
                                @endif
                                <input type="text" id="pmTransactionId"
                                       class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 placeholder-gray-400
                                ...
                                <input type="file" id="pmTransactionFile"
                                       class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 outline-none
                            ...
                            <div id="pmCouponWrap" class="hidden">
                                <div class="flex gap-2">
                                    <input type="text" id="pmCouponInput"
                                           class="flex-1 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 placeholder-gray-400
                                                  outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition"
                    ...
                    <div class="flex flex-col items-center justify-center py-8 px-4 text-center">
                        ...
                        <div class="w-full max-w-sm mb-6">
                            <div class="flex items-center justify-between text-xs mb-1.5">
                                ...
                                <span id="pmProgressEta" class="text-gray-400 italic">{{__('Estimated: 10–20 seconds')}}</span>
                            </div>
                        ...
                        <div class="w-full max-w-sm space-y-2.5 mb-6 text-left">
                            ...
                            <div class="pm-pstep flex items-center gap-3" data-pstep="{{ $n }}">
                                ...
                                <span class="text-sm text-gray-400 pm-pstep-label transition-colors duration-300">{{ $label }}</span>
                            </div>
                            @endforeach
                        </div>
        ...
        {{-- ── FOOTER NAVIGATION ── --}}
        <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 shrink-0 bg-white" id="pmFooter">
            <button type="button" id="pmBackBtn"
                    class="flex items-center gap-2 px-5 py-2.5 border border-gray-200 text-gray-600 rounded-xl text-sm font-semibold
                           hover:bg-gray-50 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    disabled>
            ...
            <div class="flex items-center gap-3">
                <span class="text-xs text-gray-400 sm:hidden" id="pmMobileStepIndicator">1 / 4</span>
...
.pm-plan-select-btn {
    color: #0C4D54;
    border: 1.5px solid rgba(12,77,84,0.25);
...
.pm-plan-card.pm-selected {
    box-shadow: 0 0 0 2px #0C4D54, 0 8px 24px rgba(12,77,84,0.14) !important;
    background: linear-gradient(150deg, #f0fafa 0%, #ffffff 70%);
...
.pm-step-item[data-completed="true"] .pm-step-label { color: #0C4D54; }
.pm-step-item[data-completed="true"] .pm-connector  { background-color: #0C4D54; }
.pm-step-item[data-active="true"] .pm-step-bubble {
    background-color: #0C4D54; border-color: #0C4D54; color: white;
}
.pm-step-item[data-active="true"] .pm-step-label { color: #0C4D54; font-weight: 600; }
...
.pm-page-btn:hover { border-color: #0C4D54; color: #0C4D54; }
.pm-page-btn.active {
    background: #0C4D54;
    border-color: #0C4D54;
    color: #fff;
...
.pm-page-btn-arrow:hover:not(:disabled) { border-color: #0C4D54; color: #0C4D54; }
...
.pm-gateway-area .payment_getway_image ul li:hover { border-color: #0C4D54; }
...
.pm-gateway-area .payment_getway_image ul li.selected { border-color: #0C4D54; }
...
.pm-pstep.pstep-active .pm-pstep-ico   { border-color: #0C4D54; background-color: #f0fafa; }
.pm-pstep.pstep-active .pm-pstep-pending { display: none !important; }
.pm-pstep.pstep-active .pm-pstep-spin  { display: block !important; }
.pm-pstep.pstep-active .pm-pstep-label { color: #0C4D54; font-weight: 600; }

.pm-pstep.pstep-done .pm-pstep-ico     { border-color: #0C4D54; background-color: #0C4D54; }
...
(function ($) {
    ...
    function showStep(n) {
        ...
        $('.pm-connector').each(function (i) {
            $(this).css('background-color', (i + 1) < n ? '#0C4D54' : '#e5e7eb');
        });
