                            ...
                            <div style="aspect-ratio:16/9;overflow:hidden;background:var(--sz-red-light);position:relative;">
                                ...
                                @else
                                    <div style="display:flex;align-items:center;justify-content:center;height:100%;">
                                        <i class="mdi mdi-dumbbell" style="font-size:48px;color:var(--sz-red);opacity:.3;"></i>
                                    </div>
                                @endif
                                @if($post->category())
                ...
                @empty
                    <div class="col-12">
                        <div style="text-align:center;padding:60px 20px;background:var(--sz-white);border:2px dashed var(--sz-border);border-radius:var(--sz-radius-xl);">
                            ...
                            <p style="color:var(--sz-muted);">{{ __('No posts found in this category') }}</p>
                        </div>
                    </div>
                @endforelse
            </div>
            ...
            <div class="sz-sidebar" style="margin-bottom:24px;">
                ...
                <div class="sz-sidebar-block">
                    <ul style="list-style:none;margin:0;padding:0;">
                        ...
                        @php $is_active = ($cat->name() === ($category_name ?? '')); @endphp
                        <li style="border-bottom:1px solid var(--sz-border);padding:8px 0;display:flex;align-items:center;justify-content:space-between;">
                            <a href="{{ $cat->url() }}"
                               style="font-family:var(--sz-font-head);font-size:13px;color:{{ $is_active ? 'var(--sz-red)' : 'var(--sz-dark)' }};text-decoration:none;text-transform:uppercase;letter-spacing:1px;display:flex;align-items:center;gap:6px;transition:color .2s;font-weight:{{ $is_active ? '700' : '400' }};"
                               onmouseover="this.style.color='var(--sz-red)'"
                               onmouseout="this.style.color='{{ $is_active ? 'var(--sz-red)' : 'var(--sz-dark)' }}'">
