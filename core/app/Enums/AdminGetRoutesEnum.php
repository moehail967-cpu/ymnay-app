...
enum AdminGetRoutesEnum
{
    public static function landlordAdminRoutes():  array
    {
        ...
        $pluginRoutes = [];
        foreach (app(MenuRegistry::class)->getMenus('landlord') as $menu) {
            $r = $menu['route'] ?? '';
            if ($r && !str_starts_with($r, 'tenant.') && Route::has($r)) {
                $pluginRoutes[$r] = $menu['label'];
            }
            foreach ($menu['children'] ?? [] as $child) {
                $cr = $child['route'] ?? '';
                if ($cr && !str_starts_with($cr, 'tenant.') && Route::has($cr)) {
                    $pluginRoutes[$cr] = $child['label'];
                }
            }
        }
    ...
    public static function tenantAdminRoutes():  array
    {
        ...
        $exclude = ['cloud'];
        $pluginRoutes = [];
        foreach (app(MenuRegistry::class)->getMenus('tenant') as $menu) {
            $r = $menu['route'] ?? '';
            if ($r && str_starts_with($r, 'tenant.') && Route::has($r) && empty(array_intersect(explode('.', $r), $exclude))) {
                $pluginRoutes[$r] = $menu['label'];
            }
            foreach ($menu['children'] ?? [] as $child) {
                $cr = $child['route'] ?? '';
                if ($cr && str_starts_with($cr, 'tenant.') && Route::has($cr) && empty(array_intersect(explode('.', $cr), $exclude))) {
                    $pluginRoutes[$cr] = $child['label'];
                }
            }
        }
