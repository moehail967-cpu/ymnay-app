...
class TenantFrontendController extends Controller
{
    ...
    public function homepage()
    {
        ...
        if ($page_post && $page_post->use_page_builder) {
        ...
        return themeView('frontend-home', ['page_post' => $page_post]);
    }
...
//        ]);
    ...
    public function shop_page(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $product_object->where('name', 'LIKE', "%{$search}%");
        }

        if ($request->filled('category')) {
            $slugs = array_values(array_filter(explode(',', $request->category)));
            $product_object->whereHas('category', function ($query) use ($slugs) {
        ...
        if ($request->has('min_price') && $request->has('max_price')) {
            ...
            $product_object->where(function ($q) use ($min_price, $max_price) {
                $q->whereBetween('sale_price', [$min_price, $max_price])
                  ->orWhere(function ($q2) use ($min_price, $max_price) {
                      $q2->whereNull('sale_price')->whereBetween('price', [$min_price, $max_price]);
                  });
            });
        }
    ...
    public function shop_search(Request $request)
    {
        ...
        // S20 T7: include variant attributes, color, and size in search
        $product_object = Product::with('badge', 'campaign_product', 'taxOptions')
            ->where('status_id', 1)
            ->where(function ($q) use ($search) {
                $q->where("name", "LIKE", "%" . $search . "%")
                  ->orWhere("sale_price", $search)
                  ->orWhereHas('inventoryDetail', function ($q2) use ($search) {
                      $q2->where('attributes', 'LIKE', '%' . $search . '%')
                         ->orWhereHas('productColor', fn($c) => $c->where('name', 'LIKE', '%' . $search . '%'))
                         ->orWhereHas('productSize', fn($s) => $s->where('name', 'LIKE', '%' . $search . '%'));
                  });
            });
...
//        dd($product_id);
    ...
    public function add_to_cart(Request $request): JsonResponse
    {
        ...
        // Sum quantities already in cart to prevent cumulative over-stock
        $existing_variant_qty = 0;
        $existing_product_qty = 0;
        foreach (Cart::instance("default")->content() as $cart_item) {
            if ((string) $cart_item->id === (string) $request->product_id) {
                $existing_product_qty += $cart_item->qty;
                $cart_variant_id = $cart_item->options->variant_id ?? null;
                if ((string) $cart_variant_id === (string) ($request->product_variant ?: null)) {
                    $existing_variant_qty += $cart_item->qty;
                }
            }
        }
        $total_variant_qty = $existing_variant_qty + $request->quantity;
        $total_product_qty = $existing_product_qty + $request->quantity;

        if ($product_inventory_details && $total_variant_qty > $product_inventory_details->stock_count) {
            return response()->json([
                'type' => 'warning',
                'quantity_msg' => __('Requested quantity is not available. Only available quantity is added to cart')
            ]);
        } elseif ($product_inventory && $total_product_qty > $product_inventory->stock_count) {
            return response()->json([
                'type' => 'warning',
                'quantity_msg' => __('Requested quantity is not available. Only available quantity is added to cart')
            ]);
        ...
        if (!empty($product->campaign_product)) {
            $cp      = $product->campaign_product;
            $product = Product::where('id', $request->product_id)->withSum('taxOptions','rate')->first();

            $product_left = $cp->units_for_sale !== null ? $cp->units_for_sale - (int) $cp->sold_count : null;
        } else {
        ...
        if ($product_left !== null && !($request->quantity <= $product_left)) {
            return response()->json([
                'type' => 'warning',
        ...
        try {
            ...
            $final_sale_price = (float) apply_filters('nazmart:product_price', $sale_price + $additional_price, $product);
            ...
            if ($cart_data['product_variant']) {
                ...
                // T6: prefer JSON attributes column; fall back to legacy attribute relation
                if ($product_detail && !empty($product_detail->attributes)) {
                    $product_attributes = $product_detail->attributes;
                    $color_name = $product_detail->productColor?->name ?? '';
                    $size_name  = $product_detail->productSize?->name ?? '';
                } else {
                    $size_name  = Size::find($cart_data['selected_size']  ?? 0)?->name ?? '';
                    $color_name = Color::find($cart_data['selected_color'] ?? 0)?->name ?? '';
                    $product_attributes = $product_detail?->attribute?->pluck('attribute_value', 'attribute_name')->toArray() ?? [];
                }
            ...
            $options['sale_price_raw'] = $sale_price + $additional_price;
            $options['type'] = ProductTypeEnum::PHYSICAL;
            $options["tax_options_sum_rate"] = $product->tax_options_sum_rate ?? 0;

            $cart_item = apply_filters('nazmart:before_add_to_cart', [
            ...
            if ($cart_item === false) {
                return response()->json(['type' => 'error', 'error_msg' => __('Item cannot be added to cart.')]);
            }

            Cart::instance("default")->add(['id' => $cart_item['product_id'], 'name' => $cart_item['name'], 'qty' => $cart_item['qty'], 'price' => $cart_item['price'], 'weight' => '0', 'options' => $cart_item['options']]);

            do_action('nazmart:after_add_to_cart', $cart_item, Cart::instance("default"));
...
//    }
    ...
    public function wishlist_page()
    {
        ...
        if (empty($username)) {
            ...
            $stock_map = [];
            return themeView('shop.cart.cart_page', compact('cart_data', 'wishlist', 'stock_map'));
        }
        ...
        $product_ids = $cart_data->pluck('id')->map(fn($id) => (int)$id)->toArray();
        $stock_map = ProductInventory::whereIn('product_id', $product_ids)
            ->get()
            ->keyBy('product_id')
            ->map(fn($inv) => (int)$inv->stock_count > 0)
            ->toArray();
    ...
    public function cart_update_ajax(Request $request)
    {
        ...
        if ($product_data->options->type == ProductTypeEnum::PHYSICAL) {
            ...
            $quantity_error = null;
            if ($product_inventory_details && $request->quantity > $product_inventory_details->stock_count) {
                $quantity_error = __('Requested quantity is not available. Only available quantity is added to cart');
            } elseif ($product_inventory && $request->quantity > $product_inventory->stock_count) {
                $quantity_error = __('Requested quantity is not available. Only available quantity is added to cart');
            }
            ...
            if (!$quantity_error) {
                ...
                $cp = $product->campaign_product ?? null;
                $product_left = ($cp && $cp->units_for_sale !== null) ? $cp->units_for_sale - (int) $cp->sold_count : null;
                if ($product_left !== null && !($request->quantity <= $product_left)) {
                    $quantity_error = __('Requested amount can not be cart. Campaign product stock limit is over!');
                }
            }

            if ($quantity_error) {
                // Return current cart markup so JS can revert the input to the actual cart quantity
                $cart_data = Cart::content();
                return response()->json([
                    'type' => 'warning',
                    'quantity_msg' => $quantity_error,
            ...
            try {
                ...
                do_action('nazmart:cart_quantity_changed', Cart::get($rowId), $qty, Cart::instance("default"));
                DB::commit();
    ...
    {
        do_action('nazmart:cart_cleared', Cart::instance("default"));
        Cart::destroy();
        return back();
    }
    ...
    public function cart_remove_product_ajax(Request $request): JsonResponse
    {
        ...
        $removedItem = Cart::get($request->product_hash_id);
        Cart::remove($request->product_hash_id);
        do_action('nazmart:cart_item_removed', $removedItem, Cart::instance("default"));
    ...
    public function wishlist_move_product_ajax(Request $request): JsonResponse
    {
        ...
        // Add to default cart but keep item in wishlist
        ...
        return response()->json([
            'type' => 'success',
            'msg' => __('Product added to cart'),
            'empty_cart' => Cart::count() == 0 ? themeView('shop.cart.cart_empty')->render() : ''
        ]);
    }

    public function wishlist_remove_by_product_id(Request $request): JsonResponse
    {
        $request->validate(['product_id' => 'required|integer']);
        if (!Auth::guard("web")->check()) {
            return response()->json(['type' => 'error', 'msg' => __('Please login first')]);
        }
        $username = Auth::guard("web")->user()->id;
        $product_id = (int)$request->product_id;
        Cart::instance("wishlist")->restore($username);
        $found = Cart::instance("wishlist")->search(fn($item) => (int)$item->id === $product_id);
        ...
        return response()->json([
            'type'  => 'success',
            'msg'   => __('Removed from wishlist'),
            'count' => Cart::instance("wishlist")->count(),
        ]);
    }

    public function product_stock_status(Request $request): JsonResponse
    {
        ...
        $stock = ProductInventory::whereIn('product_id', $ids)
            ->get()
            ->keyBy('product_id')
            ->map(fn($inv) => (int)$inv->stock_count > 0)
            ->toArray();
    ...
    public function wishlist_ids(): JsonResponse
    {
        if (!Auth::guard("web")->check()) {
        ...
        $username = Auth::guard("web")->user()->id;
        Cart::instance("wishlist")->restore($username);
        $ids = Cart::instance("wishlist")->content()
    ...
    public function wishlist_merge(Request $request): JsonResponse
    {
        if (!Auth::guard("web")->check()) {
            return response()->json(['type' => 'error', 'msg' => 'Not authenticated']);
        }
        $request->validate(['ids' => 'nullable|array', 'ids.*' => 'integer']);
        $ids = $request->ids ?? [];
        if (empty($ids)) {
            return response()->json(['type' => 'success', 'msg' => 'Nothing to merge']);
        }
        $username = Auth::guard("web")->user()->id;
        Cart::instance("wishlist")->restore($username);
        $existing_ids = Cart::instance("wishlist")->content()
            ->map(fn($item) => (int)$item->id)
            ->toArray();
        foreach ($ids as $product_id) {
            ...
            Cart::instance("wishlist")->add([
                ...
                'options' => [
                    ...
                    'size_name'       => '',
                    'attributes'      => '',
                    'used_categories' => ['category' => $category, 'subcategory' => $subcategory],
                ],
            ]);
            $existing_ids[] = $product_id;
        }
        Cart::instance("wishlist")->store($username);
        return response()->json(['type' => 'success', 'msg' => __('Wishlist synced!')]);
    }

    public function wishlist_save_for_later(Request $request): JsonResponse
    {
        $request->validate(['row_id' => 'required']);
        if (!Auth::guard("web")->check()) {
            return response()->json(['type' => 'error', 'msg' => __('Please login to save items for later')]);
        }
        $cartItem = Cart::instance("default")->get($request->row_id);
        if (!$cartItem) {
            return response()->json(['type' => 'error', 'msg' => __('Cart item not found')]);
        }
        $username = Auth::guard("web")->user()->id;
        Cart::instance("wishlist")->restore($username);
        $existing = Cart::instance("wishlist")->search(function ($item) use ($cartItem) {
            return $item->id == $cartItem->id &&
                ($item->options->variant_id ?? '') == ($cartItem->options->variant_id ?? '');
        });
        if ($existing->isEmpty()) {
            Cart::instance("wishlist")->add([
                'id'     => (int)$cartItem->id,
...
//        }
    ...
    public function sync_product_total_wth_shipping_method(Request $request)
    {
        ...
        $raw_shipping_charge = (float) ($selected_shipping_method?->options?->cost ?? 0);
        $shipping_charge     = (float) apply_filters('nazmart:product_price', $raw_shipping_charge, null);
...
//        return response()->json(['type' => 'danger', 'coupon_amount' => 0]);
        ...
        $product_tax = $data['product_tax'];
        $shipping_cost = (float) apply_filters('nazmart:product_price', (float) $data['shipping_cost'], null);
        $taxed_price = ((double)$subtotal * $product_tax) / 100;
...
// You can change the layout, design, and name of the website. Also, order management, blog settings, tax manage, mobile app management, location settings, etc.
    ...
    // S20 T4: "Notify me when back in stock"
    public function stock_notify_subscribe(Request $request)
    {
        $request->validate([
        ...
        \App\Models\StockNotification::updateOrCreate(
            [
                'product_id' => $request->product_id,
                'variant_id' => $request->variant_id ?: null,
                'email'      => $request->email,
            ],
            ['name' => $request->name, 'notified' => false, 'notified_at' => null]
        );

        return response()->json(['msg' => __('You will be notified when this product is back in stock.'), 'type' => 'success']);
    }
    ...
    public function showTenantRegistrationForm()
    {
        ...
        return themeView('user.register', compact('countries'));
    }
    ...
    public function liveCity(Request $request)
    {
        $keyword = $request->keyword;
    ...
    protected function tenant_user_create(Request $request)
    {
        $request->validate([
            ...
            'phone' => ['nullable', 'numeric', 'unique:users,mobile'],
        ...
        // Log the user in immediately — must happen outside the mail try/catch
        // so a mail failure does not prevent the session from being established.
...
//        dd($product_id);
    ...
    public static function productVariant($slug)
    {
        ...
        // Sprint 19 T14: process all inventory detail rows for handle_variants products.
        // Each row's full attribute set = color name + size name + any JSON custom attrs.
        // Previously this block only read the `attributes` JSON column, so Color and Size
        // were missing from attribute_store, breaking the JS variant selector.
        $json_attr_ids = [];
        // Treat as new-style variant product if flag is set OR any detail row has a hash
        // (hash is always written by the combination grid — handles products where
        // handle_variants=0 in DB because the product was never re-saved after migration)
        $hasNewStyleVariants = $product->handle_variants
            || $product->inventoryDetail?->contains(fn($d) => !is_null($d->hash));

        if ($hasNewStyleVariants) {
            foreach ($product->inventoryDetail ?? [] as $detail) {
                $attrs = [];

                // Color and Size go into $attrs only (for JS hash matching).
                // They are NOT added to $available_attributes because every theme renders
                // them through dedicated $productColors / $productSizes swatch sections.
                // Adding them here would cause duplicate rendering in all themes.
                if ($detail->color && $detail->productColor) {
                    $attrs['Color'] = $detail->productColor->name;
                }

                if ($detail->size && $detail->productSize) {
                    $attrs['Size'] = $detail->productSize->name;
                }

                // Include JSON custom attribute axes (RAM, Material, etc.)
                // Cast to string: JSON column may store numeric values (e.g. 3 not "3").
                // PHP json_encode preserves the type, which breaks the MD5 hash match in JS
                // where all values are always strings (read from data attributes).
                foreach ($detail->attributes ?? [] as $k => $v) {
                    $key = ucfirst(strtolower($k));
                    $attrs[$key] = (string) $v;
                    $available_attributes[$key][(string) $v] = 1;
                }

                // Include legacy custom attributes from product_inventory_detail_attributes table
                foreach ($detail->attribute ?? [] as $legacyAttr) {
                    $key = ucfirst(strtolower($legacyAttr->attribute_name ?? ''));
                    $val = $legacyAttr->attribute_value ?? null;
                    if ($key && $val !== null && $val !== '') {
                        $attrs[$key] = $val;
                        $available_attributes[$key][$val] = 1;
                    }
                }

                if (empty($attrs)) continue;

                $json_attr_ids[] = $detail->id;
                $sorted = $attrs;
                ksort($sorted);
                $product_inventory_set[] = $attrs;
                $img_data = get_attachment_image_by_id($detail->image ?? null);
                $additional_info_store[md5(json_encode($sorted))] = [
                    'pid_id'           => $detail->id,
                    'additional_price' => calculatePrice($detail->additional_price ?? 0, $product),
                    'stock_count'      => $detail->stock_count ?? 0,
                    'image'            => $img_data['img_url'] ?? '',
                ];
            }
        }

        foreach ($all_included_attributes as $id => $included_attributes) {
            if (in_array($id, $json_attr_ids)) continue; // already processed via JSON attributes
        ...
        if (!empty($colorAndSizes)) {
            foreach ($colorAndSizes as $inventory) {
                ...
                // skip rows already handled via JSON attributes
                if (in_array($product_id, $json_attr_ids)) continue;
    ...
    public function topbar_search(Request $request)
    {
        ...
        // S20 T7: include variant attributes, color, and size in topbar search
        $safe = htmlspecialchars(strip_tags($search));
        $product_object = Product::with('badge', 'campaign_product')
            ->where('status_id', 1)
            ->where(function ($q) use ($safe) {
                $q->where("name", "LIKE", "%" . $safe . "%")
                  ->orWhere("sale_price", $safe)
                  ->orWhereHas('inventoryDetail', function ($q2) use ($safe) {
                      $q2->where('attributes', 'LIKE', '%' . $safe . '%')
                         ->orWhereHas('productColor', fn($c) => $c->where('name', 'LIKE', '%' . $safe . '%'))
                         ->orWhereHas('productSize', fn($s) => $s->where('name', 'LIKE', '%' . $safe . '%'));
                  });
            })
...
//    }
    ...
    public function category_products($slug)
    {
        ...
        $product_object = Product::whereIn('id', $products_id)->paginate(12);
