...
@section('title') {{ __('Beauty Blog') }} @endsection
@section('page-title') {{ __('Beauty Blog') }} @endsection

@section('content')
<div style="background:var(--gl-gold-pale);border-bottom:1px solid var(--gl-border);padding:36px 0 28px;">
    <div class="container">
        <h2 style="font-size:26px;font-weight:300;color:var(--gl-dark);margin-bottom:8px;letter-spacing:-.3px;">{{ __('Beauty & Skincare Blog') }}</h2>
                ...
                @forelse(theme_blogs() as $post)
                    <div class="col-md-6">
                        <div style="background:#fff;border:1px solid var(--gl-border);border-radius:var(--gl-radius);overflow:hidden;box-shadow:var(--gl-shadow);transition:box-shadow .2s,transform .2s;"
                            ...
                            <div style="aspect-ratio:16/9;overflow:hidden;background:var(--gl-blush);position:relative;">
                                @if($post->has_image())
                                    <a href="{{ $post->url() }}">
                ...
                @empty
                    <div class="col-12">
                        <div style="text-align:center;padding:60px 20px;background:#fff;border-radius:var(--gl-radius);border:1px dashed var(--gl-border);">
                            ...
                            <p style="color:var(--gl-muted);">{{ __('No Blog Available') }}</p>
                        </div>
                    </div>
                @endforelse
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div style="background:#fff;border:1px solid var(--gl-border);border-radius:var(--gl-radius);padding:24px;margin-bottom:24px;box-shadow:var(--gl-shadow);">
                ...
                <form action="{{ theme_blog_search_url() }}" method="GET">
                    <div style="display:flex;border:1.5px solid var(--gl-border);border-radius:var(--gl-radius);overflow:hidden;transition:border-color .2s;" onfocus-within="this.style.borderColor='var(--gl-gold)'">
            ...
            <div style="background:#fff;border:1px solid var(--gl-border);border-radius:var(--gl-radius);padding:24px;margin-bottom:24px;box-shadow:var(--gl-shadow);">
                ...
                <ul style="list-style:none;margin:0;padding:0;">
                    ...
                    <li style="border-bottom:1px dashed var(--gl-border);padding:8px 0;">
                        <a href="{{ $cat->url() }}" style="display:flex;align-items:center;justify-content:space-between;font-size:13px;color:var(--gl-dark);text-decoration:none;"
                           onmouseover="this.style.color='var(--gl-gold)'" onmouseout="this.style.color='var(--gl-dark)'">
                            <span>{{ $cat->name() }}</span>
