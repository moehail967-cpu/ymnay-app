...
<div class="container" style="padding-top:28px; padding-bottom:60px;">
    <div class="row g-5">
        ...
        <div class="col-lg-5">
            {{-- Main image --}}
            <div class="fm-product-main-img" id="shop_details_gallery_slider">
                <div class="shop-details-thums long-img" data-src="{{ $main_img_url }}">
                    @if($main_img_url)
                        <img id="fm-main-product-img" src="{{ $main_img_url }}" alt="{{ $product->name }}" style="width:100%;height:100%;object-fit:cover;">
                    @else
            ...
            {{-- Thumbnails --}}
            <div class="fm-product-thumbs mt-3">
                @foreach($image_array as $imgId)
                    @php $tdata = get_attachment_image_by_id($imgId, 'grid'); $turl = $tdata['img_url'] ?? null; @endphp
                    <div class="fm-product-thumb {{ $loop->first ? 'active' : '' }}" onclick="fmSwitchImg(this, '{{ $turl }}')">
                        @if($turl)<img src="{{ $turl }}" alt="">@else<span style="font-size:22px;">🥦</span>@endif
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="col-lg-7">
            @if($product->badge)
                <div class="fm-product-badge">{{ $product->badge->name }}</div>
            @endif
            ...
            <div class="mb-4 d-flex align-items-baseline gap-2 flex-wrap">
                <span class="fm-product-price-sale"
                      id="price"
                ...
                @if($deleted_price)
                    <span class="fm-product-price-orig">{{ amount_with_currency_symbol($deleted_price) }}</span>
                @endif
                @if($discount)
            ...
            <div class="fm-product-form">
                @include(include_theme_path('shop.product_details.partials.product-options'))
            </div>

            {{-- Trust pills --}}
            <div class="d-flex gap-3 flex-wrap mt-3" style="font-size:12px;color:var(--fm-muted);">
                <span><i class="las la-shield-alt" style="color:var(--fm-green);"></i> {{ __('Secure Checkout') }}</span>
                <span><i class="las la-sync" style="color:var(--fm-orange);"></i> {{ __('Easy Returns') }}</span>
                <span><i class="las la-motorcycle" style="color:var(--fm-green);"></i> {{ __('Fast Delivery') }}</span>
            </div>
        </div>
    </div>
    ...
    <div style="margin-top:56px;">
        <div class="fm-tab-nav">
            <button class="fm-tab-nav-btn active" data-target="fm-tab-desc">{{ __('Description') }}</button>
            <button class="fm-tab-nav-btn" data-target="fm-tab-reviews">{{ __('Reviews') }}</button>
            <button class="fm-tab-nav-btn" data-target="fm-tab-ship">{{ __('Ship & Return') }}</button>
        </div>

        <div id="fm-tab-desc" class="fm-tab-panel active">
            @include(include_theme_path('shop.product_details.partials.product-description'))
        </div>
        <div id="fm-tab-reviews" class="fm-tab-panel">
            @include(include_theme_path('shop.product_details.partials.product-reviews'))
        </div>
        <div id="fm-tab-ship" class="fm-tab-panel">
    ...
    <div style="margin-top:56px;">
        <h2 style="font-size:22px;font-weight:800;color:var(--fm-dark);margin-bottom:20px;">{{ __('You Might Also Like') }}</h2>
        <div class="row g-3">
                ...
                <div class="col-6 col-md-3">
                    <div class="fm-card">
                        <div class="fm-card-img">
                            ...
                            @else
                                <a href="{{ theme_product_url($rp->slug) }}" class="fm-img-ph">🥦</a>
                            @endif
                        </div>
                        <div class="fm-card-body">
                            <div class="fm-card-name"><a href="{{ theme_product_url($rp->slug) }}">{{ \Illuminate\Support\Str::words($rp->name, 6) }}</a></div>
                            <div class="fm-card-price">
                                <span class="fm-price-sale">{{ amount_with_currency_symbol($rpdata['sale_price']) }}</span>
                            </div>
                            <a href="{{ theme_product_url($rp->slug) }}" class="fm-btn fm-btn-outline fm-btn-sm w-100" style="justify-content:center;">
...
@section('scripts')
<script>
function fmSwitchImg(el, url) {
    document.querySelectorAll('.fm-product-thumb').forEach(t => t.classList.remove('active'));
    el.classList.add('active');
    var mainImg = document.getElementById('fm-main-product-img');
