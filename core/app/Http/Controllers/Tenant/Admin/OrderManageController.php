...
class OrderManageController extends Controller
{
    ...
    public function all_orders(Request $request)
    {
        ...
        return view(self::ROOT_PATH . 'order-manage-all', compact('all_orders', 'commissionSetting'));
    }
    ...
    public function change_status(Request $request)
    {
        ...
        $existingCommission = CommissionHistory::where('order_id', $order_id)->first();
        ...
        do_action('nazmart:order_status_changed', $order_details, $old_status, $order_details->status);

        if ($order_details->status === 'complete') {
            do_action('nazmart:order_completed', $order_details);
        }
        if ($order_details->status === 'cancel') {
            do_action('nazmart:order_cancelled', $order_details);
        }

// 1️ TRIGGER EVENT only if payment success & commission not created before
        if ($order_details->payment_status === 'success') {
            if (!$existingCommission) {
                event(new OrderCompleted($order_id, tenant()));
            } else {
                $existingCommission->update(['payment_status' => 'success']);

            }
        } elseif ($existingCommission) {
            $existingCommission->update(['payment_status' => 'pending']);

        }
// 2️ UPDATE commission status when order status becomes complete
        if ($existingCommission) {
            if ($order_details->status === 'complete') {
                $status = commissionSettingType() ? 'complete' : 'pending';

            } else {
                $status = 'pending';

            }
            $existingCommission->update(['status' => $status]);
        }
        ...
        if ($order_details->status === 'cancel') {
            ...
            if ($existingCommission) {
                // Update commission status according to order status
    ...
    private function undostock($order)
    {
        ...
        foreach ($ordered_products ?? [] as $product) {
            DB::transaction(function () use ($product) {
                ...
                // Reverse campaign sold_count when an order is cancelled
                $cp = CampaignProduct::where('product_id', $product->product_id)->lockForUpdate()->first();
