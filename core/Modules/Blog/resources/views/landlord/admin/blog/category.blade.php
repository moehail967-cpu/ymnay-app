    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                <i class="mdi mdi-folder-outline text-primary text-base"></i>
            </div>
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Blog Categories')}}</h3>
                <p class="text-xs text-muted">{{__('Manage blog categories')}}</p>
            </div>
        </div>
            ...
            <tbody>
                @foreach($all_blog_category as $data)
                    <tr class="border-b border-main hover:bg-muted transition-colors">
                        ...
                        <td class="px-4 py-4">
                            @php
                                $statusClass = $data->status == \App\Enums\StatusEnums::PUBLISH
                        ...
                        <td class="px-4 sm:px-6 py-4">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button type="button"
                                        class="w-8 h-8 rounded-lg bg-primary-soft border border-main flex items-center justify-center hover:bg-primary hover:text-white hover:border-primary transition-all category_edit_btn"
                                        data-id="{{$data->id}}"
...
{{-- New Category Modal --}}
@can('blog-category-create')
<div id="newCategoryModal" class="hidden fixed inset-0 z-[999] flex items-center justify-center p-4">
    ...
    <div class="relative bg-surface rounded-2xl shadow-xl border border-main w-full max-w-md">
        <div class="flex items-center justify-between px-6 py-4 border-b border-main">
            <h3 class="text-sm font-bold text-dark font-urbanist">{{__('New Blog Category')}}</h3>
            <button type="button" onclick="this.closest('#newCategoryModal').classList.add('hidden')"
            ...
            <div class="px-6 py-5 space-y-4">
                <div>
                    ...
                    <div class="bg-secondary border border-main rounded-xl px-4 py-2.5 focus-within:border-primary transition">
                        <input type="text" name="title" class="w-full bg-transparent text-sm text-dark placeholder-subtle outline-none border-none focus:ring-0 p-0" placeholder="{{__('Category title')}}">
                    </div>
                </div>
                <div>
                    <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Slug')}}</label>
                    <div class="bg-secondary border border-main rounded-xl px-4 py-2.5 focus-within:border-primary transition">
                        <input type="text" name="slug" class="w-full bg-transparent text-sm text-dark placeholder-subtle outline-none border-none focus:ring-0 p-0" placeholder="{{__('category-slug')}}">
                    </div>
                </div>
                <div>
                    <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Status')}}</label>
                    <div class="bg-secondary border border-main rounded-xl px-4 py-2.5 focus-within:border-primary transition">
                        <select name="status" class="w-full bg-transparent text-sm text-dark outline-none border-none focus:ring-0 p-0 cursor-pointer">
...
{{-- Edit Category Modal --}}
@can('blog-category-edit')
<div id="editCategoryModal" class="hidden fixed inset-0 z-[999] flex items-center justify-center p-4">
    <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" onclick="this.parentElement.classList.add('hidden')"></div>
    <div class="relative bg-surface rounded-2xl shadow-xl border border-main w-full max-w-md">
        <div class="flex items-center justify-between px-6 py-4 border-b border-main">
            <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Edit Blog Category')}}</h3>
            <button type="button" onclick="this.closest('#editCategoryModal').classList.add('hidden')"
                    class="w-8 h-8 rounded-lg bg-secondary border border-main flex items-center justify-center text-muted hover:text-danger transition">
                <i class="mdi mdi-close text-base"></i>
            </button>
        </div>
        <form action="#" class="category_edit_modal_form" method="post" enctype="multipart/form-data">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div>
                    <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Status')}}</label>
                    <div class="bg-secondary border border-main rounded-xl px-4 py-2.5 focus-within:border-primary transition">
                        <select name="status" class="edit_status w-full bg-transparent text-sm text-dark outline-none border-none focus:ring-0 p-0 cursor-pointer">
                            <option value="{{\App\Enums\StatusEnums::PUBLISH}}">{{__('Publish')}}</option>
                            <option value="{{\App\Enums\StatusEnums::DRAFT}}">{{__('Draft')}}</option>
                        </select>
                    </div>
