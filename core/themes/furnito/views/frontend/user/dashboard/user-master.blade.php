...
<div class="fn-dash-section">
    <div class="container">

        <button class="fn-dash-mobile-toggle" id="fn-dash-toggle">
            <i class="las la-bars"></i> {{ __('Account Menu') }}
        </button>

        <div class="fn-dash-wrap">

            {{-- Sidebar --}}
            <div class="fn-dash-sidebar" id="fn-dash-sidebar">
                @php $u = auth('web')->user(); @endphp
                <div class="fn-dash-user">
                    <div class="fn-dash-avatar">{{ strtoupper(substr($u?->name ?? 'U', 0, 1)) }}</div>
                    <div class="fn-dash-user-info">
                        <div class="fn-dash-name">{{ $u?->name }}</div>
                        <div class="fn-dash-email">{{ $u?->email }}</div>
                    </div>
                </div>
                <ul class="fn-dash-nav">
...
@section('scripts')
<script>
$('#fn-dash-toggle').on('click', function(){
    $('#fn-dash-sidebar').toggleClass('open');
});
</script>
