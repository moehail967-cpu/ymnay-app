...
//        ]);
        ...
        if (tenant()) {
            $storageFeature = tenant()?->payment_log?->package?->storage_permission_feature;

            if (!is_null($storageFeature) && $storageFeature != -1) {
                $storage_limit = (int) $storageFeature * 1024; // MB to KB
                $directory_size = $this->getTenantDirectorySize() / 1024; // Byte to KB

                if ($directory_size >= $storage_limit) {
                    return response()->json(['message' => __('Your storage limit is over! You can not upload more media files')], 400);
                }
            }
        }
        ...
        try {
            ...
            return response()->json(['message' => 'Upload initialisation failed: ' . $e->getMessage()], 400);
        }

        // check if the upload is success, return JSON error instead of throwing
        if ($receiver->isUploaded() === false) {
            return response()->json(['message' => 'No file was uploaded. Please select a file and try again.'], 400);
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error('MediaUploader chunk receive failed: ' . $e->getMessage());
            return response()->json(['message' => 'File receive failed: ' . $e->getMessage()], 422);
        }

        // check if the upload has finished (in chunk mode it will send smaller files)
        if ($save->isFinished()) {
            try {
                $mediaRecord = $this->insert_media_image($save->getFile(), $request->user_type, $request);
            } catch (\Throwable $e) {
                \Illuminate\Support\Facades\Log::error('MediaUploader insert_media_image failed: ' . $e->getMessage(), [
            ...
            if (!$mediaRecord) {
                return response()->json(['message' => 'File was uploaded but could not be saved. Please try again.'], 500);
            }

            return response()->json([
                'id'        => $mediaRecord->id,
                'url'       => $this->getUploadAssetPath($mediaRecord->path, load_from: $mediaRecord->load_from),
                'img_url'   => $this->getUploadAssetPath($mediaRecord->path, load_from: $mediaRecord->load_from),
                'alt'       => $mediaRecord->alt ?? '',
                'title'     => $mediaRecord->title ?? '',
                'filename'  => $mediaRecord->path,
                'size'      => $mediaRecord->size,
                'mime_type' => $mediaRecord->dimensions ? 'image/' . pathinfo($mediaRecord->path, PATHINFO_EXTENSION) : 'application/octet-stream',
            ]);
        }

        // Chunk received but not yet complete — return progress
...
//        }
    ...
    public function insert_media_image($file, $userType, $request)
    {
        ...
        if (in_array($image_extenstion, ['pdf', 'doc', 'docx', 'txt', 'svg', 'zip', 'csv', 'xlsx', 'xlsm', 'xlsb', 'xltx', 'pptx', 'pptm', 'ppt', 'mp4', 'avi', 'flv'])) {
            ...
            $mediaRecord = MediaUploader::create($imageData);
        }
        ...
        if (in_array($image_extenstion, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
            ...
            $mediaRecord = MediaUploader::find($mediaId);
        }
