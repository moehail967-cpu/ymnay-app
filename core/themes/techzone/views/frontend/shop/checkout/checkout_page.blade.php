...
<div style="background:var(--tz-surface);border-bottom:1px solid var(--tz-border);padding:28px 0 20px;">
    <div class="container">
        <h2 style="font-size:20px;font-weight:700;color:#fff;margin-bottom:8px;">{{ __('Checkout') }}</h2>
...
<div class="container" style="padding-top:28px;">
    ...
    <div style="display:flex;align-items:center;margin-bottom:28px;">
        <div style="display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:var(--tz-radius-sm);background:var(--tz-blue-glow);border:1px solid var(--tz-blue);font-size:12px;font-weight:700;color:var(--tz-blue);">
            <span style="width:20px;height:20px;border-radius:50%;background:var(--tz-blue);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:10px;"><i class="mdi mdi-check"></i></span>
            {{ __('Cart') }}
        </div>
        <div style="flex:1;height:2px;background:var(--tz-blue);"></div>
        <div style="display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:var(--tz-radius-sm);background:var(--tz-blue);font-size:12px;font-weight:700;color:#fff;">
            <span style="width:20px;height:20px;border-radius:50%;background:#fff;color:var(--tz-blue);display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;">2</span>
            {{ __('Checkout') }}
        </div>
        <div style="flex:1;height:2px;background:var(--tz-border);"></div>
        <div style="display:flex;align-items:center;gap:8px;padding:8px 16px;font-size:12px;font-weight:600;color:var(--tz-muted);">
            <span style="width:20px;height:20px;border-radius:50%;border:1px solid var(--tz-border);display:inline-flex;align-items:center;justify-content:center;font-size:10px;">3</span>
            {{ __('Confirmed') }}
        </div>
    </div>
    ...
    <div class="row g-4" style="padding-bottom:72px;">
        ...
        <div class="col-lg-7">
            ...
            <div style="background:var(--tz-card);border:1px solid var(--tz-border);border-radius:var(--tz-radius);padding:24px;margin-bottom:20px;">
                <div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:16px;display:flex;align-items:center;gap:8px;">
                    <i class="mdi mdi-account-outline" style="color:var(--tz-blue);"></i> {{ __('Sign In to Continue') }}
                </div>
                <x-flash-msg/>
                <form id="login_form_order_page">
                    ...
                    <div class="mb-3">
                        <label style="font-size:12px;font-weight:600;color:var(--tz-muted);display:block;margin-bottom:6px;">{{ __('Username') }}</label>
                        <input type="text" name="username" style="{{ $inp }}" placeholder="{{ __('your@email.com') }}">
                    </div>
                    <div class="mb-3">
                        <label style="font-size:12px;font-weight:600;color:var(--tz-muted);display:block;margin-bottom:6px;">{{ __('Password') }}</label>
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
                        ...
                        <a href="{{ theme_forget_password_url() }}" style="font-size:12px;color:var(--tz-blue);text-decoration:none;">{{ __('Forgot Password?') }}</a>
                    </div>
                    <button type="submit" id="login_btn" style="background:var(--tz-blue);color:#fff;border:0;padding:10px 20px;border-radius:var(--tz-radius-sm);font-size:13px;font-weight:700;cursor:pointer;font-family:var(--tz-font);">{{ __('Sign In') }}</button>
                    <p style="margin-top:12px;font-size:12px;color:var(--tz-muted);">
                        {{ __("Don't have an account?") }}
                        <a href="{{ theme_register_url() }}" style="color:var(--tz-blue);font-weight:600;text-decoration:none;">{{ __('Sign up') }}</a>
                    </p>
                </form>
            </div>
            @endif
            ...
            <form action="{{ theme_checkout_url() }}" method="POST" class="checkout-form" enctype="multipart/form-data">
                ...
                <div style="background:var(--tz-card);border:1px solid var(--tz-border);border-radius:var(--tz-radius);padding:24px;margin-bottom:20px;">
                    <div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:16px;display:flex;align-items:center;gap:8px;">
                        <i class="mdi mdi-map-marker-outline" style="color:var(--tz-blue);"></i> {{ __('Delivery Address') }}
                    </div>
                    <div class="row g-3">
                        <div class="col-12">
                        ...
                        <div class="col-md-6">
                            <label style="{{ $lbl }}">{{ __('ZIP / Postal Code') }}</label>
                            <input type="text" name="postal_code" style="{{ $inp }}" maxlength="6" placeholder="{{ __('Postal Code') }}"
                        ...
                        <div class="col-md-6 position-relative">
                            ...
                            <input type="text" name="state" class="stateField live-state-input" style="{{ $inp }}" placeholder="{{ __('State / Province') }}" value="@auth('web'){{ $billing_info ? $billing_info->state?->name : '' }}@endauth">
                            <div class="live-dropdown state-dropdown"></div>
                        </div>
                        <div class="col-md-6 position-relative">
                            <label style="{{ $lbl }}">{{ __('City') }}</label>
                            <input type="text" name="city" class="live-city-input cityField" style="{{ $inp }}" placeholder="{{ __('City') }}" value="@auth('web'){{ $billing_info ? $billing_info->city?->name : '' }}@endauth">
                        ...
                        <div class="col-12">
                            <label style="{{ $lbl }}">{{ __('Order Notes') }} <span style="font-weight:400;color:var(--tz-muted);">({{ __('optional') }})</span></label>
                            <textarea name="message" style="{{ $inp }}height:70px;resize:vertical;" rows="2"
                                      placeholder="{{ __('Special instructions, delivery notes…') }}">{{ old('message') }}</textarea>
                        </div>
                    </div>

                    @if(!theme_is_logged_in())
                    <div class="mt-4">
                        <a href="javascript:void(0)" class="create-accounts" style="color:var(--tz-blue);font-size:12px;text-decoration:none;">
                        ...
                        <div class="checkout-form-open mt-3">
                            <div class="row g-3">
                                <div class="col-12">
                                    ...
                                    <input type="text" name="create_username" style="{{ $inp }}" placeholder="{{ __('Choose a username') }}">
                                </div>
                                <div class="col-md-6">
                                    <label style="{{ $lbl }}">{{ __('Password') }}</label>
                                    <input type="password" name="create_password" style="{{ $inp }}" placeholder="{{ __('Create password') }}">
                                </div>
                                <div class="col-md-6">
                ...
                <div style="background:var(--tz-card);border:1px solid var(--tz-border);border-radius:var(--tz-radius);padding:24px;">
                    <div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:16px;display:flex;align-items:center;gap:8px;">
                        <i class="mdi mdi-credit-card-outline" style="color:var(--tz-blue);"></i> {{ __('Payment Method') }}
                    </div>

                    <div class="mb-3">
                        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;color:var(--tz-text);">
                            <input type="checkbox" id="cash" style="accent-color:var(--tz-blue);width:15px;height:15px;">
                            <i class="mdi mdi-cash" style="font-size:18px;color:var(--tz-green);"></i>
                            {{ __('Cash On Delivery') }}
                        </label>
                    </div>
                    ...
                    <div class="summernot_wrap" style="display:none;">
                        <div class="manual_description mt-2" style="font-size:12px;color:var(--tz-muted);"></div>
                    </div>
        ...
        {{-- Order Summary --}}
        <div class="col-lg-5">
            <div class="tz-order-summary" style="background:var(--tz-card);border:1px solid var(--tz-border);border-radius:var(--tz-radius);padding:24px;position:sticky;top:100px;">
                <div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid var(--tz-border);">
                    {{ __('Order Summary') }}
                </div>
                ...
                <div class="d-flex gap-2 mb-4">
                    ...
                    <input type="text" class="coupon-code flex-grow-1"
                           style="padding:9px 12px;background:var(--tz-mid);border:1px solid var(--tz-border);border-radius:var(--tz-radius-sm);color:var(--tz-text);font-size:13px;font-family:var(--tz-font);outline:none;"
                           placeholder="{{ __('Coupon code') }}" name="used_coupon_display">
                    <button type="button" class="coupon-btn"
                            style="background:var(--tz-blue);color:#fff;border:0;padding:9px 16px;border-radius:var(--tz-radius-sm);font-size:13px;font-weight:700;cursor:pointer;font-family:var(--tz-font);white-space:nowrap;">
                        {{ __('Apply') }}
                    </button>
                </div>
                ...
                <div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--tz-border);">
                    <div style="width:48px;height:48px;border-radius:var(--tz-radius-sm);overflow:hidden;border:1px solid var(--tz-border);flex-shrink:0;background:var(--tz-mid);">
                        {!! render_image_markup_by_attachment_id($data?->options?->image) !!}
                    </div>
                    <div style="flex:1;min-width:0;">
                        <div style="font-size:12px;font-weight:600;color:var(--tz-text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $data->name }} ×{{ $data->qty }}</div>
                        @if($data?->options?->color_name || $data?->options?->size_name || !empty($data?->options?->attributes))
                        <div style="font-size:11px;color:var(--tz-muted);">
                    ...
                    <span style="font-size:13px;font-weight:700;color:var(--tz-blue);white-space:nowrap;">{{ amount_with_currency_symbol($data->price * $data->qty) }}</span>
                </div>
                @endforeach
                ...
                <div class="shipping_method_wrapper">
                    ...
                    @php
                        ...
                        $rowStyle = 'display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--tz-border);font-size:13px;';
                    @endphp

                    <div style="{{ $rowStyle }} margin-top:8px;"><span style="color:var(--tz-muted);">{{ __('Subtotal') }}</span><span style="color:var(--tz-text);font-weight:600;">{{ site_currency_symbol() }}{{ Cart::subtotal() }}</span></div>
                    <div style="{{ $rowStyle }}" class="coupon-price"><span style="color:var(--tz-muted);">{{ __('Coupon Discount (-)') }}</span><span style="font-weight:600;color:var(--tz-green);" class="tz-price">{{ amount_with_currency_symbol(0.00) }}</span></div>
                    <div style="{{ $rowStyle }}"><span style="color:var(--tz-muted);">{{ __('Tax') }}</span><span style="color:var(--tz-text);font-weight:600;">{{ $product_tax }}%</span></div>

                    @if(theme_is_logged_in() && !empty($shipping_methods ?? []))
                    <div style="font-size:11px;font-weight:700;color:var(--tz-muted);text-transform:uppercase;letter-spacing:.5px;padding:10px 0 4px;">{{ __('Shipping') }}</div>
                    @foreach($shipping_methods ?? [] as $key => $method)
                    <div style="{{ $rowStyle }}" data-country="{{ $country ?? '' }}" data-state="{{ $state ?? '' }}">
                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;color:var(--tz-text);margin:0;">
                            <input type="radio" class="shipping_methods" id="shipping-option-{{ $method['id'] }}" name="shipping_method_display"
                                   value="{{ $method['id'] }}" style="accent-color:var(--tz-blue);" {{ $key === 0 ? 'checked' : '' }}>
                            {{ $method['name'] }}
                        </label>
                        <span style="color:var(--tz-text);font-weight:600;">{{ amount_with_currency_symbol($method['options']['cost']) }}</span>
                    </div>
                    @endforeach
                    @endif

                    <div style="{{ $rowStyle }}" class="price-shipping"><span style="color:var(--tz-muted);">{{ __('Shipping Cost (+)') }}</span><span style="font-weight:600;color:var(--tz-text);" class="tz-price">{{ isset($default_shipping) ? amount_with_currency_symbol($default_shipping['options']['cost']) : '--' }}</span></div>
                    <div style="display:flex;justify-content:space-between;padding:14px 0;font-size:16px;font-weight:800;" class="price-total" data-total="{{ $subtotal + $taxed_price }}">
                        <span style="color:var(--tz-text);">{{ __('Total') }}</span>
                        <span style="color:var(--tz-blue);" class="tz-price">{{ site_currency_symbol() }}{{ $total }}</span>
                    </div>
                </div>

                {!! apply_filters('nazmart:cart_summary', '') !!}
                <div style="display:flex;align-items:center;gap:6px;margin-bottom:16px;">
                    <input type="checkbox" id="agree" checked style="accent-color:var(--tz-blue);width:14px;height:14px;">
                    <label for="agree" style="font-size:12px;color:var(--tz-muted);cursor:pointer;line-height:1.4;">
                        {{ __('I agree to the') }}
                        <a href="javascript:void(0)" style="color:var(--tz-blue);text-decoration:none;">{{ __('Terms & Conditions') }}</a>
                    </label>
                </div>

                <button type="button" class="proceed_checkout_btn checkout_disable"
                        style="width:100%;background:var(--tz-blue);color:#fff;border:0;padding:13px;border-radius:var(--tz-radius-sm);font-size:14px;font-weight:700;cursor:pointer;font-family:var(--tz-font);display:flex;align-items:center;justify-content:center;gap:6px;transition:background .2s;"
                ...
                <a href="{{ theme_cart_url() }}"
                   style="display:block;text-align:center;margin-top:10px;padding:11px;border:1px solid var(--tz-border);border-radius:var(--tz-radius-sm);font-size:13px;color:var(--tz-muted);text-decoration:none;font-family:var(--tz-font);transition:border-color .2s,color .2s;"
                   onmouseover="this.style.borderColor='var(--tz-blue)';this.style.color='var(--tz-blue)'"
                   onmouseout="this.style.borderColor='var(--tz-border)';this.style.color='var(--tz-muted)'">
                    {{ __('Return to Cart') }}
                </a>

                <div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:12px;font-size:11px;color:var(--tz-muted);">
                    <i class="mdi mdi-shield-check" style="color:var(--tz-green);"></i>
                    {{ __('Secured & encrypted checkout') }}
                </div>
            </div>
        </div>
...
(function ($) {
    $(document).ready(function () {
        ...
        $(document).on('click', '#login_btn', function (e) {
            e.preventDefault();
            var el = $(this); var form = $('#login_form_order_page');
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status === 'invalid') { el.text('{{ __("Sign In") }}'); form.find('.error-wrap').html('<div style="color:#ff6b6b;font-size:13px;padding:8px 0;">' + data.msg + '</div>'); }
                    else { el.text('{{ __("Redirecting...") }}'); location.reload(); }
                },
                error: function (data) {
                    var errors = data.responseJSON?.errors ?? {}; var msgs = [];
                    $.each(errors, function (k, v) { msgs.push(v); });
                    form.find('.error-wrap').html('<div style="color:#ff6b6b;font-size:13px;padding:8px 0;">' + msgs.join('<br>') + '</div>');
                    el.text('{{ __("Sign In") }}');
                }
            });
        });
        ...
        $(document).on('click', 'input.shipping_methods', function () {
            var method = $(this).val(); var total = $('.price-total').attr('data-total');
            $('.shipping-method').val(method);
            if (total !== undefined) { getShippingMethodBasedTotal(method, $('.coupon-country').val(), $('.coupon-state').val(), total); }
        });
            ...
            $.ajax({ url: '{{ theme_checkout_shipping_ajax_url() }}', type: 'GET',
                ...
                success: function (data) {
                    if (data.type === 'success') {
                        ...
                        $('.price-shipping .tz-price').html(c + data.selected_shipping_method.options.cost);
                        $('.price-total .tz-price').html(c + data.total);
            ...
            $.ajax({ url: '{{ theme_checkout_coupon_ajax_url() }}', type: 'GET',
                ...
                success: function (data) {
                    ...
                    if (data.type === 'error') { toastr.error(data.msg); }
                        ...
                        $('.price-total .tz-price').text(c + data.coupon_amount);
                        $('.coupon-price .tz-price').text(c + data.coupon_price);
        ...
        $(document).on('click', '.proceed_checkout_btn', function (e) {
            ...
            else { toastr.error('{{ __('You need to agree to our Terms & Conditions') }}'); }
        });
