...
@section('title') {{ __('Tech Blog') }} @endsection
@section('page-title') {{ __('Tech Blog') }} @endsection

@section('content')
<div class="tz-page-banner">
    <div class="container tz-page-banner-content">
        <h1>{{ __('TechZone Blog') }}</h1>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div style="background:var(--tz-card);border:1px solid var(--tz-border);border-radius:var(--tz-radius);overflow:hidden;transition:border-color .2s;"
                            ...
                            <div style="aspect-ratio:16/9;overflow:hidden;background:var(--tz-mid);position:relative;">
                                ...
                                @endif
                                @if($post->category())
                                    <span style="position:absolute;top:10px;left:10px;background:var(--tz-blue);color:#fff;font-size:10px;font-weight:700;padding:3px 10px;border-radius:var(--tz-radius-sm);">
                                        {{ $post->category()->name() }}
                                    </span>
                                @endif
                            </div>
                            <div style="padding:16px;">
                                ...
                                <h3 style="font-size:14px;font-weight:700;color:var(--tz-text);margin-bottom:8px;line-height:1.4;">
                                    <a href="{{ $post->url() }}" style="color:inherit;text-decoration:none;"
                                       onmouseover="this.style.color='var(--tz-blue)'" onmouseout="this.style.color='var(--tz-text)'">{!! $post->title() !!}</a>
                                </h3>
                                <p style="font-size:12px;color:var(--tz-muted);margin-bottom:12px;line-height:1.6;">{{ $post->excerpt(16) }}</p>
                                <a href="{{ $post->url() }}"
                                   style="display:inline-flex;align-items:center;gap:4px;font-size:12px;font-weight:700;color:var(--tz-blue);text-decoration:none;"
                ...
                @empty
                    <div class="col-12">
                        <div style="text-align:center;padding:60px 20px;background:var(--tz-card);border-radius:var(--tz-radius);border:1px dashed var(--tz-border);">
                            <i class="mdi mdi-newspaper-variant-outline" style="font-size:48px;color:var(--tz-border);display:block;margin-bottom:12px;"></i>
                            <p style="color:var(--tz-muted);">{{ __('No posts available yet') }}</p>
                        </div>
                    </div>
                @endforelse
            </div>
