...
<form action="{{ route('tenant.admin.mobile.settings.privacy.policy') }}" method="post">
    @csrf
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden">
                ...
                <div class="px-4 sm:px-6 py-4 border-b border-main flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                        <i class="mdi mdi-shield-lock-outline text-primary text-base"></i>
                    </div>
                    <div>
                        <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Mobile Privacy Policy')}}</h3>
                        <p class="text-xs text-muted">{{__('Select the page to display as the privacy policy in the mobile app')}}</p>
                    </div>
                </div>

                <div class="p-4 sm:p-6 space-y-5">
                    <div>
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-2">{{__('Select Privacy Policy Page')}}</label>
                        <div class="flex items-center gap-2.5 bg-secondary border border-main rounded-xl px-4 py-2.5 focus-within:border-primary transition">
                            ...
                            <select name="page" class="flex-1 bg-transparent text-sm text-dark outline-none border-none focus:ring-0 p-0 cursor-pointer">
                                ...
                                @foreach($pages as $item)
                                    <option value="{{ $item->slug }}" {{ get_static_option('mobile_privacy_and_policy') == $item->slug ? 'selected' : '' }}>{{ $item->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div class="bg-surface rounded-xl shadow-main border border-main overflow-hidden sticky top-4">
                ...
                <div class="p-4 space-y-5">
                    <div class="flex items-start gap-2.5 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
                        ...
                        <span class="text-[11px] text-dark leading-relaxed">{{__('Choose which page will be displayed as the privacy policy in the mobile application.')}}</span>
                    </div>
                    <button type="submit"
