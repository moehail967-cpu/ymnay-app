...
class TailwindMenuWithPermission
{
    ...
    private function render_menu_item(string $id, array $item, string $output): string
    {
        ...
        if ($hasChildren) {
            ...
            if ($isParentActive) {
                $btnClasses .= ' text-[#21b777] bg-[#e6f7ef] font-semibold';
            } else {
                $btnClasses .= ' text-slate-600 font-medium hover:text-[#21b777] hover:bg-slate-50';
            }

            $output .= '<button type="button" onclick="toggleSubmenu(this)" class="' . $btnClasses . '">';

            if ($this->has_icon($item)) {
                $iconClass = $isParentActive ? 'text-[#2d6a4f]' : 'text-slate-500';
            ...
            $output .= '<i class="mdi mdi-chevron-down text-xs text-slate-500 transition-transform duration-200 submenu-arrow' . $arrowRotate . '"></i>';
            $output .= '</button>';
            ...
            if ($isActive) {
                $linkClasses .= ' text-[#21b777] bg-[#e6f7ef] font-semibold';
            } else {
                $linkClasses .= ' text-slate-600 font-medium hover:text-[#21b777] hover:bg-slate-50';
            }

            $output .= '<a href="' . $route . '" class="' . $linkClasses . '">';

            if ($this->has_icon($item)) {
                $iconClass = $isActive ? 'text-[#2d6a4f]' : 'text-slate-500';
                $output .= '<i class="' . $item['icon'] . ' text-[18px] ' . $iconClass . '"></i>';
            }
    ...
    private function render_sub_menus(string $id, string $output, bool $hasActiveChild): string
    {
        ...
        $output .= '<ul class="space-y-0.5 border-l border-gray-200 pl-3">';
        ...
        foreach ($all_menu_items as $submenu_id => $sub_menu) {
            ...
            if ($isSubActive) {
                $subClasses .= ' text-[#21b777]  font-semibold';
            } else {
                $subClasses .= ' text-slate-600 hover:text-[#21b777] hover:bg-slate-50';
            }
            ...
            if ($isSubActive) {
                ...
                $output .= '<span class="w-1.5 h-1.5 rounded-full bg-[#2d6a4f] flex-shrink-0"></span>';
            } else {
                $output .= '<a href="' . $subRoute . '" class="' . $subClasses . '">';
                $output .= '<span class="w-1 h-1 rounded-full bg-slate-400 flex-shrink-0"></span>';
            }
