...
<div class="hf-dash-card">
    <div class="hf-dash-card-title"><i class="las la-headset"></i> {{ __('Ticket') }} #{{ $ticket?->id }} — {{ $ticket?->subject }}</div>
    ...
    <form action="{{ theme_user_ticket_reply_url() }}" method="post">
        @csrf
        <label class="hf-form-label" style="margin-bottom:8px;display:block;">{{ __('Your Reply') }}</label>
        <textarea name="message" class="hf-form-input hf-form-textarea" rows="4" style="margin-bottom:12px;" placeholder="{{ __('Type your message…') }}"></textarea>
...
<a href="{{ theme_user_tickets_url() }}" class="hf-btn hf-btn-ghost">
