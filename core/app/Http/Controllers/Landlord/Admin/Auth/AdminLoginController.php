...
class AdminLoginController extends Controller
{
    ...
    {
        $tenant = tenant();
        $tenant_log = \App\Models\PaymentLogs::find($tenant->renewal_payment_log_id);

        if (!$tenant_log) {
            $tenant_log = \App\Models\PaymentLogs::where('tenant_id', $tenant->id)
                ->latest()
                ->first();
        }

        $isActive = $tenant->expire_date && \Illuminate\Support\Carbon::parse($tenant->expire_date)->isFuture();
