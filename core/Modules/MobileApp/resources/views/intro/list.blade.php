...
@section('title') {{__('Mobile Intros')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main mb-6">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Mobile Intros')}}</h3>
                <p class="text-xs text-muted">{{__('Manage intro slides for the mobile app')}}</p>
            </div>
        </div>
        <a href="{{ route('tenant.admin.mobile.intro.create') }}"
    ...
    <div class="tw-table-wrap">
        <table class="w-full text-left" id="introTable">
            <thead>
                <tr class="border-b border-main">
                    ...
                    <th class="px-4 py-3 text-[10px] font-bold text-muted uppercase tracking-widest w-20">{{__('Image')}}</th>
            ...
            @forelse($mobileIntros as $slider)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5">
                        <p class="text-xs text-muted line-clamp-2">{{ Str::words($slider->description, 8) }}</p>
                    </td>
                    <td class="px-4 py-3.5">
                        @if($slider->image_id)
                            @php
                    ...
                    <td class="px-4 py-3.5">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('tenant.admin.mobile.intro.edit', $slider->id) }}"
                            ...
                            <button type="button"
                                    ...
                                    data-url="{{ route('tenant.admin.mobile.intro.delete', $slider->id) }}"
                                    title="{{__('Delete')}}">
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-4 py-8 text-center text-sm text-muted">{{__('No Data Available')}}</td>
                </tr>
            @endforelse
            </tbody>
        </table>
...
@section('scripts')
    ...
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            if ($.fn.DataTable && !$.fn.dataTable.isDataTable('#introTable')) {
