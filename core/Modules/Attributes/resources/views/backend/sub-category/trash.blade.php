...
@section('title') {{__('Trashed Sub Categories')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('Trashed Sub Categories')}}</h3>
                <p class="text-xs text-muted">{{__('Restore or permanently delete sub categories')}}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            ...
            @can('product-category-create')
                <a href="{{route('tenant.admin.product.subcategory.all')}}"
    ...
    <div class="tw-table-wrap">
        <table class="w-full text-left" id="trashSubCategoryTable">
            <thead>
                    ...
                    <td class="px-4 py-3.5">
                        <div class="flex items-center justify-end gap-1.5">
                            @can('product-category-edit')
                                <a href="{{route('tenant.admin.product.subcategory.trash.restore', $category->id)}}"
                                   class="tw-btn-icon tw-btn-icon-approve" title="{{__('Restore')}}">
                                    <i class="mdi mdi-backup-restore"></i>
                                </a>
                            @endcan
                            @can('product-category-delete')
                                <a href="{{route('tenant.admin.product.subcategory.trash.delete', $category->id)}}"
...
@section('scripts')
    <x-datatable.tw-js/>
    @can('product-category-delete')
        <x-bulk-action-js :url="route('tenant.admin.product.subcategory.trash.delete.bulk')"/>
    @endcan
    <script>
    (function ($) {
        "use strict";
        ...
        $(document).ready(function () {
            if ($.fn.DataTable && !$.fn.dataTable.isDataTable('#trashSubCategoryTable')) {
                $('#trashSubCategoryTable').DataTable({
                    "order": [[1, "desc"]],
                    "pageLength": 10,
