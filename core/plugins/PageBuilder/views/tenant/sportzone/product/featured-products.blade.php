...
<section style="background:#fff;padding-top:{{$pt}}px;padding-bottom:{{$pb}}px;">
    <div class="container">
            ...
            <div class="row g-4">
                @foreach($data['products'] as $product)
                    ...
                    <div class="col-6 col-md-4 col-lg-3">
                        <div style="background:#fff;border:2px solid var(--sz-border);border-radius:10px;overflow:hidden;transition:border-color .2s,box-shadow .2s;height:100%;display:flex;flex-direction:column;"
                             onmouseover="this.style.borderColor='var(--sz-red)';this.style.boxShadow='0 8px 28px rgba(198,40,40,.10)'"
                             onmouseout="this.style.borderColor='var(--sz-border)';this.style.boxShadow='none'">

                            {{-- Image --}}
                            <div style="position:relative;overflow:hidden;background:var(--sz-bg);">
                                <a href="{{ $product_url }}">
                                ...
                                {{-- Badge --}}
                                @if($badge)
                                    <span style="position:absolute;top:10px;left:10px;background:var(--sz-red);color:#fff;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;padding:3px 10px;border-radius:4px;">
                                        {{ $badge->name }}
                                    </span>
                                @endif
                                ...
                                <button onclick="addToWishlist({{ $product->id }}, this)"
                                        ...
                                        onmouseover="this.style.background='var(--sz-red-light)'"
                                        onmouseout="this.style.background='#fff'">
                                    <i class="mdi mdi-heart-outline" style="color:var(--sz-red);font-size:16px;"></i>
                                </button>
                            </div>
                            ...
                            <div style="padding:14px;flex:1;display:flex;flex-direction:column;gap:8px;">
                                @if($product->category)
                                    <span style="font-size:11px;font-weight:700;color:var(--sz-red);text-transform:uppercase;letter-spacing:.06em;">{{ $product->category->name }}</span>
                                @endif

                                <a href="{{ $product_url }}" style="font-size:14px;font-weight:700;color:var(--sz-navy);text-decoration:none;line-height:1.4;flex:1;">
                                ...
                                @if($product->rating_count > 0)
                                    <div style="display:flex;align-items:center;gap:4px;">
                                        @for($i=1;$i<=5;$i++)
                                            <i class="mdi mdi-star" style="font-size:12px;color:{{ $i <= round($product->average_rating) ? '#F59E0B' : 'var(--sz-border)' }};"></i>
                                        @endfor
                                        <span style="font-size:11px;color:var(--sz-muted);">({{ $product->rating_count }})</span>
                                    </div>
                                @endif

                                <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:auto;padding-top:8px;border-top:1px solid var(--sz-border);">
                                    <div>
                                        <span style="font-size:16px;font-weight:800;color:var(--sz-navy);">{{ amount_with_currency_symbol($sale_price) }}</span>
                                        @if($has_discount)
                                            <span style="font-size:13px;color:var(--sz-muted);text-decoration:line-through;margin-left:6px;">{{ amount_with_currency_symbol($reg_price) }}</span>
                                        @endif
                                    </div>
                                    <button onclick="addToCart({{ $product->id }})"
                                            title="{{ __('Add to Cart') }}"
                                            style="width:36px;height:36px;border-radius:6px;background:var(--sz-red);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .2s;"
        ...
        @else
            <p class="text-center py-5" style="color:var(--sz-muted);">{{ __('No products found.') }}</p>
        @endif
