...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-6 col-xl-3">
        <div class="tn-stat-card">
            <div class="tn-stat-icon"><i class="mdi {{ $icon }}"></i></div>
            <div>
                <div class="tn-stat-value">{{ $value }}</div>
                <div class="tn-stat-label">{{ $label }}</div>
            </div>
        </div>
    </div>
    @endforeach
...
<div class="tn-dash-box">
    <div style="padding:20px 20px 0;">
        <div class="tn-dash-section-title">
            <i class="mdi mdi-history" style="color:var(--tn-rose);"></i> {{ __('Recent Activity') }}
        </div>
    </div>
        ...
        <table class="tn-dash-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:var(--tn-muted);">{{ $log->created_at->format('d M Y') }}</td>
                    <td style="font-weight:700;color:var(--tn-rose);">{{ amount_with_currency_symbol($log->total_amount ?? 0) }}</td>
                    <td>
                        <span class="tn-status-badge" style="
                            ...
                            color:{{ $s==='complete'||$s==='success' ? '#4CAF50' : ($s==='pending'?'#D4A017':'var(--tn-rose)') }};">
                            {{ __($s) }}
                        </span>
                    </td>
                </tr>
