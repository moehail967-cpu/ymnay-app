...
@section('title') {{__('Digital Product Child Category')}} @endsection
...
<div class="bg-surface rounded-xl shadow-main border border-main">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Digital Child Categories')}}</h3>
                <p class="text-xs text-muted">{{__('Manage digital product child categories')}}</p>
            </div>
        </div>
            ...
            @foreach($all_childcategory ?? [] as $key => $subcategory)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5 hidden lg:table-cell">
                        <span class="tw-pill tw-pill-purple">{{$subcategory->subcategory?->name ?? __('N/A')}}</span>
                    </td>
                    <td class="px-4 py-3.5 hidden lg:table-cell">
                        ...
                        <div class="flex items-center justify-end gap-1.5">
                            @can('product-category-edit')
                                @php
                                    ...
                                    $all_subcategories = \Modules\DigitalProduct\Entities\DigitalSubCategories::where('category_id', $subcategory->category_id)->get();
                                @endphp
                                <button type="button"
                            ...
                            @can('product-category-delete')
                                <button type="button" class="tw-btn-icon tw-btn-icon-danger swal_delete_button"
                                        data-route="{{ route('tenant.admin.digital.product.childcategory.delete', $subcategory->id) }}"
                                        title="{{__('Delete')}}">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            <div class="flex items-center gap-3 px-5 py-4 border-b border-main flex-shrink-0">
                ...
                <div class="flex-1">
                    <h5 class="text-sm font-bold text-dark font-urbanist">{{__('Create Child Category')}}</h5>
                    <p class="text-[11px] text-muted">{{__('Add a new digital product child category')}}</p>
                </div>
            ...
            <form action="{{ route('tenant.admin.digital.product.childcategory.new') }}" method="post" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                @csrf
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <select name="category" class="lnd-input" id="category">
                    ...
                    <div>
                        ...
                        <select name="subcategory" class="lnd-input" id="subcategory">
...
<div id="subcategory_edit_modal" class="fixed inset-0 z-[800] hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            ...
            <form action="{{ route('tenant.admin.digital.product.childcategory.update') }}" method="post" class="flex flex-col flex-1 min-h-0">
                @csrf
                ...
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div class="edit-sub-category-wrapper">
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-1.5">{{__('Sub Category')}} <span class="text-danger">*</span></label>
                        <select name="subcategory" class="lnd-input" id="edit_subcategory">
...
@section('scripts')
    ...
    @can('product-category-delete')
        <x-bulk-action-js :url="route('tenant.admin.digital.product.childcategory.bulk.action')"/>
    @endcan
    <script>
    (function ($) {
        "use strict";
        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '#category', function () {
                ...
                var subcategory_select = $('select#subcategory');
            ...
            $(document).on('change', '#edit_category', function () {
                var category_id = $(this).val();
                var subcategory_select = $('select#edit_subcategory');
            ...
            $(document).on('click', '.subcategory_edit_btn', function () {
                ...
                var subcategory = el.data('subcategory');
                var subcategories = el.data('subcategories');
                ...
                // Populate sub-categories from data attribute
