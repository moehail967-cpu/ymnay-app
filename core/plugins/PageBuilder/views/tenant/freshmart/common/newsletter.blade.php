...
<section style="padding-top:{{$pt}}px; padding-bottom:{{$pb}}px; background:linear-gradient(135deg, var(--fm-dark) 0%, var(--fm-green-deep) 100%); position:relative; overflow:hidden;">
    ...
    <div class="container" style="position:relative;z-index:1;">
        <div class="row justify-content-center">
            <div class="col-lg-7 text-center">
                ...
                @if(!empty($data['title']))
                    <h2 style="font-size:clamp(22px,3vw,32px); font-weight:900; color:#fff; font-family:var(--fm-font-head); margin-bottom:10px;">
                        {{ $data['title'] }}
                    </h2>
                @endif
                ...
                <form class="d-flex flex-column flex-sm-row gap-2 justify-content-center"
                      onsubmit="fmNewsletterSubmit(event, this)">
                    @csrf
                    <input type="email"
                           name="email"
                           required
                    ...
                    <button type="submit"
                            style="white-space:nowrap; background:var(--fm-green-mid); color:#fff; border:none; padding:14px 30px; border-radius:50px; font-weight:700; font-size:14px; cursor:pointer; display:inline-flex; align-items:center; gap:8px; transition:background .2s;"
                            onmouseover="this.style.background='var(--fm-green)'" onmouseout="this.style.background='var(--fm-green-mid)'">
...
<script>
function fmNewsletterSubmit(e, form) {
    e.preventDefault();
    ...
    .then(res => {
        if (res.msg || res.message) {
            ...
            btn.style.background = 'var(--fm-green)';
