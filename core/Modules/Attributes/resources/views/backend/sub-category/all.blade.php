...
@section('title') {{__('Product Sub Category')}} @endsection
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-primary-soft flex items-center justify-center flex-shrink-0">
                <i class="mdi mdi-file-tree-outline text-primary text-base"></i>
            </div>
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Sub Categories')}}</h3>
                <p class="text-xs text-muted">{{__('Manage product sub categories')}}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            @can('product-category-delete')
                <x-bulk-action permissions="product-category-delete"/>
                <a href="{{route('tenant.admin.product.subcategory.trash.all')}}"
            ...
            @foreach($data['all_sub_category'] as $category)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5 hidden md:table-cell">
                        <span class="tw-pill tw-pill-info">{{$category->category?->name ?? __('N/A')}}</span>
                    </td>
                    <td class="px-4 py-3.5">
                        ...
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            @can('product-category-delete')
                                <button type="button" class="tw-btn-icon tw-btn-icon-danger swal_delete_button"
                                        data-route="{{ route('tenant.admin.product.subcategory.delete', $category->id) }}"
                                        title="{{__('Delete')}}">
        ...
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            <div class="flex items-center gap-3 px-5 py-4 border-b border-main flex-shrink-0">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-[11px] text-muted">{{__('Add a new product sub category')}}</p>
                </div>
            ...
            <form action="{{ route('tenant.admin.product.subcategory.new') }}" method="post" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                @csrf
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <select name="category_id" class="lnd-input" id="create_category_id">
...
<div id="category_edit_modal" class="fixed inset-0 z-[800] hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="relative bg-surface rounded-2xl border border-main w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style="box-shadow: 0 24px 48px rgba(0,0,0,0.15);">
            ...
            <form action="{{ route('tenant.admin.product.subcategory.update') }}" method="post" class="flex flex-col flex-1 min-h-0">
                @csrf
                ...
                <div class="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    <div class="edit-category-wrapper">
                        <label class="block text-[10px] font-bold tracking-widest text-muted uppercase mb-1.5">{{__('Category')}} <span class="text-danger">*</span></label>
                        <select name="category_id" class="lnd-input" id="edit_category_id">
                            <option value="">{{__('Select Category')}}</option>
                            @foreach($data['all_category'] as $cat)
                                <option value="{{ $cat->id }}">{{ $cat->name }}</option>
                            @endforeach
...
@section('scripts')
    ...
    @can('product-category-delete')
        <x-bulk-action-js :url="route('tenant.admin.product.subcategory.bulk.action')"/>
    @endcan
    <script>
    (function ($) {
        "use strict";
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.category_edit_btn', function () {
                ...
                modal.find('#edit_category_id').val(el.data('category'));
                ...
                if (imageid) {
                    ...
                    section.find('.tw-media-id-input').val(typeof imageid === 'object' ? imageid.id : imageid);
