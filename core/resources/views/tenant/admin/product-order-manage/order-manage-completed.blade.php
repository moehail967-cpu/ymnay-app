    ...
    <div class="px-4 sm:px-6 py-4 border-b border-main rounded-t-xl flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-success-soft flex items-center justify-center flex-shrink-0">
                <i class="mdi mdi-check-all text-success text-base"></i>
            </div>
            <div>
                <h3 class="text-sm font-bold text-dark font-urbanist">{{__('All Complete Orders')}}</h3>
                <p class="text-xs text-muted">{{__('Successfully completed orders')}}</p>
            </div>
        </div>
        ...
        <table class="w-full text-left" id="all_user_table">
            ...
            @foreach($all_orders as $data)
                <tr class="border-b border-main hover:bg-muted transition-colors">
                    ...
                    <td class="px-4 py-3.5">
                        @if($data->payment_status == 'complete' || $data->payment_status == 'success')
                    ...
                    <td class="px-4 py-3.5">
                        <span class="tw-pill tw-pill-success">{{__(ucfirst(str_replace('_', ' ', $data->status)))}}</span>
                    </td>
