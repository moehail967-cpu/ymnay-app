<div class="el-cart-items {{ $wishlist ? 'el-cart-items--full' : '' }}">
    <div class="el-cart-table-wrap">
        <table class="el-cart-table">
            <thead>
                <tr>
                    <th>{{__('Product')}}</th>
                    <th>{{__('Price')}}</th>
            ...
            @foreach($cart_data as $key => $data)
                ...
                @endphp
                <tr class="el-cart-row" data-product-id="{{$key}}" data-varinat-id="{{$data->variant_id}}">
                    <td data-label="{{__('Product')}}">
                        <div class="el-cart-product">
                            <div class="el-cart-thumb"
                            ...
                            <div>
                                ...
                                @endphp
                                <a href="{{$product_details_route}}" class="el-cart-name">{{$data->name}}</a>
                                <span class="el-cart-type">{{$product_type}}</span>
                    ...
                    <td data-label="{{__('Action')}}" class="el-cart-action" data-product_hash_id="{{$data->rowId}}">
                        <div class="el-cart-action-inner">
                            @if($wishlist)
    ...
    <div class="el-cart-actions mt-4">
        <a href="{{theme_shop_url()}}" class="el-btn el-btn-outline">{{__('Continue Shopping')}}</a>
        @if(!$wishlist)
            <a href="javascript:void(0)" class="el-btn el-btn-ghost clear-cart-btn">{{__('Clear Cart')}}</a>
        @endif
    </div>
</div>
