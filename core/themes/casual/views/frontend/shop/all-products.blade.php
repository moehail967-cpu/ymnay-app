...
<div class="cs-page-banner">
    <div class="container">
        <h1 class="cs-page-banner-title">{!! $page_post->title !!}</h1>
        <div class="cs-breadcrumb">
            <a href="{{ theme_home_url() }}">{{ __('Home') }}</a>
            <span class="cs-breadcrumb-sep"><i class="las la-angle-right"></i></span>
            <span class="cs-breadcrumb-current">{!! $page_post->title !!}</span>
        </div>
    </div>
</div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-3">
            ...
            <div class="cs-sidebar-card">
                <div class="cs-sidebar-title">{{ __('Search') }}</div>
                <form class="cs-search-form" onsubmit="csFilterRequest();return false;">
                    <input type="text" id="cs-search-input" class="cs-search-input" placeholder="{{ __('Search products…') }}">
                    <button type="submit" class="cs-search-btn"><i class="las la-search"></i></button>
                </form>
            </div>
...
$(function () {
    ...
    function csFilterRequest(page) {
        ...
        var search    = $('#cs-search-input').val() || null;
    ...
    // Search on Enter
    $(document).on('keydown', '#cs-search-input', function (e) {
        if (e.key === 'Enter') { csFilterRequest(); }
    });
