...
<div class="pf-dash-card">
    <div class="pf-dash-card-title"><i class="las la-shopping-bag"></i> {{ __('My Orders') }}</div>
    ...
    <div class="table-responsive">
        <table class="pf-dash-table">
            ...
            <tr>
                ...
                <td><a href="{{ theme_user_order_detail_url($order->id) }}" class="pf-btn pf-btn-teal pf-btn-sm"><i class="las la-eye"></i> {{ __('View') }}</a></td>
            </tr>
            @endforeach
            </tbody>
        </table>
    ...
    <div style="text-align:center;padding:48px;">
        <i class="las la-shopping-bag" style="font-size:48px;color:var(--pf-border);display:block;margin-bottom:12px;"></i>
        <p style="color:var(--pf-muted);font-size:14px;margin-bottom:16px;">{{ __('You haven\'t placed any orders yet.') }}</p>
        <a href="{{ theme_shop_url() }}" class="pf-btn pf-btn-teal"><i class="las la-store"></i> {{ __('Shop Now') }}</a>
    </div>
    @endif
</div>
@endsection
