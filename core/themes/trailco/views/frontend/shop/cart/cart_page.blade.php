...
<div style="background:var(--tr-bark);padding:24px 0;border-bottom:2px solid var(--tr-olive);">
    <div class="container">
        <h1 style="font-size:26px;font-weight:900;color:#fff;margin-bottom:6px;">{{ $page_title }}</h1>
...
<div class="container" style="padding-top:28px;">
    <div style="display:flex;align-items:center;gap:0;margin-bottom:28px;max-width:480px;">
        <div style="display:flex;align-items:center;gap:8px;background:var(--tr-olive);color:#fff;padding:10px 18px;border-radius:var(--tr-radius) 0 0 var(--tr-radius);font-size:13px;font-weight:700;">
            <span style="width:20px;height:20px;border-radius:50%;background:#fff;color:var(--tr-olive);display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;">1</span> {{ __('Cart') }}
        </div>
        ...
        <div style="display:flex;align-items:center;gap:8px;color:var(--tr-stone);padding:10px 18px;font-size:13px;font-weight:600;">
            <span style="width:20px;height:20px;border-radius:50%;border:2px solid var(--tr-border);display:inline-flex;align-items:center;justify-content:center;font-size:10px;">2</span> {{ __('Checkout') }}
        </div>
        <div style="flex:1;height:2px;background:var(--tr-border);"></div>
        <div style="display:flex;align-items:center;gap:8px;color:var(--tr-stone);padding:10px 18px;border-radius:0 var(--tr-radius) var(--tr-radius) 0;font-size:13px;font-weight:600;">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-{{ $wishlist ? '12' : '8' }}">
            <div style="background:#fff;border:1px solid var(--tr-border);border-radius:var(--tr-radius);overflow:hidden;">
                <div class="table-responsive">
                    <table style="width:100%;border-collapse:collapse;font-size:14px;">
                        <thead>
                            <tr style="background:var(--tr-cream);border-bottom:1px solid var(--tr-border);">
                                <th style="padding:14px 20px;text-align:left;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--tr-stone);">{{ __('Product') }}</th>
                                <th style="padding:14px 16px;text-align:left;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--tr-stone);">{{ __('Price') }}</th>
                                <th style="padding:14px 16px;text-align:left;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--tr-stone);">{{ __('Qty') }}</th>
                                <th style="padding:14px 16px;text-align:left;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--tr-stone);">{{ __('Subtotal') }}</th>
            ...
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap mt-4">
                <a href="{{ theme_shop_url() }}" class="tr-btn" style="background:transparent;border:1.5px solid var(--tr-border);color:var(--tr-bark);"><i class="mdi mdi-arrow-left"></i> {{ __('Continue Shopping') }}</a>
                @if(!$wishlist)
                <a href="javascript:void(0)" class="tr-btn clear-cart-btn" style="background:transparent;border:1.5px solid var(--tr-terra);color:var(--tr-terra);">
        ...
        <div class="col-lg-4">
            <div style="background:#fff;border:1px solid var(--tr-border);border-radius:var(--tr-radius);overflow:hidden;">
                <div style="background:var(--tr-bark);padding:14px 20px;font-size:14px;font-weight:700;color:#fff;">{{ __('Order Summary') }}</div>
                <div style="padding:24px;" class="coupon-contents">
                    @php $subtotal = \Gloudemans\Shoppingcart\Facades\Cart::subtotal(2, '.', ','); $total = \Gloudemans\Shoppingcart\Facades\Cart::priceTotal(2, '.', ','); @endphp
                    ...
                    <a href="{{ theme_checkout_url() }}" class="tr-btn tr-btn-primary" style="width:100%;justify-content:center;margin-top:16px;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:14px;font-size:12px;color:var(--tr-stone);">
                        <i class="mdi mdi-shield-check" style="color:var(--tr-olive);"></i>
...
(function ($) {
    ...
    function triggerQtyUpdate(el) { var row = el.closest('[data-product-id]'); var input = row.find('.quantity-input'); updateQty(row.data('product-id'), input.val(), row.data('varinat-id') || row.data('variant-id') || ''); }
    ...
    $(document).on('click', '.close-table-cart', function (e) {
        ...
        if (!hashId) return;
        $.ajax({ url: '{{ theme_cart_remove_url() }}', type: 'GET', data: { product_hash_id: hashId }, beforeSend: function () { $('.loader').show(); }, success: function (data) { if (data.msg) { toastr.success(data.msg); if (data.empty_cart !== '') { $('.cart-main-wrapper').html(data.empty_cart).hide().fadeIn(); } else { location.reload(); } } $('.loader').hide(); }, error: function () { $('.loader').hide(); } });
    });
    $(document).on('click', '.close-table-wishlist', function (e) {
        e.stopPropagation();
        var hashId = $(this).closest('[data-product_hash_id]').attr('data-product_hash_id');
        if (!hashId) return;
        $.ajax({ url: '{{ theme_wishlist_remove_url() }}', type: 'GET', data: { product_hash_id: hashId }, beforeSend: function () { $('.loader').show(); }, success: function (data) { if (data.msg) { toastr.success(data.msg); if (data.empty_cart !== '') { $('.cart-main-wrapper').html(data.empty_cart).hide().fadeIn(); } else { location.reload(); } } $('.loader').hide(); }, error: function () { $('.loader').hide(); } });
    });
    $(document).on('click', '.move-to-wishlist', function (e) {
        e.stopPropagation();
        var hashId = $(this).closest('[data-product_hash_id]').attr('data-product_hash_id');
        if (!hashId) return;
        $.ajax({ url: '{{ theme_wishlist_move_url() }}', type: 'GET', data: { product_hash_id: hashId }, beforeSend: function () { $('.loader').show(); }, success: function (data) { if (data.msg) { toastr.success(data.msg); if (data.empty_cart !== '') { $('.cart-main-wrapper').html(data.empty_cart).hide().fadeIn(); } else { location.reload(); } } $('.loader').hide(); }, error: function () { $('.loader').hide(); } });
    });
