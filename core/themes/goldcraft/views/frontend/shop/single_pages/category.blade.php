...
<div style="background:var(--gc-warm);border-bottom:1px solid var(--gc-border);padding:36px 0 28px;">
    <div class="container">
        <h1 style="font-size:13px;font-weight:400;letter-spacing:4px;text-transform:uppercase;color:var(--gc-rose);margin-bottom:8px;">{!! $category->name !!}</h1>
...
<div style="background:var(--gc-ivory);border-bottom:1px solid var(--gc-border);">
    <div class="container">
        <div style="display:flex;align-items:center;gap:8px;padding:12px 0;overflow-x:auto;flex-wrap:nowrap;">
            <button class="gc-cat-nav-btn" data-slug=""
                    style="flex-shrink:0;padding:6px 18px;border-radius:var(--gc-radius);border:1.5px solid var(--gc-border);background:transparent;color:var(--gc-muted);font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;cursor:pointer;transition:all .2s;font-style:normal;">
                {{ __('All') }}
            </button>
            @foreach($categories as $cat)
            <button class="gc-cat-nav-btn" data-slug="{{ $cat->slug }}"
                    style="flex-shrink:0;padding:6px 18px;border-radius:var(--gc-radius);border:1.5px solid {{ $cat->slug === ($category->slug ?? '') ? 'var(--gc-rose)' : 'var(--gc-border)' }};background:{{ $cat->slug === ($category->slug ?? '') ? 'var(--gc-rose)' : 'transparent' }};color:{{ $cat->slug === ($category->slug ?? '') ? '#fff' : 'var(--gc-muted)' }};font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;cursor:pointer;transition:all .2s;font-style:italic;">
                {{ $cat->name }}
            </button>
            @endforeach
        </div>
        ...
        <div class="col-lg-3">
            ...
            <div style="background:var(--gc-warm);border:1px solid var(--gc-border);border-radius:var(--gc-radius);padding:24px;margin-bottom:20px;box-shadow:var(--gc-shadow);">
                ...
                <div style="display:flex;border:1.5px solid var(--gc-border);border-radius:var(--gc-radius);overflow:hidden;">
                    <input type="text" id="gc-search-input" placeholder="{{ __('Search products…') }}"
                        style="flex:1;border:none;outline:none;padding:8px 12px;font-size:13px;background:#fff;font-family:Georgia,serif;color:var(--gc-dark);">
                    <button type="button" onclick="gcFilterRequest()"
                        style="background:var(--gc-rose);color:#fff;border:none;padding:0 14px;cursor:pointer;">
            ...
            <div style="background:var(--gc-warm);border:1px solid var(--gc-border);border-radius:var(--gc-radius);padding:24px;box-shadow:var(--gc-shadow);">
                ...
                <div style="margin-top:12px;">
                    <a href="javascript:void(0)" onclick="$('.gc-rating-filter,.gc-cat-checkbox-input').prop('checked',false);gcFilterRequest();"
                        style="font-size:12px;color:var(--gc-muted);text-decoration:none;font-style:italic;">
...
$(function () {
    function gcFilterRequest(page) {
        ...
        var search    = $('#gc-search-input').val() || null;
    ...
    $(document).on('keydown', '#gc-search-input', function (e) {
        if (e.key === 'Enter') { gcFilterRequest(); }
    });
