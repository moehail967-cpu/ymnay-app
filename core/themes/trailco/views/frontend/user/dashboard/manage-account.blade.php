...
<div class="tr-dash-card" style="margin-bottom:20px;overflow:hidden;padding:0;">
    <div style="display:flex;border-bottom:1px solid var(--tr-border,#e0e0e0);overflow-x:auto;">
        @foreach([
            ['profile',  'las la-user-edit',  __('Profile')],
            ['address',  'las la-map-marker', __('Address')],
            ['password', 'las la-lock',       __('Password')],
        ] as [$key, $icon, $label])
        <button class="tr-account-tab" data-tab="{{ $key }}" type="button"
                style="padding:13px 22px;background:none;border:none;border-bottom:3px solid transparent;font-size:13px;font-weight:700;color:var(--tr-muted,#888);cursor:pointer;white-space:nowrap;transition:all .2s;display:flex;align-items:center;gap:7px;">
...
{{-- ===== Profile Tab ===== --}}
<div class="tr-account-panel" id="tr-tab-profile">
    <div class="tr-dash-card">
        <div class="tr-dash-card-header">{{ __('Edit Profile') }}</div>
        <div class="tr-dash-card-body">
            <form class="profile-edit-form" action="#" enctype="multipart/form-data">
                @csrf
                <div style="display:flex;align-items:center;gap:16px;padding:14px 16px;background:var(--tr-bg,#f5f7fa);border:1px solid var(--tr-border,#e0e0e0);border-radius:var(--tr-radius,6px);margin-bottom:20px;">
                    <x-fields.media-upload :name="'image'" :title="''" :id="$user_details->image"/>
                    <div>
                        <div style="font-size:14px;font-weight:700;color:var(--tr-dark,#1a1a1a);margin-bottom:4px;">{{ __('Profile Photo') }}</div>
                        <div style="font-size:12px;color:var(--tr-muted,#888);">{{ __('Click the image to upload') }}</div>
                    </div>
                </div>
                <div class="row g-3">
                    <div class="col-md-6 single-input">
                        <label class="label-title">{{ __('Full Name') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                        <input class="form--control" type="text" name="name" value="{{ $user_details->name }}" placeholder="{{ __('Your name') }}">
                    </div>
                    <div class="col-md-6 single-input">
                        <label class="label-title">{{ __('Email Address') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                    ...
                    <div class="col-12">
                        <button type="submit" class="tr-btn tr-btn-primary profile-submit-btn">
...
{{-- ===== Address Tab ===== --}}
<div class="tr-account-panel" id="tr-tab-address" style="display:none;">
    <div class="tr-dash-card">
        <div class="tr-dash-card-header">{{ __('Address Book') }}</div>
        <div class="tr-dash-card-body">
            <form class="address_form" action="#" id="tr_address_form">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6 single-input">
                        <label class="label-title">{{ __('Full Name') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                    ...
                    <div class="col-md-6 single-input">
                        <label class="label-title">{{ __('Email') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                        <input class="form--control" type="email" name="email" value="{{ $delivery?->email }}" placeholder="{{ __('Email address') }}">
                    </div>
                    <div class="col-md-6 single-input">
                        <label class="label-title">{{ __('Phone') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                    ...
                    <div class="col-md-4 single-input">
                        <label class="label-title">{{ __('Country') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                        <select class="form--control countryField" name="country" id="trAddressCountryField" style="cursor:pointer;">
                            <option value="">{{ __('Select country') }}</option>
                            @foreach($countries ?? [] as $country)
                    ...
                    <div class="col-md-4 single-input">
                        <label class="label-title">{{ __('State') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                        <select class="form--control stateField" name="state" id="trAddressStateField" style="cursor:pointer;">
                    ...
                    <div class="col-12">
                        <button type="submit" class="tr-btn tr-btn-primary address-submit-btn">
...
{{-- ===== Password Tab ===== --}}
<div class="tr-account-panel" id="tr-tab-password" style="display:none;">
    <div class="tr-dash-card">
        <div class="tr-dash-card-header">{{ __('Change Password') }}</div>
        <div class="tr-dash-card-body">
            <form class="change_password_form" action="#">
                @csrf
                <div class="row g-3" style="max-width:480px;">
                    <div class="col-12 single-input">
                        <label class="label-title">{{ __('Current Password') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                        <input class="form--control" type="password" name="old_password" placeholder="{{ __('Enter current password') }}">
                    </div>
                    <div class="col-12 single-input">
                        <label class="label-title">{{ __('New Password') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                        <input class="form--control" type="password" name="password" placeholder="{{ __('Enter new password') }}">
                    </div>
                    <div class="col-12 single-input">
                        <label class="label-title">{{ __('Confirm New Password') }} <span style="color:var(--tr-primary,#2e7d32);">*</span></label>
                        <input class="form--control" type="password" name="password_confirmation" placeholder="{{ __('Confirm new password') }}">
                    </div>
                    <div class="col-12">
                        <button type="submit" class="tr-btn tr-btn-primary save-password-btn">
...
$(function () {
    function trTab(key) {
        $('.tr-account-panel').hide();
        $('.tr-account-tab').css({'color':'var(--tr-muted,#888)','border-bottom-color':'transparent'});
        $('#tr-tab-' + key).show();
        $('.tr-account-tab[data-tab="' + key + '"]').css({'color':'var(--tr-primary,#2e7d32)','border-bottom-color':'var(--tr-primary,#2e7d32)'});
    }
    trTab('profile');
    $(document).on('click', '.tr-account-tab', function () { trTab($(this).data('tab')); });
    ...
    $(document).on('change', '#trAddressCountryField', function () {
        $.post('{{ theme_state_search_url() }}', { _token: '{{ csrf_token() }}', country: $(this).val() }, function (data) {
            var sf = $('#trAddressStateField').empty().append('<option value="">{{ __("Select a state") }}</option>');
