...
@section('content')
    ...
    <section class="hf-page-section">
        <div class="container">
            ...
            <!-- Campaign Hero -->
            <div class="hf-campaign-hero">
                <div class="hf-campaign-meta">
                    <h1 class="hf-campaign-title">{!! $campaign->title !!}</h1>
                    @if(!empty($campaign->description))
                        <p class="hf-campaign-desc">{!! $campaign->description !!}</p>
                    @endif
                </div>
                <div class="hf-countdown flash-countdown-camp flash-countdown" data-date="{{ $campaign->end_date }}">
                    <div class="hf-countdown-item">
                        <span class="hf-countdown-value counter-days item"></span>
                        <span class="hf-countdown-label item">{{ __('Days') }}</span>
                    </div>
                    <div class="hf-countdown-sep">:</div>
                    <div class="hf-countdown-item">
                        <span class="hf-countdown-value counter-hours item"></span>
                        <span class="hf-countdown-label item">{{ __('Hours') }}</span>
                    </div>
                    <div class="hf-countdown-sep">:</div>
                    <div class="hf-countdown-item">
                        <span class="hf-countdown-value counter-minutes item"></span>
                        <span class="hf-countdown-label item">{{ __('Mins') }}</span>
                    </div>
                    <div class="hf-countdown-sep">:</div>
                    <div class="hf-countdown-item">
                        <span class="hf-countdown-value counter-seconds item"></span>
                        <span class="hf-countdown-label item">{{ __('Secs') }}</span>
                    </div>
                </div>
            </div>
                        ...
                        <div class="hf-card">
                            <div class="hf-card-img-wrap">
                                <a href="{{ to_product_details($product->slug) }}">
                                    <img class="hf-card-img" src="{{ $img_url }}" alt="{{ $img_alt }}">
                                </a>

                                <!-- Wishlist -->
                                <button class="hf-card-wish add-to-wishlist-btn" data-product_id="{{ $product->id }}" title="{{ __('Add to Wishlist') }}">
                            ...
                            <div class="hf-card-body">
                                <a href="{{ to_product_details($product->slug) }}" class="hf-card-name">
                                    {{ product_limited_text($product->name, 'title') }}
                                </a>

                                <!-- Countdown per card -->
                                <div class="hf-card-countdown flash-countdown-camp flash-countdown" data-date="{{ $campaign->end_date }}" style="display:flex;gap:6px;font-size:11px;color:#E8603C;font-weight:700;margin:4px 0;">
                                ...
                                <div class="hf-card-footer">
                                    <div class="hf-card-prices">
                                        <span class="hf-card-price flash-prices">{{ amount_with_currency_symbol($final_price) }}</span>
                                        @if(!empty($regular_price) && $regular_price != $final_price)
                                            <span class="hf-card-old flash-old-prices">{{ amount_with_currency_symbol($regular_price) }}</span>
                                        @endif
                                    </div>

                                    @if($product->inventory_detail_count < 1)
                                        <button class="hf-card-atc add-to-cart-btn cart-loading"
                                                data-product_id="{{ $product->id }}"
                                                title="{{ __('Add to Cart') }}">
                                            <i class="las la-shopping-bag"></i>
                                        </button>
                                    @else
                                        <button class="hf-card-atc product-quick-view-ajax cart-loading"
