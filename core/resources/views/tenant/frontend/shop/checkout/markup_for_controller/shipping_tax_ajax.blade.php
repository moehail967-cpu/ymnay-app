    ...
    <ul class="coupon-contents-details-list coupon-border">
        ...
        @foreach($shipping_methods ?? [] as $key => $method)
            <li class="coupon-contents-details-list-item" data-country="{{$country}}" data-state="{{$state}}">
                ...
                <span>{{amount_with_currency_symbol((float) apply_filters('nazmart:product_price', (float) ($method['options']['cost'] ?? 0), null))}}</span>
            </li>
        @endforeach
    </ul>
@endif
            ...
            @php
                if (isset($coupon)) {
                    if ($coupon['discount_type'] == 'amount') {
                        $discount = amount_with_currency_symbol((float) apply_filters('nazmart:product_price', (float) $coupon['discount'], null));
                    } else {
...
<ul class="coupon-contents-details-list coupon-border">
    ...
    @endphp
    <li class="coupon-contents-details-list-item price-total" data-total="{{round($total, 2)}}">
